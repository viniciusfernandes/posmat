...
\begin{document}
	...
	\textbf{Definição (grau de aplicação):} Sejam $M^{n}, N^{n}$ n-variedades diferenciáveis, conexas, compactas e orientadas. Seja $f: M \to N$ uma aplicação suave, então podemos afirmar que ose seguintes mapas $f^{*}: H^{n}(M) \to H^{n}(N)$, $\int_{M}: H^{n}(M) \to \mathbb{R}$ e $\int_{N}: H^{n}(N) \to \mathbb{R}$ são isomorfismos. Vamos definir uma aplicação de modo que o diagrama abaixo seja comutativo.
	\\
	\begin{tikzpicture}
