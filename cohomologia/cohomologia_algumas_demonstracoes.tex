...
\begin{document}
	...
	\textbf{Lema (integração é invariante parametrização):} Sejam $(U_{i}, h_{i}),(U_{j}, h_{j})$ duas cartas em $M$ tais que $U_{i} \cap U_{j} \neq \emptyset$, então a função de transição $h_{ij} : U_{i} \cap U_{j} \to U_{i} \cap U_{j}$ é um difeomorfismo, então dada $\omega \in \omega^{n}_{c}(V)$ teremos:	
