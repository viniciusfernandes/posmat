...
\begin{document}
	...
	\textbf{Teorema:} Sejam $M^{n}$ uma variedade diferenciável, conexa e orientável, então a sequinte sequência é exata:
	$$
	\Omega^{n-1}_{c}(M) \xrightarrow{d} \Omega^{n}_{c}(M) \xrightarrow{\int_{M}} \mathbb{R}
	$$
	
	$\square$ Seja $\omega \neq 0 \in Im(d)$, então $\omega = d\alpha $ e pelo teorema de Stokes temos que $\int_{M} d\alpha = \int_{\partial M} i^{*}(\alpha) = 0$(???), logo, $Im(d) \subset Ker(\int_{M})$. Por outro lado, tomando $\omega \neq 0 \in Ker(\int_{M})$, isto é, $\int_{M} \omega = 0$, e como os coeficientes dessa $n$-forma são suáveis, então $\omega = 0$
	
	$\int_{M} : \Omega^{n}_{c}(M) \to \mathbb{R}$ é um 
