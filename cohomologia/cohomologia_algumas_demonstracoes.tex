...
\begin{document}
	...
	$\square$ Seja $N : S \to S^{2}$ o mapa de Gauss da superfície $S$, logo $D_{p}N :T_{p}S \to T_{N(p)}S^{2} $, e utilizando o resultado já conhecido de teoria das súperfícies regulares, podemos afirmar que $K(p) = det(D_{p}N)$. Seja $q \in S$ tal que $N(q) \in S^{2}$ seja polo norte da esferea, isto é, $N(q) = (0,0,1)$. Defina agora $f : S^{2} \to \mathbb{R}$ tal que $f(p) = ||p_{+} - p||$, onde $p_{+} =(0,0,1) \in S^{2}$. É imediato que $f$ é uma função suave, além disso, atinge um mínimo quando $p = p_{+}$ e um máximo quando $p = p_{-} = (0,0,-1)$, que é o pólo sul da esféra, isto é, $D_{p_{+}}f = D_{p_{-}}f= 0$, logo $\{p_{+}, p_{-}\} \subset S^{2}$ são pontos críticos de $f$. Vimos também que nesse ponto as derivadas de segunda ordem não se anulam, assim, quando $q \in \{p_{+}, p_{-}\}$ teremos $det(D_{q}^{2}f) \neq 0$, logo, $f(q) = 0 \in \mathbb{R}$ é um valor regular de $f$.
\end{document}