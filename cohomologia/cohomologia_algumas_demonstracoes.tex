...
\begin{document}
	...
	{  
		...
		\\
		H^{n - r+1}(U)^{*} \oplus H^{n-r+1}(U)^{*} & H^{n - r+1}(U \cap V)^{*} & H^{n - r}(M)^{*} & H^{n - r}(U)^{*} \oplus H^{n - r}(V)^{*} & H^{r}(U \cap V)^{*}
		\\
	};
	{ [start chain] 
		\chainin (m-1-1);
	...
	$\square$ Vejamos que cada um dos diagramas comutam, isto é: 
	$$
	\begin{aligned}
	(I_{U \cap V} \circ J^{*})([\gamma],[\beta]) 
	=& \int_{U \cap V} \alpha \wedge (\gamma - \beta)
	\\
	=& \int_{U \cap V} \alpha \wedge \gamma - \int_{U \cap V} \alpha \wedge \beta
	\\
	=& 	I_{U}[\gamma]  -I_{V}([\beta])
	\\
	=& 	(I_{U} \oplus -I_{V})([\gamma],[\beta])
	\\
	=& 	(I^{T} \circ I_{U} \oplus -I_{V})([\gamma],[\beta]).
	\\
	(I_{M} \circ \partial^{*})(\beta) =& sssss
	\end{aligned}
	$$
\end{document}