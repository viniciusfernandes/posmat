...
\begin{document}
	...
	Quando trabalhamos com integração em variedades não-compactas operacionamos sobre formas com suporte compacto, caso as variedades sejam compactas, então esse caso cobre o anterior. Assim, tomemos $M^{n}$ uma variedade diferenciável e $\omega \in \Omega_{c}^{n}(M)$, e por definição, $\int_{M}\omega \in \mathbb{R}$, isto é, $\int_{M} \in \Omega_{c}^{n}(M)^{*}$ pois é um operador linear, por construção. Como, a partir da operação de integração, construir um funcional linear em $\Omega_{c}^{p}(M)^{*}$ onde $1 \leq p \leq n-1$. Seja $0\leq k \leq n$ e reescreveremos $p = n-k$, sendo que $k=0$ é o caso conhecido. Para cada $k$ tome $\alpha \in \Omega^{k}(M)$ e definindo $I : \Omega^{k}(M) \to \Omega_{c}^{n-k}(M)^{*}$ tal que $I(\alpha)(\omega) = \int_{M}\alpha \wedge \omega \in \mathbb{R}$. Note que através do produto exterior conseguimos um funcional para cada $p = n-k$, podendo integrar na variedade, como desejávamos. Note que, temos $supp(\alpha\wedge \omega) = supp(\alpha) \cap supp(\omega) \subset supp(\omega)$, logo a integração $\int_{supp(\alpha \wedge \omega  )}\alpha \wedge \omega  \leq \int_{supp(\omega)}\alpha \wedge \omega < \infty $, e o funcional $I(\alpha)$ esta bem-definido. 
	
	\vspace{2mm}
	\textit{\textbf{Proposição (funcional linear da homologia):} Seja $M^{n}$ uma variedade diferenciável e orientável, então o mapa $I: H^{k}(M) \to H^{n-k}(M)^{*}$ dado por $I([\alpha])([\omega]) = \int_{M} \alpha \wedge \omega$ resulta em um funcional linear e esta bem definido, e por simplicidade o denotaremos por $I_{M}$, logo, $I_{M}([\omega]) = \int_{M} \alpha \wedge \omega$.}
	
	$\square$ Tomemos $[\alpha] = [\beta] \in H^{k}(M)$ e um $[\omega] \in H^{n-k}(M)^{*}$, então
	$$
	\begin{aligned}
	I([\alpha])([\omega]) =& \int_{M} \alpha \wedge \omega = \int_{M} (\beta + d\lambda)\wedge \omega
	\\
	...
	$$
	I_{M}(a[\beta]+[\omega] ) = I_{M}[a\beta+\omega] = \int_{M}\alpha \wedge (a\beta+\omega) = a\int_{M}\alpha \wedge \beta + \int_{M}\alpha \wedge \omega = aI_{M}([\beta]) + I_{M}([\omega] ),
	$$ 
	portanto é linear.
	...
	{ [start chain] 
		...
		{ [start branch=A] \chainin (m-2-1)
			[join={node[right,labeled] {I_{U} \oplus -I_{V}}}];
		}
		\chainin (m-1-2) [join={node[above,labeled] {J^{*}} }];
		{ [start branch=A] \chainin (m-2-2)
			[join={node[right,labeled] {I_{U \cap V}}}];
		}
		\chainin (m-1-3) [join={node[above,labeled] {\partial^{*} }}];
		{ [start branch=B] \chainin (m-2-3)
			[join={node[right,labeled] { I_{M} }}];
		}
	...
	$\square$ Primeiramente, vamos definir as seguintes operações indicadas no diagrama: $J^{T}$ e $I^{T}$ tais que $J^{T}(I_{M}) = I_{U} \oplus -I_{V}$ e $I^{T}(I_{U} \oplus I_{V}) = I_{U \cap V}$. Vejamos agora que cada um dos diagramas comutam, isto é: 
	$$
	\begin{aligned}
	(I_{U \cap V} \circ J^{*})([\gamma],[\beta]) 
	=& I_{U \cap V} ([\gamma] - [\beta])
	= I_{U \cap V}([\gamma])  -I_{U \cap V}([\beta])
	=I_{U \cap V}([\gamma]) \oplus -I_{U \cap V}([\beta]),
	\\
	(I^{T} \circ I_{U} \oplus -I_{V} )([\gamma],[\beta]) =& I^{T}( I_{U}([\gamma]) \oplus -I_{V}([\beta])) = I_{U \cap V}([\gamma]) \oplus -I_{U \cap V}([\beta]),
	\end{aligned}
	$$
	como $([\gamma],[\beta]) \in H^{r-1}(U) \oplus H^{r-1}(V)$ é arbitrário, então $I_{U \cap V} \circ J^{*} = I^{T} \circ I_{U} \oplus -I_{V}$. Analogamente, temos:
	$$
	\begin{aligned}
	(I_{U} \oplus -I_{V} \circ I^{*} )([\gamma]) =& I_{U} \oplus -I_{V} ([\gamma|_{U}], [\gamma|_{V}]) = I_{U}([\gamma]) \oplus -I_{V} ([\gamma]),
	\\
	(J^{T} \circ I_{M} )([\gamma]) 
	=& J^{T}(I_{M} ([\gamma]))
	= I_{U}([\gamma])  \oplus -I_{V}([\gamma]),	\end{aligned}
	$$
	como $[\gamma] \in H^{r}(M)$ é arbitrário, então $(I_{U} \oplus -I_{V} \circ I^{*} ) = (J^{T} \circ I_{M} )$. Por fim, temos também que:
	$$
	\begin{aligned}
	(I_{M} \circ \partial^{*} )([\gamma]) =& I_{M}(\partial^{*} [\gamma]),
	\\
	(\partial^{T} \circ I_{U \cap V} )([\gamma]) 
	=& \partial^{T}(I_{U \cap V} ([\gamma]))
	= I_{M}(\partial^{*} [\gamma]),
	\end{aligned}
	$$
	novamente, como $[\gamma] \in H^{r-1}(U \cap V)$ é arbitrário, então $I_{M} \circ \partial^{*} = \partial^{T} \circ I_{U \cap V}$, como desejávamos. Assim, o diagrama acima é comutativo.
