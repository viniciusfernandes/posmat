...
\begin{document}
	...
	\vspace{2mm}
	\textit{\textbf{Teorema (existência da Dualidade de Poincaré):} Seja $M^{n}$ uma n-variedade diferenciável orientável, então existe $U \subseteq M$ tal que $D_{U} : \cohomologia{p}{U} \to \cohomologiacompacdual{n-p}{U}$ onde $0 \leq p \leq n$.}
	
	$\square$ Seja $(V, h)$ uma carta de $M$, e por definição, $U = h(V) \subseteq M$ é homeomorfo a bola aberta $B_{r}(x) \subseteq \real{n}$, consequentemente, $\cohomologia{p}{U} \cong \cohomologia{p}{V}$. Contudo, toda bola aberta $B_{r}(x)$ é homeomorfa a $\real{n}$, logo $\cohomologia{p}{U} \cong \cohomologia{p}{\real{n}}$, e tal cohomologia é dada por $\cohomologia{0}{\real{n}} \cong \real{}$ e $\cohomologia{p}{\real{n}} = 0$ para $p>0$. Assim, temos apenas o caso em que $D_{U}: \cohomologia{0}{U} \to \cohomologiacompacdual{n}{U}$ para examinar. Tomando as classes das funções constantes $[a] \in \cohomologia{0}{U}$ e $[\omega] \in \cohomologiacompac{n}{U}$ tal que $\int_{U}\omega = 1$, então $D_{U}([a])([\omega]) = \int_{U} a\omega = a\int_{U} \omega = a \in \real{}$. Podemos ver que é injetora pois $D_{U}([a])([\omega]) = a = 0 \Rightarrow [a] = [0]$, portanto $Ker(D_{U}) = \{0\}$, e é sobrejetora pois, dado $b \in \real{}$, podemos tomar um $[b] \in \cohomologia{0}{U}$ tal que $D_{U}([b])([\omega]) = b \in \real{}$. Por construção $D_{U}$ é uma aplicação linear, logo é um isomorfismo. Para todo $0\leq p \leq n$ temos a relação de continência $\cohomologiacompac{p}{U} \subseteq \cohomologia{p}{U}$, e nos casos em que $p>0$ temos $\cohomologia{p}{U} = 0 \Rightarrow \cohomologiacompac{n-p}{U} \subseteq \cohomologia{n-p}{U} = 0$, logo $\cohomologiacompac{n-p}{U}=0$ e $D_{U}$ é o isomorfismo trivial. Portanto $D_{U}$ é um isomorfismo para $0\leq p \leq n$, como desejávamos.
	...
	\begin{enumerate}
		\item $\cohomologia{p}{U} \to \prod_{\alpha \in J} \cohomologia{p}{U_{\alpha}}$, dado por $[\omega] \mapsto ([i^{*}_{\alpha}(\omega)])_{\alpha \in J}$
		
		\item $\bigoplus_{\alpha \in J} \cohomologiacompac{p}{U_{\alpha}} \mapsto \cohomologiacompac{p}{U}$, dado por $\bigoplus_{\alpha \in J}[\omega_{\alpha}] \mapsto \sum_{\alpha \in J} i^{*}_{\alpha}([\omega_{\alpha}])$.
	\end{enumerate}
	...
	Para 2) como os subconjuntos abertos de $M$ são dois a dois disjuntos, então, pelo \textit{lema} anterior, temos os isomorfismos $\cohomologia{p}{U} \cong \bigoplus_{\alpha} \cohomologia{p}{U_{\alpha}}$ e $\cohomologiacompacdual{n-p}{U} \cong \bigoplus_{\alpha} \cohomologiacompacdual{n-p}{U_{\alpha}}$. Definindo $\bigoplus_{\alpha}D_{U_{\alpha}} : \bigoplus_{\alpha} \cohomologia{p}{U_{\alpha}} \to \bigoplus_{\alpha} \cohomologiacompacdual{n-p}{U_{\alpha}}$ tal que $\bigoplus_{\alpha}D_{U_{\alpha}} [\omega_{\alpha}]$, e como $D_{U_{\alpha}}$ é um isomorfismo para para $\alpha \in J$, então $\bigoplus_{\alpha}D_{U_{\alpha}}$ é um isomorfismo. Pelo lema anterior, vimos que $\phi : \cohomologia{p}{U} \to \bigoplus_{\alpha} \cohomologia{p}{U_{\alpha}}$ e $\psi : \cohomologiacompacdual{n-p}{U} \to \bigoplus_{\alpha} \cohomologiacompacdual{n-p}{U_{\alpha}}$ são isomorfismos, e pela sequência de Mayer-Vietoris temos a comutatividade do diagrama expressa pela comutação dos seguintes morfismos $(\bigoplus_{\alpha}D_{U_{\alpha}}) \circ \phi = \psi \circ D_{U}$, e como $(\bigoplus_{\alpha}D_{U_{\alpha}}) \circ \phi$ é um isomorfismo pois é a composição de isomorfismos, então $\psi \circ D_{U}$ também é um isomorfismo. Sabemos que, por construção, $D_{U}$ é linear e sobrejetor. Tomando $[\omega] \in \cohomologia{p}{U}$ temos $(\psi \circ D_{U})([\omega]) = \bigoplus_{\alpha}D_{U_{\alpha}}([\omega_{\alpha}]) =: D$, e como é um isomorfismos, podemos inverter a operação $[\omega] = (\psi \circ D_{U})^{-1}(D)$ o que implica que $D_{U}$ deve ser injetor, pois do contrário não poderemos inverter a operação, logo é um isomorfismo.
	
	Para 3) por hipótese $U \subseteq M$ é difeomorfo a $\real{n}$, logo $\cohomologia{p}{U} \cong \cohomologia{p}{\real{n}}$ e $\cohomologiacompac{p}{U} \cong \cohomologiacompac{p}{\real{n}}$, e temos que $\cohomologia{0}{\real{n}} \cong \real{}$ e $\cohomologia{p}{\real{n}} = 0$ caso $p >0$, e como $\cohomologiacompac{p}{U} \subseteq \cohomologia{p}{U}$ teremos $\cohomologiacompac{0}{U} = \cohomologia{0}{U} \cong \real{}$ e $\cohomologiacompac{p}{U} = \cohomologia{p}{U} = 0$ caso $p>0$. Temos as dualidades entre os grupos de hologogia de ordem $p$, mas temos apenas um caso não-trivial que é $p=0$, como já vimos, logo $D_{U}: \cohomologia{0}{U} \to \cohomologiacompacdual{n}{U}$. Tomando uma função constante $[a] \in \cohomologia{0}{U}$ teremos $D_{U}([a]) = \int_{U} a : \cohomologiacompac{n}{U} \to \real{}$, e já vimos em caso de suportes compactos, esse funcional linear é um isomorfismo com a reta real, logo $\cohomologiacompacdual{n}{U} \cong \real{}$, pois tomando $[\omega] \in \cohomologiacompac{n}{U}$ com $\int_{U}\omega = 1$, teremos $D_{U}([a])([\omega]) = \int_{U} a \omega = a\int_{U}  \omega = a \in \real{}$, logo, $D_{U}([a])$ é uma aplicação linear injetiva (pois $Ker(D_{U}) = 0$) e sobrejetiva (pois dado $b \in \real{}$ teremos $D_{U}([b])([\omega]) = b$), portanto um isomorfismo.
