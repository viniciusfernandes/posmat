...
\newcommand{\curvasfechadascirculo}[2]{L([#1,#2], S^{1})}
\begin{document}
	...
	\begin{definicao}\label{definicao_matriz_positiva_definida}
		(Matrizes positiva-definidas) Sejam $V$ um k-espaço vetorial real  e $A \in \generalgroupreal{k}$, então dizemos que $A$ é uma matriz positiva-definida se $A=A^{t}$ e para qualquer $v\in V$ temos $v^{t}Av > 0$. Denotamos $Sym_{+}(k)$ como sendo o conjunto das matrizes positivas-definidas
	\end{definicao}
	...
	\begin{lema}\label{lema_caracterizacao_matriz_normal}
		(Caracterização matriz normal) Seja $A\in \matrizquadreal{k}$ uma matriz normal, isto é, $A^{t}A=AA^{t}$. Afirmo que existem uma matriz diagonal $D \in \matrizquadreal{k}$ positiva-definida e uma matriz $S\in \matrizortogonal{k}$ tal que $A=SDS^{t}$. Nesse caso dizemos que $A$ é uma matriz normal e seus auto-valores são positivos-definidos. 
	\end{lema}
	\begin{prova}
		\vermelho{Veja em ........}
	\end{prova}
	...
	\begin{lema}
		(Raíz de matriz normal) Seja $V$ um k-espaço vetorial e $A\in \matrizquadreal{k}$ uma matriz normal, então existe $P\in \matrizquadreal{k}$ \vermelho{positiva-definida} tal que $A=P^{2}$. 
	\end{lema}
	\begin{prova}
		Como $A$ é normal, então pelo Lema $\ref{lema_caracterizacao_matriz_normal}$ podemos escrever $A=SDS^{t}$, onde  $D$ é sua matriz diagonal $D=(D_{ii})$ e suas entradas são positivos-definidos e $S\in \matrizortogonal{k}$, logo podemos definir $C \in \matrizquadreal{k}$ como sendo $C = (\sqrt{D_{ii}})$, o que implica em $C^{2} = D$. Definindo $P = SCS^{t}$ teremos $P^{2} = SCS^{t}SCS^{t} = SC^{2}S^{t} = SDS^{t}=A$, como desejávamos.
	\end{prova}
	...
	\begin{lema}
		$\matrizsimetricapositiva{k}$ é conexo por caminhos, logo é conexo.
	\end{lema}
	\begin{prova}
		Sejam $V$ um k-espaço vetorial real, $A, B \in \matrizsimetricapositiva{k}$ e $\gamma : [0,1] \to \matrizquadreal{k}$ tal que $\gamma(\lambda) = \lambda A +(1-\lambda)B$, então é simétrica pois é a combinação linear de matrizes simétricas, e para qualquer $v \in V$:
		$$
		v^{t}\gamma(t)v = v^{t}(\lambda A +(1-\lambda)B)v = \lambda v^{t}Av + (1-\lambda)v^{t}Bv \geq 0,
		$$
		pois é a soma de valores não-negativos, logo $\gamma([0,1]) \subset \matrizsimetricapositiva{k}$. Como $A, B \in \matrizsimetricapositiva{k}$ são arbitrários e são conectados por um caminho, então $\matrizsimetricapositiva{k}$ é conexo por caminhos, logo é conexo.
	\end{prova}
	
	\begin{observacao}
		Note que $\mathcal{U}$ é conexo pois $\matrizunitaria{n}$ é conexo \vermelho{ver referencia}, e como a conexidade é preservada por funções contínuas, então $F(\matrizunitaria{n}) = \mathcal{U}$ é conexo.
	\end{observacao}
	...
	\end{teorema}
	\begin{prova}
		Dado $A \in \gruposimpletico{2n}$, então pelo Teorema $\ref{teorema_decomposicao_polar}$ podemos escrever $A=PO$. Pela unicidade da decomposição polar podemos definir a aplicação injetora $G: \gruposimpletico{2n} \to \matrizsimetricapositiva{2n}\times \mathcal{U}$ tal que $G(A) = (P,O)$. Por outro lado, dado $(P,O) \in \matrizsimetricapositiva{2n}\times \mathcal{U}$ temos $(PO)^{t}J_{0}PO = O^{t}P^{t}J_{0}PO = O^{t}J_{0}O = J_{0}$ que pelo Lema $\ref{lema_caracterizacao_Sp2n}$ implica $PO \in \gruposimpletico{2n}$. Pela unicidade da decomposição polar podemos definir a aplicação $G^{-1}:\matrizsimetricapositiva{2n}\times \mathcal{U} \to \gruposimpletico{2n}$, tal que $G^{-1}(P,O) = PO \in \gruposimpletico{2n}$, logo $G^{-1} \circ G =Id$. Como $G, G^{-1}$ são aplicações contínuas, portanto um homeomorfismo, e $\matrizsimetricapositiva{2n}\times \mathcal{U}$ é conexo, pois é o produto cartesianos de espaços topológicos conexos, funções contínuas preservam a conexidade, então $G^{-1}(\matrizsimetricapositiva{2n}\times \mathcal{U}) = \gruposimpletico{2n}$ é conexo.
	\end{prova}
	
	\begin{observacao}\label{observacao_decomposicao_Sp2n}
		No teorema anterior exibimos um homeomorfismo $G:\gruposimpletico{2n} \to \matrizsimetricapositiva{2n} \times \mathcal{U}$, isto é, mostramos que se $A \in \gruposimpletico{2n}$, então ela pode ser decomposta univacamente como $A=PO$, onde $P\in \matrizsimetricapositiva{2n}$ e $O \in \mathcal{U}$. Usaremos essa afirmação na demonstração de alguns resultados adiante.
	\end{observacao}
	
	Analisaremos agora o espectro $\espectro{A}$ de um dado simpléctomorfismo $A \in \gruposimpletico{2n}$ de um determinado espaço vetorial. Tal resultado será usado na demonstração da contratibilidade do quociente $\gruposimpletico{2n}/\mathcal{U}$, que por sua vez será utilizado na construção do índice de Maslov. 
	\begin{lema}
	...
	\begin{lema}\label{lema_potenciacao_grupo_simpletico}
		(Potênciação em $\gruposimpletico{2n}$) Seja \vermelho{$\gruposimpleticopositivo{2n} = \gruposimpletico{2n} \cap \matrizsimetricapositiva{2n}$ (o conjunto das matrizes simpléticas positivas-definidas)}. Dado $A \in \gruposimpleticopositivo{2n}$, então $A^{\alpha} \in \gruposimpletico{2n}$ para qualquer \vermelho{$\alpha \in \real{}_{+}$}.
	\end{lema}
	\begin{prova}
		Seja $A \in \gruposimpleticopositivo{2n}$, então $A$ é dianonalizável. Podemos decompor $V$ na soma direta de seus auto-espaços $V_{\lambda}$, onde $\lambda \in \espectro{A}$. Sejam $u \in V_{\lambda_{u}}$ e $v \in V_{\lambda_{v}}$, então:
		$$
	...
	\end{teorema}
	\begin{prova}
		Na demonstração do Teorema $\ref{teoerma_sp2n_conexo}$ (conexidade de $\gruposimpletico{2n}$) foi mostrado que se $A \in \gruposimpletico{2n}$ temos $A=PO$, onde $P \in \matrizsimetricapositiva{2n}$ e $O \in \mathcal{U}$. Com isso temos $AA^{t} = POO^{t}P^{t} = PP^{t}=P^{2}$. Como $A,A^{t} \in \gruposimpletico{2n}$, então $AA^{t} \in \gruposimpletico{2n}$ pois $\gruposimpletico{2n}$ é um grupo, logo $AA^{t} = P^{2} \in \gruposimpletico{2n}$. Pelo Lema $\ref{lema_potenciacao_grupo_simpletico}$ anterior podemos afirmar que $P^{\alpha} \in \gruposimpletico{2n}$ para todo $\alpha \in \real{}_{+}$. Definindo a aplicação $r:\gruposimpletico{2n}\times [0,1] \to \gruposimpletico{2n}$ tal que $r(A, \alpha) = (AA^{t})^{-\alpha/2}A$ é contínua pois é o produto de matrizes, que é uma operação contínua em $\gruposimpletico{2n}$. De fato $r$ é um retrato de deformação de $\gruposimpletico{2n}$ sobre $\mathcal{U}$ pois $r(A, 0) = A$, $r(A, 1) = (AA^{t})^{-1/2}A = P^{-1}A = O \in \mathcal{U}$ e, por fim, tomando $B \in \mathcal{U}$ temos $r(B, 1) = (BB^{t})^{-1/2}B = B$, pois $BB^{t} = Id$.
	...
	Sabe-se que o grupo fundamental de $S^{1}$ é isomorfo aos inteiros, isto é, $\pi_{1}(S^{1}) \cong \mathbb{Z}$. Com isso podemos afirmar que tal grupo é um grupo cíclico infinito, pois $\mathbb{Z}$ é cíclico infinito. A cada curva fechada $\gamma:[0,1] \to S^{1}$ pode-se associar um número $deg(\gamma) \in \mathbb{Z}$, chamado de \textit{grau de $\gamma$}, de modo que duas curvas $\gamma, \beta$ em $S^{1}$ são homotópicas se, e somente se, $deg(\gamma) = deg(\beta)$ (possuem o mesmo grau). Por fim, tal aplicação é um homomorfismo pois $deg(\gamma.\beta)=deg(\gamma)+deg(\beta)$, o que induz um isomorfismo entre $\pi_{1}(S^{1})$ e $\mathbb{Z}$.
	
	Seja $exp:\real{} \to S^{1}$ a aplicação exponencial $exp(t) = e^{it}$. Pode-se mostrar o importante resultado sobre levantamento de curvas em $S^{1}$:
	
	\begin{proposicao}\label{proposicao_levantamento_curvas}
		(Levantamento de curvas) Seja $\gamma:[a,b] \to S^{1}$ uma aplicação contínua e $t_{a}\in \real{}$ tal que $\gamma(a) = exp(t_{a})$.Existe uma única aplicação contínua $\alpha:[a,b] \to \real{}$ tal que $\gamma(t) = exp(\alpha(t))$ para todo $t\in [a,b]$ e $\alpha(a) = t_{a}$. Esse levantamento é a apicação que faz com que o diagrama abaixo comute:
		$$
		\xymatrix{
	...
	Procuramos uma função que associe uma curva em $S^{1}$ a um número inteiro. Note que cada curva fechada envolve $S^{1}$ uma quantidade inteira de vezes, por exemplo: uma curva constante envolve $S^{1}$ zero vezes; uma curva homotópica a $S^{1}$ envolve 1 vez. Assim, vamos definir o grau de uma curva em $S^{1}$ como uma função com essas características.
	
	\begin{proposicao}
		(O grau de uma curva fechada) Seja $\curvasfechadascirculo{0}{1}$ o conjunto de todas as curvas contínuas e fechadas em $S^{1}$, então a aplicação $deg:\curvasfechadascirculo{0}{1} \to \mathbb{Z}$ onde $deg(\gamma) = (\alpha(1)-\alpha(0))/2\pi$ esta bem-definida e $\alpha:[0,1] \to \real{}$ é o levantamento de $\gamma$ da Proposição $\ref{proposicao_levantamento_curvas}$. Além disso, se $\gamma$ e $\beta$ são curvas homotópicas, então $deg(\gamma)=deg(\beta)$ e para o produto $\gamma.\beta \in \curvasfechadascirculo{0}{1}$ temos $deg(\gamma.\beta) = deg(\gamma)+deg(\beta)$.
	\end{proposicao}
	\begin{prova}
		Seja $\gamma \in \curvasfechadascirculo{0}{1}$. Pela Proposição $\ref{proposicao_levantamento_curvas}$ temos $\gamma(\lambda) = e^{i\alpha(\lambda)}$. Como é uma curva fechada temos $\gamma(0)=\gamma(1)$, o que implica em $e^{i\alpha(0)}=e^{i\alpha(1)}$, logo $e^{i(\alpha(1)-\alpha(0))} = 1$, portanto $\alpha(1)-\alpha(0) = 2\pi k$, onde $k\in \mathbb{Z}$. Portanto, $deg(\gamma) = (\alpha(1)-\alpha(0))/2\pi \in \mathbb{Z}$ e esta bem-definida pois para cada $\gamma \in \curvasfechadascirculo{0}{1}$ existe um único levantamento $\alpha$, logo $deg(\gamma)$ é único.
		
		Sejam $\gamma, \beta \in \curvasfechadascirculo{0}{1}$ homotópicas e $h:S^{1}\times [0,1]\to S^{1}$ a homotopia entre elas, onde $h(p,0) = \gamma(\lambda)$ e $h(p,1) = \beta(\lambda)$. Com isso, para cada $t \in [0,1]$ fixo temos $h_{t} \in \curvasfechadascirculo{0}{1}$, logo $h_{t}$ possui um único levantamento $\alpha_{t}$ tal que $(\alpha_{t}(1)-\alpha_{t}(0))/2\pi = q \in \mathbb{Z}$, portanto $deg(h_{t})=q$ é constante, logo $deg(\gamma)=deg(h_{0})=deg(h_{1})=deg(\beta)$, isto é, o curvas homotópicas possuem o mesmo grau. 
		
		Tomando o produto das curvas anteriormente definidas, temos $(\gamma.\beta)(\lambda)= e^{i\alpha(\lambda)}e^{i\xi(\lambda)} = e^{i(\alpha(\lambda)+\xi(\lambda))} \in S^{1}$ o que implica que $deg(\gamma.\beta) = ((\alpha(1)+\xi(1))-(\alpha(0)+\xi(0)))/2\pi = deg(\gamma)+deg(\beta)$.
	\end{prova}
	
	
	Agora temos ferramental para definirmos o índice de Maslov.
	
	\begin{teorema}
		Existe uma única aplicação $\mu$, chamada índice de Maslov, que associa a cada curva fechada $\gamma: [0,1] \to \gruposimpletico{2n}$ a um inteiro $\mu(\gamma) \in \mathbb{Z}$. Sejam curvas fechadas $\gamma, \beta: [0,1] \to \gruposimpletico{2n}$, então satisfaz os seguintes axiomas:
		\begin{enumerate}
			\item \textbf{(homotopia)} $\gamma$ e $\beta$ são homotópicas se, e somente se, $\mu(\gamma)=\mu(\beta)$.
			
			\item \textbf{(produto)} $\mu(\gamma.\beta) = \mu(\gamma)+\mu(\beta)$, em particular, se $\gamma$ é uma curva constante, então $\mu(\gamma) = 0$.
			
			\item \textbf{(normalização)} Se $\gamma:[0,1] \to \mathcal{U} \subset \gruposimpletico{2n}$, então $\mu(\gamma) = 1$.
		\end{enumerate}
	\end{teorema}
	\begin{prova}
		Seja $A\in \gruposimpletico{2n}$, então pela Observação $\ref{observacao_decomposicao_Sp2n}$ temos a decomposição $A=PO$. Definindo a projeção $\pi:\gruposimpletico{2n}\to \mathcal{U}$ tal que $\pi(A) = \pi(PO) = O$, que é uma aplicação conítnua. Seja $F$ o isomorfismo do Lema $\ref{lema_isomorfismo_U}$, então $F^{-1}(O) = X+iY \in \matrizunitaria{n}$. Definindo a aplicação $\rho = det\circ F^{-1}\circ \pi :\gruposimpletico{2n} \to S^{1}$ esta bem-definida. De fato, dado $B \in \matrizunitaria{n}$, então $B^{*}B = Id$, o que implica que $1=det(B^{*}B)=det(B^{*})det(B) = \overline{det(B)}det(B) = |det(B)|$, logo $det(B) \in S^{1}$. Com isso, $\rho(A) = det(F^{-1}(\pi(A))) = det(F^{-1}(O)) = det(X+iY) \in S^{1}$.
		
		Seja $\gamma:[0,1] \to \gruposimpletico{2n}$ uma curva fechada, então temos a aplicação contínua $\rho_{\gamma}=\rho\circ \gamma:[0,1] \to S^{1}$ pois é a composição de aplicações contínuas. Pela Proposição $\ref{proposicao_levantamento_curvas}$ temos $deg(\rho_{\gamma}) \in \mathbb{Z}$. Definimos como o índice de Maslov da curva $\gamma$ o número:
		$$
			\mu(\gamma) = deg(\rho \circ \gamma) = deg(\rho_{\gamma}) \in \mathbb{Z}.
		$$
		Afirmo que a aplicação $\mu$ satsifaz os axiomas de índice de Maslov. É verdade para o axioma da homotopia pois: supondo $\gamma, \beta :[0,1] \to \gruposimpletico{2n}$ sejam duas curvas fechadas homotópicas e $h:\gruposimpletico{2n}\times [0,1]\to \gruposimpletico{2n}$ a homotopia entre elas, isto é $h(p, 0) = \gamma(\lambda)$ e $h(p, 1) = \beta(\lambda)$. Afirmo que $H:S^{1}\times [0,1] \to S^{1}$ tal que $H(p, t) = (\rho \circ h)(p,t)$ é uma homotopia entre $\rho_{\gamma}$ e $\rho_{\gamma}$. De fato, $H$ é composição de aplicações contínuas, logo é contínua, e $H(p, 0) = (\rho \circ h)(p,0) = \rho (h(p,0))= \rho (\gamma(\lambda)) = \rho_{\gamma}(\lambda)$, analogamente, $H(p, 0) = (\rho \circ h)(p,0) = \rho (h(p,))= \rho_{\beta}(\lambda)$, portanto $\rho_{\gamma}$ e $\rho_{\beta}$ são homotópicas, então:
		$$
		\mu(\gamma) = deg(\rho_{\gamma}) = deg(\rho_{\beta}) = \mu(\beta).
		$$
		
		É verdade para o axioma do produto pois:
		$$
		\mu(\gamma.\beta) = deg(\rho_{\gamma}.\rho_{\beta}) = deg(\rho_{\gamma})+deg(\rho_{\beta}) = \mu(\gamma)+\mu(\beta).
		$$	
		Além disso, se $\gamma$ é curva constante o que implica que $\rho_{\gamma}$ é uma curva constante, então temos pela efinição de grau de uma curva $0=deg(\rho_{\gamma}) = \mu(\gamma)$.
	\end{prova}
