...
\begin{document}
	...
	$ é um fribrado vetorial sobre $\caminhosexponenciaisconectantesabrev$  onde $\pi: \fibradocaminhosexponenciais \to \caminhosexponenciaisconectantesabrev$ é tal que $\pi(u, X)=u$ é a projeção e $\pi^{-1}(u) = L^{p}(\pullbackfibradotangenteM{u})$ é uma fibra em $u$.
	...
	\begin{definicao}
		...
		$$
		\mapafloerparametro{u} = \derivadaparcial{u}{s} + J(u)\derivadaparcial{u}{t}+ \nabla H(u).
		$$
	\end{definicao}
	
	Pela definição de $\caminhosexponenciaisconectantesabrev \ni u$ podemos escrever $u = \aplicacaoexponencial{w}{X}$, onde $w \in \caminhosdecaimentoexponencialpadrao$ e $X \in \pullbackfibradotangenteM{w}$.
	
	
	\vermelho{Precisamos mostrar que a definicao acima tem sentido, isto é, $Im(\mapafloer) \subset L^{p}$}
