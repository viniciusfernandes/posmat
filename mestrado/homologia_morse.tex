...
\begin{document}
	...
	\begin{prova}
		...
		$$
		Afirmo que a aplicação $\mu$ satsifaz os axiomas de índice de Maslov. De fato, supondo $\gamma, \beta :[0,1] \to \gruposimpletico{2n}$ sejam duas curvas fechadas homotópicas e $h:\gruposimpletico{2n}\times [0,1]\to \gruposimpletico{2n}$ a homotopia entre elas, isto é $h(p, 0) = \gamma(\lambda)$ e $h(p, 1) = \beta(\lambda)$. Afirmo que $H:S^{1}\times [0,1] \to S^{1}$ tal que $H(p, t) = (\rho \circ h)(p,t)$ é uma homotopia entre $\rho_{\gamma}$ e $\rho_{\gamma}$. De fato, $H$ é composição de aplicações contínuas, logo é contínua, e $H(p, 0) = (\rho \circ h)(p,0) = \rho (h(p,0))= \rho (\gamma(\lambda)) = \rho_{\gamma}(\lambda)$, analogamente, $H(p, 0) = (\rho \circ h)(p,0) = \rho (h(p,))= \rho_{\beta}(\lambda)$, portanto $\rho_{\gamma}$ e $\rho_{\beta}$ são homotópicas, então:
		$$
