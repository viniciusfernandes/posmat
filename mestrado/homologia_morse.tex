...
\newcommand{\homologiarelskel}[2]{H_{#1}(X^{(#2)}, X^{(#2-1)})}
...
\begin{document}
	...
	$$
	é um $\Lambda-$módulo gerado por $n-$células de $X$ que não estão em $A$, além disso:
	$$
	\somadir{\sigma}f_{\sigma*}: \somadir{\sigma} \homologiarelcel{n}{n}{\sigma} \to \homologiarelskel{n}{n}
	$$
	...
	\xymatrix{
		...
		\homologiarelskel{k}{n}.
	}
	\]
	Por fim, sabemos que $\homologiarelcel{k}{n}{\sigma} \cong \Lambda$ para $k=n$ e é trivial para $k\neq n$, então pela sequência anterior temos $\homologiarelskel{k}{n} \cong \somadir{\sigma}\homologiarelcel{k}{n}{\sigma} \cong \somadir{\sigma} \Lambda$ se $k=n$ e é trivial caso $k\neq n$, como desejávamos.}
