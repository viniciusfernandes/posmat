...
\begin{document}
	...
	Sejam $Sp(2n)^{*} = \{A\in Sp(2n): det(Id-A) \neq 0\}$ e $\mathcal{S} = \{\gamma:[0,1] \to Sp(2n): \gamma(0) = Id,\; \gamma(1)\in Sp(2n)^{*} \}$. Sejam $e(t)$ e $e'(t)$ duas bases ao longo de $x(t)$, então podemos afirmar que as matrizes $A(t), A'(t)$ associadas a essas bases, respectivamente, são homotópicas. Tome $u,v : D^{2} \subset \mathbb{C}^{n} \to M$ duas extensões de $x(t)$, isto é, duas aplicações suaves tal que $u|_{\partial D^{2}} = v|_{\partial D^{2}} = x$. \vermelho{Sabe-se que, dada uma aplicação contínua $f: D^{2} \to M$, o fibrado simpléctico $f^{*}TM$ pode ser trivializado e todas as suas trivializações são homotópicas. A equivalência homotópica entre as trivializações seguse do fato de que duas trivializações podem ser associadas a uma aplicação $f:D^{k} \to Sp(2n) $ e do fato que $Sp(2n)$ é conexo por caminhos.}
	
	Desse modo, também pode-se mostrar que 
	
	Desse modo podemos ver que, por construção, pode-se mostrar que existe uma aplicação contínua $h: D^{2} \times [0,1]\to M$ tal que $h(p, 0) = u(p)$, $h(p, 1) = v(p)$  e $h(\partial D^{2}, t) = x$, que é uma colagem uma colagem das extensões, isto é, uma homotopia entre $u$ e $v$. Conclusão, 
