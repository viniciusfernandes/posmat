...
\begin{document}
	...
	\begin{corolario}
		Seja $A \in \gruposimpletico{2n}$ uma matriz positiva-definida, então para cada $\alpha \in \real{}$ existe um único $S \in \gruposimpleticopositivo{2n}$ tal que $S=A^{\alpha}$, e em particular, $S^{1/2}\in \gruposimpletico{2n}$
	\end{corolario}
	\begin{prova}
		content...
	\end{prova}
	...
	\begin{observacao}\label{observacao_decomposicao_Sp2n}
		No teorema anterior exibimos um homeomorfismo $G:\gruposimpletico{2n} \to \gruposimpleticopositivo{2n} \times \mathcal{U}$, isto é, mostramos que se $A \in \gruposimpletico{2n}$, então ela pode ser decomposta univacamente como $A=PO$, onde $P\in \gruposimpleticopositivo{2n}$ e $O \in \mathcal{U}$. Usaremos essa afirmação na demonstração de alguns resultados adiante.
	\end{observacao}
	...
	\end{teorema}
	\begin{prova}
		Na demonstração do Teorema $\ref{teoerma_sp2n_conexo}$ (conexidade de $\gruposimpletico{2n}$) foi mostrado que se $A \in \gruposimpletico{2n}$ temos $A=PO$, onde $P \in \matrizsimetricapositiva{2n}$ e $O \in \mathcal{U}$. Com isso temos $AA^{t} = POO^{t}P^{t} = PP^{t}=P^{2}$. Como $A,A^{t} \in \gruposimpletico{2n}$, então $AA^{t} \in \gruposimpletico{2n}$ pois $\gruposimpletico{2n}$ é um grupo, logo $AA^{t} = P^{2} \in \gruposimpletico{2n}$. Pela Proposição $\ref{proposicao_potenciacao_grupo_simpletico}$ anterior podemos afirmar que $P^{\alpha} \in \gruposimpletico{2n}$ para todo $\alpha \in \real{}_{+}$. Definindo a aplicação $r:\gruposimpletico{2n}\times [0,1] \to \gruposimpletico{2n}$ tal que $r(A, \alpha) = (AA^{t})^{-\alpha/2}A$ é contínua pois é o produto de matrizes, que é uma operação contínua em $\gruposimpletico{2n}$. De fato $r$ é um retrato de deformação de $\gruposimpletico{2n}$ sobre $\mathcal{U}$ pois $r(A, 0) = A$, $r(A, 1) = (AA^{t})^{-1/2}A = P^{-1}A = O \in \mathcal{U}$ e, por fim, tomando $B \in \mathcal{U}$ temos $r(B, 1) = (BB^{t})^{-1/2}B = B$, pois $BB^{t} = Id$.
	...
	\begin{definicao}
		O índice de Maslov de uma curva é a aplicação $\mu:\curvasfechadascirculo{0}{1}\to \mathbb{Z}$ tal que $\mu(\gamma) = deg(\gamma)$, onde $deg(\gamma)$ é o grau de $\gamma$.
	\end{definicao}
		
	\begin{teorema}
		O índice de Maslov esta bem-definido, além disso, dadas curvas $\gamma, \beta \in \curvasfechadascirculo{0}{1}$, então satisfaz os seguintes axiomas:
		\begin{enumerate}
