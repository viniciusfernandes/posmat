...
\begin{document}
	...
	\begin{corolario}
		Sejam $(M, \omega)$ uma 2n-variedade simpléctica, $H : M \to \real{}$ uma função hamiltoniana autônoma e $x \in Crit(H)$. Assumindo que $||Hess_{x}(H)|| < 2\pi$, então o índice de Maslov $\mu(x)$ da solução periódica $x$ do sistema hamiltoniano $\dot{x}(t) = X_{H}(x(t), t)$, com $Ind(x)$ seu índice como ponto crítico da função $H$, é dado por:
		$$
		\mu(x) = Ind(x) - n.
		$$
	\end{corolario}
