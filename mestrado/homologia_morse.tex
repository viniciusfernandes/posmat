...
\begin{document}
	...
	\end{lema}
	$\square$
	Como $f, g$ são contínuas, então $g\circ f:A\to C$ é contínua, logo $(g\circ f)^{*}\eta = (E_{g\circ f}, A, p_{g\circ f})$ e $f^{*}(g^{*}\eta) = (E_{f}, A, p_{f})$ são n-fibrados vetoriais onde $E_{g\circ f} = \{(a, e) \in A\times E: p(e) = (g\circ f)(a)\}$ e $E_{f} = \{(a, e) \in A\times E_{g}: p_{f}(e) = f(a)=b\}$. Pela definição 
	\hfill$\blacksquare$
	...
	\end{corolario}
	$\square$
	Sejam $\eta = (E, B, p)$ e $\xi=(E', B', p')$ n-fibrados vetoriais com $B$ um compacto e $B' = \{*\}$ um espaço topológico com um único ponto de $B$. Por construção $\xi$ é um fibrado trivial. Definindo $f:B\to \{*\}$ e $g:\{*\}\to B$ teremos $f\circ g : \{*\} \to \{*\}$ e $g\circ f:B\to \{*\}$, logo $f\circ g = Id_{\{*\}}$ e pela hipótese devemos ter $g\circ f \simeq Id_{B}$. Pelo Teorema $\ref{pullback_isomorfismo}$ teremos $(g\circ f)^{*}\xi \cong Id_{B}^{*}\eta = \eta$, por outro lado, $g\circ f \simeq Id_{\{*\}}$ $(f \circ g)^{*}\xi \cong Id_{\{*\}}\xi = \xi$, o que implica que $\eta \cong \xi$. 
	
	
	\hfill$\blacksquare$
	\section{Índice de Maslov}
	\begin{definicao}
		(Fibrado simplético) Um n-fibrado simplético $(E,B, p)$ é um n-fibrado vetorial onde o grupo estrutural é reduzido so grupo de simpléctomorfismos $Symp(M,\omega)$ da n-variedade simplética $(M,\omega)$. Nesse caso, cada fibra $F_{p}$ é munida de uma 2-forma fechada e não-degenerada $\omega_{p}$.
	\end{definicao}
