...
\begin{document}
	...
	\begin{definicao}
		(O grau de uma curva fechada) Seja $\curvasfechadascirculo{0}{1}$ o conjunto de todas as curvas contínuas e fechadas em $S^{1}$, então a aplicação $deg:\curvasfechadascirculo{0}{1} \to \mathbb{Z}$ tal que $deg(\gamma) = (\alpha(1)-\alpha(0))/2\pi$, onde $\alpha:[0,1] \to \real{}$ é o levantamento de $\gamma$ da Proposição $\ref{proposicao_levantamento_curvas}$, esta bem-definida.
	\end{definicao}
	\begin{prova}
	...
	\begin{prova}
		...
		$$
		\vermelho{\mu(\gamma.\beta) = deg(\rho_{\gamma}.\rho_{\beta})} = deg(\rho_{\gamma})+deg(\rho_{\beta}) = \mu(\gamma)+\mu(\beta).
		$$	
