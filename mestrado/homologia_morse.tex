...
\begin{document}
	...
	\subsection{\gruposimpletico{2n} e sua topologia}
	...
	\subsection{O grupo simpletico Sp(2n)}
	...
	\begin{definicao}
		(Grupo simplético) Seja $(V,\omega)$ um espaço vetorial simplético 2n-dimensional, então o grupo simplético de $V$ é denotado por $Sp(2n) = \{g \in GL(2n; \real{}) : \omega(g(v), g(w)) = \omega(v, w), \; v,w \in V \}$. 
	\end{definicao}
	
	\begin{observacao}\label{obs_topologia_grupo_simpletico}
		O grupo $GL(2n, \real{})$ é um espaço topológico com a topologia gerada pela norma da convergência uniforma $||.|| : GL(2n, \real{}) \to \real{}$ tal que $||T||= max|T_{ij}| $, então $Sp(2n)$ como subconjunto de $GL(2n, \real{})$ será um espaço topológico com a topologia induzida.
	\end{observacao}
	
	Vejamos a seguinte caracterização do grupo simpletico e sua relação com a estrutura complexa:
	
	\begin{lema}\label{lema_caracterizacao_GL2n}
		(Caracterização de $Sp(2n)$) Sejam $(V, \omega)$ um 2n-espaço vetorial simpletico e $J_{0}$ uma estrutura complexo, então dada $A\in GL(2n,\real{})$ temos $AJ_{0}=J_{0}A$. Além disso, se $A\in Sp(2n)$ temos $A^{t}J_{0}A = J_{0}$.
	\end{lema}
	\begin{prova}
		Temos os produto interno $\innerprod{v}{w} = \omega(v,J_{0w})$. 
		Supondo $A \in GL(2n,\real{})$, então:
		$$
		xxx
		$$
		
		Seja $A \in Sp(2n)$, então:
		$$
			\omega(Av, J_{0}(w))=\innerprod{A(v)}{w} = \innerprod{v}{A^{t}(w)}=\omega(v, J_{0}A^{t}(w)) = \omega(Av, AJ_{0}A^{t}(w)),
		$$
		e como $v,w$ sã arbitrários, então $J_{0}=AJ_{0}A^{t}$.
	\end{prova}
	
	Sabemos que $\real{2n}$ e $\mathbb{C}^{n}$ são espaço vetoriais de dimensão $2n$, logo são isomorfos. Tal identificação pode ser realizada tomando $z=(x,y) \in \real{2n}$ tal que $(x,y) \mapsto x+iy \in \mathbb{C}^{n}$, onde $x,y \in \real{n}$. Note que $J_{0}(z) = (-y, x) \mapsto -y +ix = i(x+iy)$, isto é, podemos identificar a aplicação de $J_{0}$ pela multiplicação por $i$ nos complexos. Assim, podemos identificar $GL(n, \mathbb{C}) \mapsto GL(2n, \real{})$ e com um abuso de notação enunciaremos o lema abaixo:
