...
\newcommand{\normaLp}[1]{||#1||_{L_{p}}}
\newcommand{\normaLpdefinicao}[2]{ \Big(\int_{#2}#1^{p}dx\Big)^{1/p}}
...
\begin{document}
	...
	\begin{prova}
		...
		Se $A \in \mathcal{U}$, então $A^{t}=A^{-1}$ e $J_{0}=A^{t}J_{0}A=A^{-1}J_{0}A$, o que implica que $AJ_{0}=J_{0}A$, assim:
		$$
		\begin{aligned}
			AJ_{0}&=J_{0}A
			\\
	...
	\begin{observacao}\label{observacao_transporte_paralelo}
		(Transporte paralelo) 	Não podemos efetuar uma álgebra vetorial para campos vetoriais em pontos diferentes de uma $N$ n-variedade diferenciável, pois essa operação não esta bem-definida. Contudo, o transporte paralelo é um meio de efetuar uma comparação de como um campo vetorial muda quando avaliado em pontos diferentes da variedade $N$. Suponha que $\gamma:[0,1] \to N$ uma curva suave e $Y \in TN$. Então o transporte paralelo de $Y \in TN$ ao longo de $\gamma$ é dado por 
		$\nabla_{\frac{d\gamma}{dt}}Y$. O conceito de conexão afim generaliza o transporte paralelo. Mais detalhes sobre podem ser encontrados em $\cite{nakahara}$.
	\end{observacao}
	...
	\subsection{O operador de Floer $\operadorfloer$}
	...
	\\
	&\leq G \int_{\retacartesianocirculo}e^{-\delta|s|p}dx
	\\
	&< \infty,
	\end{aligned}	
	$$ 
	...
	\begin{aligned}
		...
		\\
		- & J(w+\xi)\bigparenteses{X_{H}(w+\xi) - X_{H}(\xi)} - J(w+\xi)X_{H}(w+\xi)
	\end{aligned}
	$$
	Como os termos $w, \xi, J, X_{H}$ são suaves, então o limite $\norma{\xi} \to 0$ existe e é expresso por:
	$$
	D\mapafloer_{w}(\xi) = \derivadaparcial{\xi}{s} + J(w)\derivadaparcial{\xi}{t}+ dJ_{w}(\xi)\bigparenteses{\derivadaparcial{w}{t} - X_{H}(w)} - J(w) (dX_{H})_{w}(\xi).
	$$
