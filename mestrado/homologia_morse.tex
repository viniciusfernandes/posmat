...
\begin{document}
	...
	\begin{definicao}
		(Estrutura complexa) Dizemos que $J: V \to V$, onde $J^{2} = -Id$, é uma estrutura complexa compatível com a forma simplética (ou $\omega$-compatível) se $g(u,v):=\omega(u, Jv)$ define um produto interno em $V$. Vamos fixar a notação $\estruturacomplexa$ como sendo:
		$$
		\estruturacomplexa=
		\left(
			\begin{array}{cc}
	...
	\begin{lema}\label{lema_caracterizacao_Sp2n}
		(Caracterização de $Sp(2n)$) Sejam $(V, \omega)$ um 2n-espaço vetorial simplético e $J \in \estruturascomplexaspadrao$ uma estrutura complexa, então $A\in Sp(2n)$ se, e somente se, $A^{t}JA = J$. Além disso, podemos escrever 
		$$
		A=
		\left(
		\begin{array}{cc}
	...
	\begin{observacao}\label{observacao_determinante_matriz_unitaria}
		Note que, dado $A \in \matrizunitaria{n}$, $1= det(AA^{*}) = det(A)det(A^{*}) = det(A)\overline{det(A)} = ||det(A)||^{2}$, portanto $det(\matrizunitaria{n}) \subseteq S^{1}$.
	\end{observacao}
	...
	A demonstração do lema abaixo pode ser encontrada em $\cite{audi_floer_homology}$
	
	\begin{lema}\label{lema_desigauldade_Lp}
		(Desigualdade $L^{p}$) Seja $\xi \in \espacosobolevretacirculo$. Existe uma constante $c>0$ tal que
		$$
	...
	\textbf{Etapa 1:} Vamos mostrar que o Lema $\ref{lema_diferencial_floer_bijecao}$ vale para $p=2$. Como $S$ não depende do parametro $s \in \real{}$, então podemos definir $A = \estruturacomplexa \derivadaparcialabrev{t} + S: \espacosobolevgeral{2}{\circulo;\real{2n}} \to \espacoLdois{\circulo;\real{2n}}$. \vermelho{O operador $A$ é auto-adjunto e ilimitado no espaço de Hilbert $H=\espacoLdois{\circulo;\real{2n}}$ (não sei demonstrar isso).} Por hipótese temos que $\dot{\Psi}(t)=\estruturacomplexa S(t)\Psi(t)$ o que implica em $(\derivadaparcialabrev{t} - \estruturacomplexa S)\Psi(t) = 0$. Aplicando $\estruturacomplexa$ nessa identidade e usando o fato de que $\estruturacomplexa^{2} = -Id$ teremos  $(\estruturacomplexa\derivadaparcialabrev{t} + S)\Psi(t) = A\Psi(t)= 0$. \vermelho{Portanto $A$ invertível (não sei demonstrar issso),} com isso não existe auto-valor nulo. Sejam $E^{+}, E^{-}$ os auto-espaços de $A$ cujos auto-valores são positivos e negativos, respectivamente. Então temos a decomposição $W = \espacosobolevgeral{2}{\circulo;\real{2n}} = E^{+}\oplus E^{-}$. Definindo $A^{\pm}=A |_{E^{\pm}}$ e $P^{\pm}: W \to E^{\pm}$ as projeções ortogonais, $\real{}_{+} \ni s \mapsto exp(\mp s A^{\pm}) \in \mathcal{L}(W)$ e $K: \real{}\to \mathcal{L}(W)$.
	$$
	...
	Esse operador esta bem-definido pois, dado $\eta(s) \in \espacoLdois{\retareal, H}$ temos
	$$
	\begin{aligned}
		...
		\\
		&\leq
		\int_{\retareal}e^{-\delta|s-\alpha|}  \underbrace{\norma{\eta(\alpha, t)}_{\espacoLdois{\retacartesianocirculo}}}_{N < \infty}d\alpha
		\\
		&\leq
		N\int_{\retareal}e^{-\delta|s-\alpha|} d\alpha
		\\
		&< \infty,	
	\end{aligned}
	$$
	...
	\begin{aligned}
		Q(\eta(s)) 
		&= \int_{\retareal}K(s-\alpha)\eta(\alpha)
		\\
	...
	$$
	Como $\eta(s) \in \espacoLdois{\retareal; H}$ é arbitrário, então mostramos que $\diferebcialmapafloerabrev \circ Q = Id$. Com isso, temos que a imagem de $Q$ é o domínio de $\diferebcialmapafloerabrev$, logo $Q(\eta(s)) \in \espacosobolevgeral{2}{\retacartesianocirculo;\real{2n}}$. Vamos mostrar que $Q\circ \diferebcialmapafloerabrev = Id$, sendo que para isso vamos calcular diretamente $Q(\diferebcialmapafloerabrev\eta)=Q((\derivadaparcialabrev{s}+A)\eta) = \eta$ para um dado $\eta \in \espacosobolevgeral{2}{\retacartesianocirculo;\real{2n}}$. Primeiramente, calculemos 
	$$
	\begin{aligned}
