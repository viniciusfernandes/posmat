...
\begin{document}
	...
	\tese{Teorema (CW-Homologia)}{Existe uma identificação natural entre a homologia do complexo $C_{*}(X,A; \Lambda)$ e a homologia singular $\homologiarel{*}{X}{A}$, isto é, $\homologia{k}{C_{*}(X,A; \Lambda)} \cong \homologiarel{k}{X}{A}\; \forall k \in \mathbb{Z}$}{Por definição de CW-complexo temos que $\skeleton{k} \subseteq \skeleton{k+1}$, logo podemos considerar a sequência exata loga da tripla $\skeleton{n-2} \subseteq \skeleton{n-1} \subseteq \skeleton{n}$ com as respectivas setas horizontais indicando as inclusões, e as verticais os isomorfismos garantidos pelos resultados anteriores:
	\[
	\xymatrix{
		\homologiarelskele{n}{n-1}{n-2} \ar[r] \ar[d]^{\cong} & 
		\homologiarelskele{n}{n}{n-2} \ar[r]^{i_{*}} \ar[d]^{h} & 
		\homologiarelskele{n}{n}{n-1} \ar[r]^{\delta_{*}} \ar[d]^{\Psi_{n}} & 
		\homologiarelskele{n-1}{n-1}{n-2}\ar[d]^{\Psi_{n-1}} 
		\\
		0 \ar[r] & 
		\homologiarel{n}{\skeleton{n}}{A} \ar[r]^{I} & 
		C_{n}(X,A) \ar[r]^{\partial_{n}} & 
		C_{n-1}(X,A)
	}
	\]	
	onde temos as setas verticais dadas pelo teorema do isomorfismo
	dos grupos de homologia ou pelo lema anterior. Temos a aplicação $\Psi\circ i_{*}:\homologiarelskele{n}{n}{n-2} \to C_{n}(X,A)$, e como $h$ é também um isomorfismo, podemos definir $I$ como sendo $I\circ h:\homologiarelskele{n}{n}{n-2} \to C_{n}(X,A)$ tal que $\Psi\circ i_{*}=I\circ h$. Como temos uma sequência exata, temos $Im(i_{*}) = Ker(\delta_{*})$, e como $\Psi_{n}$ é um isomorfismo, então 
	}
	\section{Exemplos}
