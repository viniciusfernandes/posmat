...
\begin{document}
	...
	\begin{lema}
		A restrição $F|_{\matrizunitaria{n}}: \matrizunitaria{n} \to \mathcal{U}$, onde $\mathcal{U} = \gruposimpletico{2n}\cap \matrizortogonal{2n}$, é um isomorfismo.
	\end{lema}
	\begin{prova}
		...
		Seja $A \in \mathcal{U}$, então vimos que
	\end{prova}
	
	e $\matrizunitaria{n}$ como subgrupo de $\matrizortogonal{2n}$. Por abuso de notação denotaremos a continência dessas identificações entre os grupos $\generalgroupcomplexo{n} \subset \generalgroupreal{2n}$ e $\matrizunitaria{n} \subset \matrizortogonal{2n}$. Podemos agora enunciar o lema:
	
	\begin{lema}
		$Sp(2n) \cap O(2n) = Sp(2n) \cap GL(n, \mathbb{C}) = O(2n) \cap GL(n, \mathbb{C}) = U(n)$.
	\end{lema}
	\begin{prova}
		Primeiramente, todas as intersecções da hipótese são não-vazias pois a identidade pertence a todas elas. Sejam:
		\begin{enumerate}
			\item Por definição do conjunto ortogonal temos $Sp(2n) \cap O(2n) \subseteq Sp(2n) \cap GL(n, \mathbb{C}) $. Por outro lado, seja $A \in Sp(2n) \cap GL(n, \mathbb{C})$, então $J_{0} = AJ_{0}A^{t}$ e $J_{0} = AJ_{0}A^{-1}$ o que implica em $A^{t}=A^{-1}$, então $A \in Sp(2n) \cap O(2n)$, portanto $Sp(2n) \cap O(2n)=Sp(2n) \cap GL(n, \mathbb{C})$.
		\end{enumerate}
	\end{prova}
	
	\begin{teorema}\label{teorema_decomposicao_polar}
		(Decomposição polar) Sejam $(V, \innerprod{}{})$ um n-espaço vetorial real com um produto interno positivo definido e $A \in \generalgroupreal{n}$, então podemos escrever $A=PO$ onde $P,O\in  \generalgroupreal{n}$ tal que $P = P^{t}$ (auto-adjunta) e $OO^{t} = Id$ (ortogonal).
	\end{teorema}
	\begin{prova}
	...
	Os próximos passos serão dados em direção a demonstração de que o grupo simpletico $\gruposimpletico{2n}$ é um espaço topológico contrátil.
