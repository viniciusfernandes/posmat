...
\begin{document}
	...
	\begin{definition}
		(Sequência da triplas) Sejam $X, Y, Z$ espaços topológicos tais que $Z \subseteq Y \subseteq X$. Então a sequência da tripla $(X,Y,Z)$ é dada por:
			...
			\]
			onde $i_{*},\;j_{*}$ são inclusões e $\delta_{*}$ o homomorfismo conectante.
	\end{definition}
	
	\section{CW-Homologia}
	\begin{lemma}
		(isomorfismo dos grupos de homologia) Sejam $\Lambda$ um anel comutativo com unidade e $X$ um CW-complexo,  então $\homologiarelskelesimpl{k}{n} $ é um $\Lambda-$módulo livre e finitamente gerado pelas $n-$cpelulas de $X$ tal que
		$$
