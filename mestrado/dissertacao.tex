...
\begin{document}
	...
	\begin{prova}
		...
		\begin{enumerate}
			\item \textbf{\textit{Naturalidade:}} Sejam $T : \complexo{2n} \to \complexo{2n}$ um isomorfismo simplético, sito é, $T\in \gruposimpletico{2n}$. Então $T$ preserva a $\Omega$-ortogonalidade de $A$. De fato, sejam $E_{\lambda}$ e $E_{\lambda'}$ subespaços $\Omega$-ortogonais de $A$, então $(T^{*}\Omega)(E_{\lambda}, E_{\lambda'}) = \Omega(T(
			E_{\lambda}), T(
			E_{\lambda'})) = \Omega(E_{\lambda}, E_{\lambda'}) = 0$. Seja $S^{-1}DS$ a representação diagonal de $A$. Então $B = T^{-1}AT = (ST)^{-1}DST$, logo $B$ é similar a matriz diagonal $D$ e possui os mesmo auto-valores de $A$. Além disso, temos que $T^{-1}AT\in \gruposimpletico{2n}$ pois $\gruposimpletico{2n}$ é um grupo. Portanto $\rho(T^{-1}AT) = \rho(B) = \rho(A)$. 
			...
			\item \textbf{\textit{Deteminante:}} XXXXXXXXXXXXXXXXXXXXXXx
