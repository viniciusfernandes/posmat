...
\begin{document}
	...
	\begin{prova} A forma quadrática $Q$ é não-degenerada pois $B$ é não-degenerada.
		...
		Portanto $\complexificado{V}^{+} $ é um subespaço vetorial de $\complexificado{V}$.  Além disso, $dim(\complexificado{V}^{+}) = n$. Com isso, $Q(v) = 2\parteImaginaria{\produtointerno{\overline{v}_{(1)}}{v_{(1)}}} = 2\norma{v_{(1)}}^{2} \geq 0$, para todo $v \in \complexificado{V}^{+}$. Analogamente, definindo $\complexificado{V}^{-} = \{v \in \complexificado{V}: v_{(1)} =- v_{(2)}\}$, tem-se que $Q(v) = 2\parteImaginaria{\produtointerno{\overline{v}_{(1)}}{-v_{(1)}}} = -2\norma{v_{(1)}}^{2} \leq 0$, para todo $v \in \complexificado{V}^{-}$. Como $\complexificado{V}^{+}\cap \complexificado{V}^{-} = \{0\}$ e $dim(\complexificado{V}^{+}) = dim(\complexificado{V}^{-}) = n$, então $\complexificado{V}=\complexificado{V}^{+}\oplus\complexificado{V}^{-}$ e ambos os subespaços são máximais com respeito a $Q$ ser positiva ou negativa definida. Logo, $m(Q) = m_{+}(Q) -m_{-}(Q) = n-n = 0$.
	\end{prova}
	
	Sejam $A \in \gruposimpletico{2n}$ e $\lambda\in \sigma(A)$ tal que $\lambda \in \complementar{\circulo}{\reta}$ \vermelho{(não entendi essa condição...pode ser que envolva a $\omega$-ortogonalidade. Esta no começo das propriedades de $\rho$ do Michelle. No Salamon $\complementar{\circulo}{\{\pm 1\}}$ e logo em seguida um resultado da ortoganalidade)}. Realizando a decomposição $E_{\lambda} = E_{\lambda}^{+} \oplus E_{\lambda}^{-}$, onde $E_{\lambda}^{\pm} $ são subespaços vetoriais onde $Q$ é positivo-definida, ou negativo-definida, respectivamente, tem-se que a multiplicidade de $\lambda$ é dada por $m(\lambda) = m_{+}(\lambda)+m_{-}(\lambda)$, onde  $m_{\pm}(\lambda) = dim_{\complexo{}}(E_{\lambda}^{\pm} )$ e as dimensões de $E_{\lambda}^{\pm}$ são iguais, pela maximalidade. Com isso, $m(\lambda) = 2dim(E_{\lambda}^{+}) \in 2\mathbb{N}$.
	
	\begin{lema}\label{lema_identidade_aplicacao_rho}
		Sejam $A \in \gruposimpleticoespecial{2n}$, $m_{0} = \sum_{\lambda \in \sigma(A)} m_{-}(\lambda)$  e $\sigma'(A)=\sigma(A)\cap \complementar{\circulo}{\real{-}}$ \vermelho{(acredito que aqui tiramos os auto valores negativos para seguir com a ordem $\leq_{o}$)}. Então
		$$
	...
	\begin{prova}
		...
		Escrevendo a aplicação $\rho$ como
		$$
		\rho(A) =\prod_{j=1}^{n}\frac{\lambda_{j}}{|\lambda_{j}|} = 
		$$
		$$
