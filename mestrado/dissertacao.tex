...
\begin{document}
	...
	\end{lema}
	\begin{prova}
		Sejam $Id \in \matrizunitaria{n}$ e $1 \in \circulo$ pontos base. Defina $A(t) = diag\{e^{i\alpha(t)}, 1, \dots, 1\}$, onde $\alpha:\intervalo\to \reta$ é uma aplicação contínua tal que $\alpha(t+1) = \alpha(1)$ para todo $t \in \intervalo$. Temos que $A(t)A(t)^{t} = Id$, logo $A(t) \in \matrizunitaria{n}$. Além disso, $A(t+1) = A(t)$, portanto $A = \caminhospontobasegeral{Id}{\matrizunitaria{n}}$.
		...
		\end{aligned}
		$$
		logo é um homomorfismo. Como $det$ é sobrejetora, então $det_{*}$ é sobrejetora. Supondo que $Id = det_{*}\classe{A} = \classe{det(A(t))}$, então $det(A) \sim c$, onde $c$ é uma curva constante em $\circulo$. Logo $A \sim Id $, onde $C$ é a curva constate em $\matrizunitaria{n}$. Com isso, $\classe{A} = Id$ e $det_{*}$ é injetora, portanto é um isomorfismo.
		Como $\matrizunitaria{n}$ é conexo por caminhos, então 
		$$
		\grupofundamental{\matrizunitaria{n}} = \grupofundamentalpontobase{\matrizunitaria{n}}{Id}.
		$$
	\end{prova}
	...
	\end{teorema}
	\begin{prova}
		Como $\matrizSimpleticaOrtogonal$ é um retrato de deformação de $\gruposimpletico{2n}$, então $\grupofundamental{\gruposimpletico{2n}}$ e $\grupofundamental{\matrizSimpleticaOrtogonal}$ são isomorfos. 
