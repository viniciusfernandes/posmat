...
\begin{document}
	...
	\begin{corolario}
		...
		$$
		Além disso, $dim(V_{j}) = n_{j}$,  o polinômio de $T|_{V_{j}}$ é $m_{j}(\lambda) = (\lambda - \lambda_{j})^{m_{j}}$ e $V_{j} = Ker(T-\lambda_{j})^{m_{j}}$, em que $1\leq m_{j}\leq n_{j}$ é o comprimento do maior bloco de Jordan associado ao auto-valor $\lambda_{j}$.
	\end{corolario}
	
	\begin{observacao}\label{observacao_autoespaco_generalizado}
		Os subespaços vetoriais $V_{j} \subset V$ do teorema anterior também são chamados de auto-espaços generalizados.
	\end{observacao}
