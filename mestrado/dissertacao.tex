...
\begin{document}
	...
	\begin{teorema}\label{teorema_aplicacao_rho}
		...
		\begin{enumerate}
			\item \label{item_naturalidade_rho} \textbf{Naturalidade:}  Se $T:V_{1} \to V_{2}$ é um isomorfismo simplético, isto é, $T^{*}\omega_{2} = \omega_{1}, $então 
			$$
			\rho(TAT^{-1}) = \rho(A)
			$$
	...
	\begin{prova}
		...
		\begin{enumerate}
			\item \label{item_naturalidade_rho} \textbf{\textit{Naturalidade:}} Sejam $T : \complexificado{V}_{1} \to \complexificado{V}_{2}$ um isomorfismo simplético e $A \in \gruposimpletico{\complexificado{V}}$. Pelo Lema \ref{lema_caracterizacao_espectro_semelhante} tem-se que $\espectrooperador{T} = \espectrooperador{T^{-1}}$. Definindo $\sigma'(A) = \lambda\in \espectrooperador{A}\cap\complementar{\circulo}{\reta}$ e analogamente $\sigma'(T)$, tem-se
			$$
				\begin{aligned}
						\rho(TAT^{-1}) 
	...
	\end{lema}
	\begin{prova}
		Tem-se que $(\psi^{-1}\psi)(t)=Id$, o que implica em $\psi^{-1}(t)=(\psi(t))^{-1}$ para todo $t\in \intervalo$. Como a inversa de toda transformação simplética é uma transformação simplética, então $\psi^{-1}(\intervalo) \subset \gruposimpletico{2n}$. Pelo Lema \ref{lema_caracterizacao_espectro_semelhante}, tem-se que $\espectrooperador{\psi^{-1}(1)} = \espectrooperador{\psi(1)}$. Como $1\notin \espectrooperador{\psi(1)}$, então $1\notin \espectrooperador{\psi^{-1}(1)}$. Portanto $\psi^{-1}(1)\in \gruposimpleticonaodegenerado{*}$ e $\psi^{-1}\in \caminhosespeciais{\gruposimpletico{2n}}$.
		
		Denote os auto-espaços de $\psi(t)$ e $\psi^{-1}(t)$ associados aos seus auto-valores $\lambda$ e $\lambda^{-1}$ por $E_{\lambda}$ e $F_{\lambda^{-1}}$, respectivamente. Dado $v \in E_{\lambda}^{+}$ tem-se que $v\in F_{\lambda^{-1}}^{+}$ pois $\psi^{-1}(t)v=\lambda^{-1}v$ e $Q(v)>0$, portanto $E_{\lambda}^{+}\subseteq F_{\lambda^{-1}}^{+}$. A inclusão $E_{\lambda}^{+}\supseteq F_{\lambda^{-1}}^{+}$ segue de um argumento análogo. Logo $E_{\lambda}^{+} = F_{\lambda^{-1}}^{+}$ e $m_{+}^{\psi}(\lambda) = m_{+}^{\psi^{-1}}(\lambda^{-1})$. Note que, o número de pares de auto-valores negativos $m_{0}$ é igual para $\lambda$ e $\lambda^{-1}$. Então
		$$
		\begin{aligned}
			(\rho(\psi^{-1}(t))) 
			&= (-1)^{m_{0}}\prod (1/\lambda)^{m_{+}^{\psi^{-1}}(1/\lambda)}
			\\
			&=(-1)^{-m_{0}}\prod \lambda^{-m_{+}^{\psi}(\lambda)}
			\\
			&=\bigparenteses{(-1)^{m_{0}}\prod \lambda^{m_{+}^{\psi}(\lambda)}}^{-1}
			\\
			&	(\rho(\psi(t))) ^{-1},
		\end{aligned}
		$$
		para todo $t \in \intervalo$.
		\end{prova}
	...
	\begin{prova}
		\begin{enumerate}
			...
			\\
			&= \frac{1}{2}\big(\alpha_{\psi_{p}}(1)-\alpha_{\psi_{p}}(1) -\alpha_{\psi_{q}}(0)+\alpha_{\psi_{q}}(0) \big) 
			\\
			...
			\begin{enumerate}
				...
				\item \textbf{Caso $a_{(1)}.a_{(2)}>0$ e $a_{(1)} = a_{(2)} = (-\epsilon,\dots,-\epsilon)$:} esse caso é análogo ao anterior, contudo, tem-se que  $R(t\epsilon)$,  $r(t\epsilon)$ é a matriz $2\times 2$ de rotação por um ângulo de $t\epsilon>0$ no plano $\real{2}$ e $det_{\complexo{}}(R(t\epsilon)) = \exp(int\epsilon)$. Isso implica em $\mu(\Psi_{s}) = 2deg(\exp(t\estruturacomplexa S)) = n$. Como $Ind(S) = 2n$, então $ Ind(S)-n = 2n-n = n = \mu(\Psi_{s})$, como desejávamos.
				
				\item \textbf{Caso $a_{(1)}.a_{(2)}<0$ e $a_{(1)}=(\epsilon,\dots,\epsilon), \; a_{(2)} = -a_{(1)}$:} Tem-se
				$$
				\exp(t\estruturacomplexa S ) = 
				\left(
				\begin{array}{cc}
				...
				Além disso, o sistema $\exp(t\estruturacomplexa S ) v = \lambda(t\epsilon) v$ tem como solução os auto-valores $\lambda \in \{\cosh(t\epsilon)\pm i\sinh(t\epsilon)\}$. Portanto $\exists t\in \intervalo$ tal que $\lambda(t\epsilon) \notin \circulo$ e $\espectrooperador{\Psi_{s}}\cap\circulo = \emptyset$. Como a aplicação $\rho$ depende apenas dos auto-valores em $\espectrooperador{\Psi_{s}}\cap\circulo = \emptyset$, então $\mu(\Psi_{s}) = 0$. Como não há auto-valores negativos, então $Ind(S)=0$ e $Ind(S)=\mu(\Psi_{s})$.
			\end{enumerate}
	
			\item \textbf{(Inverso)} O Lema \ref{lema_inversa_caminho_especial} mostra que $\psi^{-1} \in \caminhosespeciais{\gruposimpletico{2n}}$. Como $\rho_{\psi}(t) = \exp(i\alpha_{\psi}(t))$, então $\rho_{\psi^{-1}}(t) = (\rho_{\psi}(t))^{-1} = \exp(-i\alpha_{\psi}(t))$. Logo 
			$$
