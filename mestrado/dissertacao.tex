...
\begin{document}
	...
	Suponha que $\Psi^{\pm}$ sejam soluções do sistema $\dot{\Psi}(t) = \estruturacomplexa S^{\pm}(t)\Psi(t)$ tais que $\Psi(0)=Id$ e $\det(Id-\Psi^{\pm}(1))\neq 0$. Aplicando o teorema da regularidade elíptica acima, pode-se mostrar que $\diferencialfloerponto{u}$ é um operador de Fredholm para $p>1$. Além disso, valem as Proposições \ref{proposicao_gamma_sobrejetora} e \ref{proposicao_orbitasConectantesZ}.
	...
	\begin{proposicao}\label{proposicao_orbitasConectantesZ}
		O conjunto $\orbitasConectantesZ$ é uma variedade de Banach.
	\end{proposicao}
	...
	\begin{teorema}\label{teorema_orbitas_conectantes_finitas}
		Sejam $x^{-}, x^{+} \in \pontoscriticos{\funcionalH}$. Então, $\energiafinitaMconectante$ é uma variedade de dimensão $\mu(x^{-})-\mu(x^{+})$ e possui um número finito de elementos.
	\end{teorema}
