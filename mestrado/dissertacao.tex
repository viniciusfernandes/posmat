...
\newcommand{\espacosobolevgeneralizado}[2]{W^{k,p}(#1; #2)}
...
\begin{document}
	...
	Foi demonstrado no Exemplo \ref{exemplo_fibrado_circulo} que todo fibrado vetorial cujo espaço base é $\circulo$ é um fibrado  trivial. Portanto, tomando $x\in\lacocontrateis$ e usando a Observação \ref{observacao_pullback_fibrado_tangente}, tem-se que $\pullbackfibradotangenteM{x} \cong \circulo\times \real{2n}$. As seções do fibrado $\pullbackfibradotangenteM{x}$ são as seções de $TM$ restritas a $x(\reta )\subset M$ (veja a Observação \ref{observacao_pullback_fibrado_tangente}). Com isso, tem-se o espaço de Sobolev 
	$$
	\espacosobolev{\circulo;\pullbackfibradotangenteM{x}}=
	\{X\in \campossuaves{x(\reta)}: \exists\; Y\in \campossuaves{M}, \; X=Y|_{x}\;\text{e}\;  X_{i}\in \espacosobolev{\circulo} \},
	$$
	onde $X_{i} \in \funcoessuaves{\circulo}$ são os componentes do campo vetorial $X$. Por simplicidade esse espaço de Sobolev será denotado por $\espacosobolev{\pullbackfibradotangenteM{x}}$. Em \cite{audi_floer_homology} pode ser encontrado que esse definição permite associar $\espacosobolev{\pullbackfibradotangenteM{x}}$ com o espaço de Sobolev $\espacosobolevcontradominio{\circulo}{\real{2n}}$.
	
	Com argumentos análogos aos anteriores, para um dado $u \in \energiafinitaM$, o espaço de Sobolev $\espacosobolevcontradominio{\retacartesianocirculo}{\pullbackfibradotangenteM{u}}$ é denotado por $\espacosobolev{\pullbackfibradotangenteM{u}}$ e é identificado com $\espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}}$.
	
	Seja $D \subset TM$ uma vizinhança da seção nula de $TM$. Então a aplicação exponencial $\exp_{x}: \espacosobolev{x^{*}D} \to C^{0}(\circulo;M)$ é dada por $\exp_{x}(Y)(t) = \exp_{x(t)}(Y(t))$.
...
\alerta{	\begin{observacao}
	...
	\begin{definicao}
		Um n-fibrado vetorial diferenciável $\eta$ sobre um espaço topológico $B$ é denotado por $\eta = (E, B, p, G)$ e consiste das seguinte propriedades
		...
		\begin{enumerate}
			...
			\item Uma aplicação sobrejetora $\pi: E\to M$ chamada projeção. A pré-imagem $\pi^{-1}(b) = F_{b} \cong F$ chamada fibra em $b\in B$. 
			
			\item Um grupo de Lie $G$ chamado grupo de estrutura, o qual age a esquerda em $F$.
			
			\item Uma conjunto de abertos $\{U_{i}\}_{i\in I}$ que cobrem $B$ com um difeomorfismo $\phi_{i}: U_{i}\times F\to \pi^{-1}(U_{i})$ tal que $(\pi\circ \phi_{i})(b,f) = p$. A aplicação $\phi_{i}$ é chamada trivialização local se $\phi^{-1}: \pi^{-1}(U_{i}) \to U_{i}\times F$ é sobrejetora.
			
			\item A aplicação $\phi_{i,b}: F\to F_{b}$ definida por $\phi_{i,b}(f) = \phi_{i}(b,f)$ é um difeomorfismo. Na intersecção $U_{i}\cap U_{j}\neq \emptyset$ e exigido que $t_{ij}(b)=\phi_{i,b}^{-1}\circ \phi_{i,b}:F\to F$ é um elemento de $G$. Então $\phi_{i}$ e $\phi_{j}$ estão relacionados pela aplicação suave $t_{ij}: U_{i}\cap U_{j}\to G$ por $\phi_{j}(b, f) = \phi_{i}(b, t_{ij}(b)f)$. As aplicações $t_{ij}$ são chamadas funções de transição.
		\end{enumerate}
		
	\end{definicao}
	
	Um n-fibrado vetorial diferenciável $\eta$ é um fibrado trivial quando todas as funções de transição se reduzem a aplicação identidade. Com isso, o fibrado pode ser escrito como o produto $B\times F$, isto é, o fibrado $\eta$ é homeomorfo a $B\times F$.
	
	\begin{exemplo}\label{exemplo_fibrado_circulo}
		Seja $\eta=(E, \circulo,\pi, G)$ um 1-fibrado vetorial com as fibras $F=\reta$ e $G$ é o grupos das transformações lineares em $\reta$ que preservam sua orientação. Sejam $U_{1} = (0,2\pi)$ e $U_{1} = (-\pi,\pi)$ uma cobertura de $\circulo$. Então $U_{1}\cap U_{2} =(0,\pi)\cup(\pi,2\pi)$. As trivializações locais são 
		$$
	...
	\begin{definicao}
		...
		$$
		e tal que $F$ é um isomorfismo entre $\pi^{-1}(b)$ e $\pi'^{-1}(f(b))$. A aplicação $f:B\to B'$ é chamada mapa de espaço base.
	\end{definicao}
	...
	\begin{exemplo}
		(1-Fibrado vetorial) Sejam $E=S^{1} \times \real{}$, $B=S^{1}$, $F_{b} = \real{}$ para qualquer $b \in S^{1}\subset \real{2}$ e $\pi:S^{1} \times \real{}\to S^{1}$ tal que $p(b, e)=p \in S^{1}$. Então $\eta=(S^{1} \times \real{}, S^{1}, \pi)$ é um fibrado vetorial pois $\pi$ é uma aplicação contínua, $E, B$ são espaços topológicos e $F_{b} = \pi^{-1}(b) = \real{}$ é um 1-espaço vetorial. Além disso, $\eta$ é um fibrado vetorial trivial pois, tomando os abertos $S^{+} = S^{1} \backslash \{(0,1)\}$ e $S^{-} = S^{1} \backslash \{(0,-1)\}$, tem-se as projeções $\pi^{-1}(S^{\pm}) = S^{\pm} \times \real{} \subset E$, e com isso, $(S^{+}\times\real{}) \cup ( S^{-}\times\real{}) = (S^{+}\cup S^{-})\times\real{} = S^{1} \times\real{} =E$, logo $\eta$ é trivial. 
	\end{exemplo}
	
	\begin{proposicao}
		(Fibrado tangente) Sejam $M$ uma variedade suave n-dimensional. Definindo $B=M$, dado $q\in M$ temos as fibras $F_{q} = T_{q}M$ e o espaço total é $TM=\{(q, v): q\in M, v\in T_{q}M \}$. A tripla $\eta = (TM, M, \pi)$, denominado fibrado tangente de $M$, é localmente trivial, portanto é um fibrado vetorial.
	\end{proposicao}
	\begin{prova}
		Note que a projeção $\pi(q,v_{q}) = q$ é uma aplicação contínua, $\pi^{-1}(\{q\}) = T_{q}M$ é um n-espaço vetorial em $q\in M$ e  o espaço base $M$ é um espaço topológico pois é uma variedade. Seja $U$ uma vizinhança de $q$. Como $T_{q}M$ é homeomorfo a $\real{n}$, então $\pi^{-1}(U) = \bigcup_{q\in M}(\{q\}\times T_{q}M)$ é homeomorfo a $U\times \real{n}$ e $\eta$ é localmente trivial. Portanto $\eta$ é um n-fibrado vetorial.
	\end{prova}
	
	\begin{observacao}
		Os fibrados tangentes $\eta$ serão denotados por $TM$, o que é convencionado em grande parte da literatura.
	\end{observacao}
	...
	\begin{observacao}\label{observacao_identificacao_secao_campo_vetorial}
		Os campos vetoriais definidos em $M$ podem ser identificados com as seções do fibrado tangente $TM$ do seguinte modo: seja $X\in \campossuaves{M}$ e definida $s_{X}$ a seção do fibrado tangente por $s_{X}(q) = (q, X(q))$. tem-se que, $(\pi\circ s_{X})(q) = q$, logo $\pi\circ s_{X}=Id$. Com isso, segue-se a identificação $s_{X} \mapsto X$.
	\end{observacao}
	
	\begin{definicao}
		(Pullback de um fibrado) Sejam $B, B'$ dois espaços topológicos, $\eta=(E', B', p')$ um n-fibrado vetorial e $f:B\to B'$ uma aplicação contínua. Defina $E \subseteq B\times E'$ como sendo o conjunto $\{(b, e): b \in B,\; e \in \pi'^{-1}(f(b)) \}$. Além disso, defina o mapa de projeção $\pi:E\to B$ tal que $\pi(b,e) = b$. O pullback de $\eta$ por $f$ é a tripla $f^{*}\eta = (E,B, p)$.
	\end{definicao}
	
	\begin{observacao}\label{observacao_pullback_fibrado_tangente}
		Seja $\gamma:\reta \to M$ um caminho contínuo na variedade $M$. Então o pullback $\pullbackfibradotangenteM{\gamma}$ é o conjunto $\{(\gamma(t), X(\gamma(t)): X\in \campossuaves{M} \; \text{e}\; t \in \reta \}$. Note que, $\pullbackfibradotangenteM{\gamma}$ é o $2n$-fibrado vetorial cujas fibras são $\espacotangenteponto{\gamma(t)}{M}$ para cada $t\in \reta$ e o espaço base é $\gamma(\reta)\subset M$. De fato, se $\pi: \pullbackfibradotangenteM{\gamma} \to B$ e o mapa de projeção desse fibrado, então $\pi(\gamma(t), Y(\gamma(t))) = \gamma(t)$ e $\pi^{-1}(\gamma(t)) = \espacotangenteponto{\gamma(t)}{M}$ para todo $t\in \reta$. Com isso, toda seção $X: \gamma(\reta)\to \pullbackfibradotangenteM{\gamma}$ dada por $X(\gamma(t)) = (\gamma(t), X(\gamma(t)))$, pode ser vista como a restricao de alguma seção $X': M\to TM$ restrita ao subconjunto $\gamma(\reta) \subset M$, isto é, $X= X'|_{\gamma}$.
	\end{observacao}
	...
	\end{lema}
	\begin{prova}
		Por definição tem-se que $f^{*}\eta = (E_{f}, A, \pi_{f})$, onde $E_{f}=\{(a, e): a\in A,\; e \in \pi^{-1}(f(a)) \}$ e $E_{f} \subseteq A\times E$. Como $\eta$ é trivial, então existe um homeomorfismo $h: E \to B\times \real{n}$. Com isso, tem-se que $E_{f}\subseteq A\times f(A)\times \real{n} \subseteq A\times B\times \real{n}$. Afirmo que existe um homeomorfismo $E_{f}\mapsto A\times f(A)\times \real{n}$. De fato, defina $H:A\times E \to A\times B\times \real{n}$ por $H(a,e) = (a, h(e))$. É imediato que $H$ é um homeomorfismo. Note que, para um dado $(a, f(a), F_{f(a)}) \in A\times f(A)\times \real{n}$ existe $(a, e)\in E_{f}$ tal que $H(a,e)=(a, h(e))=(a,f(a), F_{f(a)})$. Logo a restrição de $H$ ao conjunto $E_{f}$ é sobrejetora em $A\times f(A)\times \real{n}$ e $H|_{E_{f}}:E_{f}\to A\times f(A)\times \real{n}$ é um homoeomorfismo. Portanto, $f^{*}\eta$ é trivial.
	\end{prova}
	
	Os seguintes resultados nos dão informação sobre a homotopia dos fibrados vetoriais e podemo ser encontrados em \cite{nakahara} e \cite{steenrod_fibre_bundles}.
	
	\begin{teorema}\label{pullback_isomorfismo}
		Sejam $A, B$ dois espaços topológicos, $\eta=(E, B, \pi)$ um n-fibrado vetorial e $f,g: A\to B$ duas apliações homotópicas, então $f^{*}\eta \cong g^{*}\eta$.
	\end{teorema}
	...
	\end{corolario}
	\begin{prova}
		Como $D^{n}$ é contrátil, então a aplicação identidade $i:D^{n}\to D^{n}$ é homotópica a aplicação constante $c:D^{n}\to \{p\}$, para um dado $p\in D^{n}$. Pelo Teorema \ref{pullback_isomorfismo} tem-se que $i^{*}\eta  \cong c^{*}\eta$. Como $c^{*}\eta =(E ,\{p\}, F) \cong \{p\}\times F$, então $\eta =i^{*}\eta \cong \{p\}\times F$. 
	\end{prova}
	...
	\begin{definicao}\label{definicao_espalo_sobolev_generalizado}
		(Espaço de Sobolev Generalizado) Seja $\Omega$ é um aberto de $\real{n}$. O espaço de Sobolev $\espacosobolevgeneralizado{\Omega}{\real{m}}$ é o conjunto das aplicações  
		$$
		\begin{aligned}
		f:\Omega \subset\real{n} &\to \real{m}
		\\
		...
		\end{aligned}
		$$ 
		tais que $f_{j} \in \espacosobolevgeneralizado{\Omega}{\reta}$ para $1\leq j \leq k$.
	\end{definicao}
	
	A importante proposição a seguir esta demonstrada em \cite{breazis_sobolev_spaces}.
	
	\begin{proposicao}
		Os espaços $\espacoLpcontradominio{\Omega}{\real{m}}$ e $\espacosobolevgeneralizado{\Omega}{\real{m}}$, munidos das normas 
		$$
	...
	\begin{observacao}
		Da proposição anterior é imediato que $\normaLp{f} \leq \normaWpgeneralizado{f}$. Logo, $\espacosobolevgeneralizado{\Omega}{\real{m}} \subset\espacoLpcontradominio{\Omega}{\real{m}}$.
	\end{observacao}
	...
	\alerta{	
		\begin{teorema}\label{teorema_rellich}
			(Rellich) Sejam $\Omega \subseteq \real{n}$ um subconjunto aberto limitado. Se $p>n$, então $\espacosobolevcontradominio{\Omega}{\real{m}} \subset C^{0}(\Omega;\real{m})$ e tal inclusão é um operador compacto. (Acho que posso remover esse resultado.)
		\end{teorema}
	}
