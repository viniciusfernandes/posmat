...
\begin{document}
	...
	\section{A propriedade de Fredholm de $D\operadorFloer$ e seu operador dual.}
...
\aviso{Brezis pag 43}
	...
	\begin{teorema}\label{teorema_representacao_reiz}
		...
		$$
		para todo $X \in \espacoLpretacirculo$, onde $\produtointerno{.}{.}$ é o produto interno canônico de $\real{2n}$.
	\end{teorema}
	
	\begin{proposicao}\label{proposicao_diferencial_floer_adjunto}
		Em termos de distribuição, o operador $\diferencialfloerabrev^{*} :\espacoLpcontradominio{\retacartesianocirculo}{\real{2n}} \to \espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}}$ é dado por
		$$
		\diferencialfloerabrev^{*}=-\derivadaparcial{}{s}+\estruturacomplexa \derivadaparcial{}{t}+S^{t}.
		$$
	\end{proposicao}
	\begin{prova} Tome $f \in \espacoLpdual$ e $Y \in \espacoLpGeral{q}{\retacartesianocirculo;\real{2n}}$ tal que 
		$$
		f(.) = \int_{\retacartesianocirculo}\produtointerno{Y}{.},
		$$
		de acordo com o Teorema \ref{teorema_representacao_reiz}. Seja  $\funcoessupcompactcontradom{\retacartesianocirculo}{\real{2n}}$ o conjunto das aplicações suaves e de suporte compacto de $\retacartesianocirculo$ em $\real{2n}$. Então, para todo $X$ no espaço de funções teste $\espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}} \cap \funcoessupcompactcontradom{\retacartesianocirculo}{\real{2n}}$ tem-se que
		$$
		\begin{aligned}
		...
		\end{aligned}
		$$
		Como o domínio $\espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}} $ de $\diferencialfloerabrev$ é denso em $\espacoLpcontradominio{\retacartesianocirculo}{\real{2n}}$ (veja em \cite{breazis_sobolev_spaces}), então o operador adjunto de $\diferencialfloerabrev$ é dado em termos de distribuição por
		$$
		\diferencialfloerabrev^{*}=-\derivadaparcial{}{s}+\estruturacomplexa \derivadaparcial{}{t}+S^{t}.
		$$
	\end{prova}
	...
	\begin{teorema}\label{teorema_regularidade_eliptica}
		(Regularidade Elíptica) Seja $p> 1$. Se $Y \in \espacoLpcontradominio{\retacartesianocirculo}{\real{2n}}$ é uma solução fraca (no sentido distribuicional) da equação $\diferencialfloerabrev Y =0$, então 
		$$
		Y \in \espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}} \cap C^{\infty}(\retacartesianocirculo;\real{2n}).$$
	\end{teorema} 
		
	Seja $T:A\to B$ um operador linear entre espaços vetoriais. Sabe-se que $T$ é um isomorfimos se, e somente se, $Ker(T) = 0$ e $T(A) = B$, isto é, $B/T(A) = 0$. Suponha que $T$ sejá limitado e que existam subespaços fechados $V \subset A$ e $W \subset B$ tais que $A=V\oplus Ker(T)$ e $B=W\oplus T(A)$. Pode-se mostrar que $B/T(A) \cong W$ e $T:V \to T(A)$ é um isomorfismo (veja $\cite{abramovich}$). Nesse sentido, fica claro que as dimensões dos espaços $Ker(T)$ e $W$ são indicadores de quanto o operador se distancia de um isomorfismo. Com essa motivação defini-se os operadores de Fredholm.
