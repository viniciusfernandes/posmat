...
\begin{document}
	...
	\begin{definicao}\label{definicao_caminhos_homotopicos}
		(Homotopia livre entre caminhos) Sejam $\gamma, \gamma' \in \caminhos$ tais que $\gamma(0)=\gamma'(0)$ e $\gamma(1)=\gamma'(1)$. Uma aplicação contínua $F:X \times [0,1] \to X$ é chamada homotopia entre $\gamma$ e $\gamma'$ se $F(t, 0) = \gamma(t)$ e $F(t, 1) = \gamma'(t)$ para qualquer $t\in \circulo$. Dizemos que  $\gamma$ e $\gamma'$ são homotópicos se, e somente se, existe uma homotopia entre eles. Nesse caso denotaremos $\gamma \sim \gamma'$. 
	\end{definicao}
	
	\begin{definicao}\label{definicao_homotopia_extremos_fixos}
		(Homotopia não-livre entre caminhos) Suponha as condições da definição anterior. Dizemos que a homotopia entre os caminhos $\gamma$ e $\gamma'$ é uma homotopia não-livre se $F(0,s) = \gamma(0) = \gamma'(0)$ e $F(1,s) = \gamma(1) = \gamma'(1)$ para todo $s\in \intervalo$.
	\end{definicao}
	
	\begin{observacao}
		Como toda homotopia não-livre entre caminhos é uma homotopia entre caminhos, também diremos que tais caminhos são homotopicamente equivalentes.
	\end{observacao}
	...
	O seguinte resultado, cuja demonstração pode ser encontrada em $\cite{massey}$, mostra que o grupo fundamental de $\circulo$ é um grupo abeliano cíclico infinito.
	...
	\begin{prova}
		\begin{enumerate}
			...
			$$
			Por outro lado, vamos supor que $n = deg(\gamma)=deg(\beta)$. Considere os levantamentos $\alpha$ e $\lambda$, dos caminhos $\gamma$ e $\beta$, respectivamente. Seja a homotopia livre $H:[0,1]\times [0,1] \to \real{}$ definida por $H(t,s) = (1-s)\alpha(t) + s\beta(t)$. Então $H(t,0)=\alpha(t)$ e $H(t,1)=\lambda(t)$, logo $H$ é uma homotopia livre entre $\alpha$ e $\lambda$. Além disso 
			$$
			\begin{aligned}
			H(1,s) - H(0,s) 
	...
	\begin{definicao}
		(Simplexo singular) Denotaremos por $\Delta_{k}$ o simplexo k-dimesional cujos vertices $e_{0}, \dots, e_{k}$ formam um base canônica de $\real{k+1}$ de modo que $\Delta_{k} = \{(x_{0}, \dots, x_{k}) \in \real{k+1}: x_{j}\geq 0, \;\sum_{j}x_{j}=1\}$. Um k-simplexo singular no espaço topológico $X$ é uma aplicação contínua $\sigma:\Delta_{k} \to X$.
	\end{definicao}
	
	Denotaremos por $\cadeia{k}{X}$ o A-módulo livre gerado pelos k-cadeias singulares de $X$, consequentemente, seus elementos são a soma formal $\alpha = \sum_{\sigma} \alpha_{\sigma}.\sigma $ de k-simplexos singulares $\sigma$, onde $\alpha_{\sigma} \in A$ é diferente do elemento neutro um número finito de vezes.
	...
	Uma consequência imediata do teorema anterior é que $\imagembordo{k}{X} \subseteq \kernelbordo{k}{X}$, logo $\imagembordo{k}{X}$ é um subgrupo normal de $\kernelbordo{k}{X}$ e o quociente $\kernelbordo{k}{X}/\imagembordo{k}{X}$ define um grupo quotiente.
	
	\begin{definicao}
		(k-grupo de homologia singular) Seja $X$ um espaço topológico. Então para cada $k \in \inteirospos$ o grupo quociente
		$$
		\homologia{k}{X} = \frac{\kernelbordo{k}{X}}{\imagembordo{k}{X}}
		$$
		é o $k$-ésimo grupo de homologia singular de $X$ com coeficientes em $A$.
	\end{definicao}
	
	\begin{definicao}
		(Grupo graduado) Um grupo abeliano graduado $G$ é uma coleção de grupos abelianos $\{G_{j} \}_{j\in \inteiros}$ tal que a operação em $G$ é fechada em cada componente. Se $G$ e $G'$ são grupos abelianos graduados, então um homomorfismo $f:G\to G'$ de grau $r$, onde $r \in \inteiros$ é uma coleção de homomorfismos $\{f_{j} \}_{j\in \inteiros}$, onde 
		$$
		f_{j}:G_{j}\to G'_{j+r}.
		$$
	\end{definicao}
	...
	Um subgrupo abeliano graduado $H\subseteq G$ é um grupo graduado $H=\{H_{j} \}_{j\in \inteiros}$, onde $H_{j} \subseteq G_{j}$ é um subgrupo para $j \in \inteiros$. Nesse caso, o grupo quociente $G/H$ é o grupo graduado $\{G_{j}/H_{j} \}_{j\in \inteiros}$.
	
	Notemos que, dado um espaço topológico $X$, as coleções $\cadeia{*}{X} = \{\cadeia{j}{X}\}_{j\in \inteiros}$, $\kernelbordo{*}{X}=\{\kernelbordo{j}{X}\}_{j\in \inteiros}$, $\imagembordo{*}{X}=\{\imagembordo{j}{X}\}_{j\in \inteiros}$ e $\homologia{*}{X}=\{\homologia{j}{X}\}_{j\in \inteiros}$ são grupos graduados.
	
	\begin{definicao}
		(Homologia singular) O grupo graduado $\homologia{*}{X}$ é chamado de homologia singular do espaço $X$ e é denotado por
		$$
		\bigoplus_{j\in \inteiros}\homologia{j}{X}.
		$$
	\end{definicao}
	
	\begin{definicao}
		(Complexo de cadeias) Um complexo de cadeias é um grupo graduado $C_{*} = \{C_{j}\}_{j\in \inteiros}$ juntamente com um homomorfismo $\bordo{}:C_{*} \to C_{*}$ (de grau -1) tal que
		$$
		\xymatrix{
			\dots \ar[r]^{\bordo{k+1}}  & C_{k} \ar[r]^{\bordo{k}} & C_{k-1}\ar[r]^{\bordo{k-1}} &\dots
		}
		$$
		e temos a composição $\bordo{k-1}\circ\bordo{k} = 0$ para $k \geq 0$. Denotamos o complexo de cadeia pelo par $(C_{*}, \bordo{*})$ e chamamos o homomorfismo $\bordo{*}$ de operador bordo do complexo de cadeia.
	\end{definicao}
	
	Note que o par $(\cadeia{k}{X}, \bordo{*})$, onde $\bordo{*}=\{\bordo{j} \}_{j\in \inteiros}$, forma um complexo de cadeia.
	
	\begin{definicao}
		(Homologia do complexo de cadeia) Seja $C=(\cadeia{k}{X}, \bordo{*})$ um complexo de cadeias. Então o $k$-ésimo grupo de homologia do complexo de cadeia $C$ é o grupo quociente
		$\homologiaabrev{k}{C} = \kernelbordoabrev{k}{C}/\imagembordoabrev{k}{C}$. O grupo graduado $\homologiaabrev{*}{C} = \{\homologiaabrev{j}{C} \}_{j\in \inteiros}$ é chamado homologia do complexo de cadeia $C$ e é denotado por
		$$
		\bigoplus_{j\in \inteiros}\homologiaabrev{j}{C}.
		$$				
	\end{definicao}
	...
	\begin{definicao}
		(Homotopia de cadeias) Sejam $f, g: C\to C'$ duas aplicações de cadeias. Dizemos que $T:C\to C'$ é uma homotopia de cadeias entre $f$ e $g$ se $T$ é um homomorfismo de grau 1 tal que $\bordo{}'T+T\bordo{} = f-g$. Assim, denotaremos $f \sim g$.
	\end{definicao}
	
	Pode-se mostrar que a homotopia entre aplicações de cadeia é uma relação de equivalência.
	
	\begin{teorema}
		Sejam $f,g : C\to C'$ duas aplicações de cadeias tais que $f \sim g$. Então ambas induzem o mesmo homomorfismos na homologia, isto é, $f_{*} = g_{*}: \homologiaabrev{*}{C} \to \homologiaabrev{*}{C'}$.
	\end{teorema}
	...
	Com isso, o teorema a seguir mostra que a homologia singular é um invariante topológico.
	
	\begin{teorema}
		(Isomorfimos induzido) Sejam $f:X \to Y$ um homomorfismo entre espaços topológicos e $C = \cadeia{*}{X}$ e $C'=\cadeia{*}{Y}$ complexos de cadeiasde $X$ e $Y$, respectivamente. Então $f_{*}: \homologiaabrev{*}{C} \to \homologiaabrev{*}{C'}$ é um isomorfismo.
	\end{teorema}
	
	Sejam $f, g: X\to Y$ duas aplicações contínuas entre espaços topológicos. Dizemos que $f$ e $g$ são homotopicamente equivalentes se existe uma aplicação contínua $h:\intercal\times X \to Y$, tal que $h(0, .) = f$ e $h(1, .) = g$. Com isso, denotamos $f\sim g$ e aplicação $h$ é chamada homotopia entre $f$ e $g$.
	
	\begin{teorema}
		Sejam $f, g: X\to Y$ duas aplicações homotóícas entre espaços topológicos. Então $f_{*}=g_{*}: \homologiaabrev{*}{C}\to \homologiaabrev{*}{C'}$, onde $C$ e $C'$ são complexos de cadeia de $X$ e $Y$, respectivamente.
	\end{teorema}
	
	\begin{definicao}
		(Equivalência homotópica) Seja $f:X\to Y$ uma aplicação entre espaços topológicos. Dizemos que $f$ é uma equivalência homotópica se existe uma aplicação $g:Y \to X$ tal que $f\circ g \sim Id_{Y}$ e $g\circ f \sim Id_{X}$. Dizemos que $X$ e $Y$ são homotopicamente equivalente e denotamos por $X\sim Y$.
	\end{definicao}
	...
	\begin{definicao}
		Uma tripla $\xymatrix{C\ar[r]^{f} & D\ar[r]^{g} & E }$ de grupos abelianos e homomorfismo é chamada exata se $Im(f) = Ker(g)$. Uma sequência de grupos abelianos e homomorfismos
		$$
	...
	Dizemos que uma sequência do tipo $ \xymatrix{0 \ar[r] & C\ar[r]^{f} & D\ar[r]^{g} & E \ar[r]& 0}$ é uma sequência exata curta. Nesse caso temos que $f$ é um monomorfismo (injetor) e $g$ é um epimorfismo (sobrejetor). Sejam $C, D, E$ são complexos de cadeias. Uma sequência exata curta de complexos de cadeias é uma sequência é a sequência exata
	$$ 
	\xymatrix{0 \ar[r] & C_{k}\ar[r]^{f_{k}} & D_{k}\ar[r]^{g_{k}} & E_{k} \ar[r]& 0}
	$$
	para todo $k \in \inteirospos$.
	
	Utilizando a sequência exata curta de complexos de cadeias, um homomorfismo entre as sequências exatas curtas, chamado homomorfismo conectante, que será utilizado na construçao da sequência exata longa. Considere o diagrama abaixo
	$$
	\xymatrix{
	...
	\begin{teorema}
		(Sequêcia do exata par $(X, U)$) Seja $(X, U)$ uma par topológico. Então a sequência exata longa desse par é a sequência
		$$
	...
	Seja $U'\subseteq U \subseteq X$ subespaços do espaço topológico $X$ e considere a tripla topológica $(X, U, U')$. Analogamente ao caso do par, o teorema seguinte garante a existência deuma sequência exata longa da tripla.
	
	\begin{teorema}
		(Sequência exta da tripla $(X, U, U')$) Seja $(X, U, U')$ uma tripla topológica. Então a sequência exata longa da tripla dessa é a sequência
		$$
	...
	\begin{definicao}
		(Grau homológico) Seja $f: S^{n} \to S^{n}$ uma aplicação contínua e $f_{*}: \homologia{n}{S^{n}} \to \homologia{n}{S^{n}}$ o homomorfismo induzindo. Seja $[S^{n}] \in \homologia{n}{S^{n}}$ o gerador não-trivial desse grupo, então $f_{*}[S^{n}] = k[S^{n}]$ para algum $k \in \Lambda$. Denominamos $k=deg(f)$ o grau da aplicação $f$.
	\end{definicao}
	...
	\begin{observacao}
		Como consequência do Lema de Morse e do Teorema da Função Inversa, pode-se mostrar que os ponto críticos de uma função de Morse $f \in \funcoesmorse{M}$ são isolados. Pela compacidade de $M$ tem-se que os pontos isolados são finitos.
	\end{observacao}
	...
	O seguinte teorema afirma que, fixando $a \in \reta$ e variando $t \in \reta$, e se $f^{-1}([a,t]) \cap \pontoscriticos{f} \neq \emptyset$, os conjuntos de nível $M^{a}$ e $M^{t}$ não podem ser deformados um no outro.
	...
	Essa partição existe e sua construção pode ser feita perturbando  a função de Morse $f$ de modo a obter uma outra $f' \in \funcoesmorse{M}$ com os mesmos pontos críticos. Essa construção é viabilizada pela densidade das funções de Morse em $\funcoessuaves{M}$.
	...
	$$
	Como $M$ é compacta e sem brodo, então a solução $\gamma = \gamma_{p}$ existe para todo $t\in \reta$. Assim, a aplicação $\phi: \retacartesianovariedade\to M$ definida por $\phi(t,p) = \gamma_{p}(t)$ é chamada de fluxo gerado por $X$ e, fixado $p\in M$, a curva $\gamma_{p}:\reta\to M$ é chamada de linha do fluxo de $X$. Como o fluxo $\phi$ está bem-definido para todo $t \in \reta$, então podemos efetuar a composição $\phi_{s}\circ\phi_{t}(p) = \phi(s+t, p)$ para todo $p\in M$, logo $\phi_{s}\circ\phi_{t} = \phi_{s+t}$ e $\phi_{0} = Id$.
	\begin{definicao}
	...
	\end{proposicao}
	 ...
	 O propósito das definições desse ponto em diante é a construção de um complexo de cadeias associado ao fluxo gradiente de uma função de Morse com algumas propriedades adicionais, que chamaremos de propriedade de transversalidade, e as variedades instáveis e estáveis.
	 
	 \begin{definicao}
	 	(Variedade Conectante e o Espaço Moduli) Sejam $f \in \funcoesmorse{M}$ e $p,q \in \pontoscriticos{f}$. A variedade conectante de $p$ e $q$ é definida por $\variedadeconectantepontos{p}{q} = \variedadeinstavel{p}\cap \variedadeestavel{q}$. Se $a = f(q)$, $b = f(p)$ e tomando $c \in (a, b) \subset \reta$ é um valor regular de $f$, o espaço moduli de $p$ e $q$ é definido por $\espacomoduli{p}{q} = \variedadeconectantepontos{p}{q}\cap f^{-1}(c)$.
	 \end{definicao}
	 
	 Sejam $f: N\to M$ e $g: Z \to M$ duas aplicações suaves entre variedades diferenciáveis. Dizemos que $f$ é transversal a $g$, isto é, $f \pitchfork g$ sempre que $f(x) = g(z) = y$ tem-se $df_{x}(\espacotangenteponto{x}{N}) + dg_{z}(\espacotangenteponto{z}{Z}) = \espacotangenteponto{y}{M} $. Se $Z \subseteq M$ e $g$ é a aplicação de inclusão, então denotaremos a transversalidade por $f\pitchfork Z$. Trataremos constatemente os casos em que $N, Z \subseteq M$, com isso $f$ e $g$ serão as inclusões e denotaremos a transversalidade por $N \pitchfork Z$.
	...
	O seguinte teorema é de grande importancia pois afirma o isomorfismo entre a homologia do complexo de Morse-Smale-Witten e a homologia singular da variedade. Sua demonstração pode ser encontrada em $\cite{banyaga_morse_homology}$.
	
	\begin{teorema}
		(Teorema da Homologia de Morse) Sejam $f \in \funcoesmorsesmale{M}$ e o par $(C_{*}(f), \partial_{*})$ é o complexo de cadeias de Morse-Smale-Witten da função $f$. Então $H_{*}((C_{*}(f), \partial_{*})) $ é isomorfo a homologia singular $ H_{*}(M, \inteiros)$.
	\end{teorema}
	...
	Daqui em diante fixaremos a notação $\complexificacaotensorialabrev{V} = \complexificacaotensorial{V}$ para a complexificação de $V$ via produto tensorial.
	...
	\begin{proposicao}\label{proposicao_base_complexificada}
		Sejam $B = \{e_{j}\}_{j=1}^{n}$ uma base do n-espaço vetorial real $V$ e $B^{*} = \{e_{j}^{*}\}_{j=1}^{n}$ a base de seu dual $V^{*}$ tal que $e^{*}_{j}(e_{k}) =\delta_{jk}$. Então $1\otimes B = \{a_{j}\}_{j=1}^{k}$ e $1^{*}\otimes B^{*}=\{a^{*}_{j}\}_{j=1}^{k}$ são bases de $\complexificacaotensorialabrev{V}$ e $\complexificacaotensorialabrev{V}^{*}$, respectivamente, tais que $a^{*}_{j}(a_{k})=\delta_{ik}$.
	\end{proposicao}
	\begin{prova}
		 Por construção $1\otimes B$ gera $\complexificacaotensorialabrev{V}$. Afirmo que $B$ é linearmente independente pois, $0=\sum_{j}z_{j}a_{j} = \sum_{j}z_{j}(1\otimes e_{j}) = \sum_{j}(b_{j} +ic_{j})(1\otimes e_{j}) = 1\otimes \sum_{j}b_{j}e_{j}+i\otimes \sum_{j}c_{j}e_{j}$, o que implica que $\sum_{j}b_{j}e_{j}=\sum_{j}c_{j}e_{j} = 0$. Logo, $b_{j}=c_{j}=0$  e $z_{j} = 0$ para $1\leq j\leq k$. O isomorfismo $\complexificacaotensorialabrev{V} \mapsto \complexificacaotensorialabrev{V}^{*}$ garante que $B^{*}$ é base de $\complexificacaotensorialabrev{V}^{*}$. Por fim, temos que $a^{*}_{j}(a_{k})=(\complexificacaoelemento{1^{*}}{e^{*}_{j}})(\complexificacaoelemento{1}{e_{k}}) = 1^{*}(1)e^{*}_{j}(e_{k}) = \delta_{ik}$.
	\end{prova}
	
	Como o produto tensorial real $\otimes_{j=1}^{k}V_{j}$ é um espaço vetorial real, então sua complexificação será o espaço vetorial complexo $\complexificacaotensorial{(\produtotensorialreal{k}{V})} \cong \complexificacaotensorial{V_{1} \otimes_{\reta} \dots \otimes_{\reta} V_{k}}$, pelo isomorfismo anteriormente citado. Contudo, pode-se se mostrar que essa complexificação é isomorfa ao produto tensorial sobre os complexos $\bigotimes^{k}_{j=1}\complexificacaotensorialabrev{V}$, a qual denotamos por $\complexificacaotensorialabrev{V}^{k\otimes}$. 
	
	Na seção seguinte vamos utilizar o produto tensorial complexificado $\complexificacaotensorialabrev{V}^{k\otimes}$ pois ela facilita a demonstração de algumas propriedades da complexificação da forma simplética, que é um tensor de ordem 2.
	
	\begin{exemplo}\label{exemplo_complexificacao_tensorial}
		Sejam $V$ um n-espaço vetorial real e $B = \{e_{j}\}_{j=1}^{n}$ uma base ortonormal. Supondo que $\complexificacaotensorialabrev{V}$ seja a complexificação de $V$, então pela Proposição $\ref{proposicao_base_complexificada}$ temos que $\complexificacaoelemento{1}{B}$ é uma base de $\complexificacaotensorialabrev{V}$. Com isso, dado $T \in \complexificacaotensorialabrev{V}^{2\otimes}$ podemos escrever $T = \sum_{i,j = 1}^{n} T^{ij}a^{*}_{i}\otimes a^{*}_{j}$, onde $T^{ij} \in \complexo{}$. Dados $v, u\in \complexificacaotensorialabrev{V}$ tais que $v=\complexificacaoelemento{x}{\alpha}$ e $u=\complexificacaoelemento{y}{\beta}$, temos
		$$
		\begin{aligned}
			T(v,u) &= \sum_{i,j = 1}^{n}
	...
	Utilizando as notações da Proposição $\ref{proposicao_base_complexificada}$, defina $\Omega=\sum_{i,j}\omega^{ij}a^{*}_{i}\otimes a^{*}_{j} \in \complexificacaotensorialabrev{V}^{2\otimes}$. Temos que
	
	\begin{proposicao} $(\complexificacaotensorialabrev{V}, \Omega)$ é um 2n-espaço vetorial simplético sobre $\complexo{}$.
	\end{proposicao}\label{proposicao_complexificacao_espaco_simpletico}
	\begin{prova}
		Pelo Teorema $\ref{teorema_isomorfismo_complexificacao}$ temos que $\complexificacaotensorialabrev{V}$ é um 2n-espaço vetorial complexo. Por construção $\Omega$ é uma aplicação $\complexo{}$-bilinear. Do fato $\omega^{ij}  = -\omega^{ji} $ temos que $\Omega$ é anti-simétrica. Afirmo que $\Omega$ é não degenerada. De fato, tomando $v, u \in \complexificacaotensorialabrev{V}$ tal que $v= \complexificacaoelemento{x}{\alpha}$ e $u=\complexificacaoelemento{y}{\beta} $, temos pelo Exemplo $\ref{exemplo_complexificacao_tensorial}$ que 
		$$
		\begin{aligned}
		...
		\end{aligned}
		$$
		Como $\omega$ é não-degenerada, então $\Omega$ é não-degenerada. Portanto, $(\complexificacaotensorialabrev{V}, \Omega)$ é um 2n-espaço vetorial simplético sobre $\complexo{}$.
	\end{prova}
	
	\begin{observacao}
		Note que $\complexificacaotensorialabrev{V}_{0} = \{\complexificacaoelemento{1}{v} \in \complexificacaotensorialabrev{V} \}$ é um subespaço vetorial de $\complexificacaotensorialabrev{V}$ isomorfo a $\real{2n}$. Além disso, $\Omega|_{\complexificacaotensorialabrev{V}_{0}} = \omega$ e $\Omega$ é uma extensão contínua de $\omega$.
	\end{observacao}
	...
	\begin{prova}
		...
		Temos que $g,k:\complexificacaotensorialabrev{V}\times \complexificacaotensorialabrev{V} \to \reta$ são aplicações $\complexo{}$-bilineares e anti-simétricas pois são combinações lineares de aplicações $\complexo{}$-bilineares anti-simétricas. Pelo mesmo argumento, $g,k$ são não-degeneradas.
	\end{prova}
	
	\begin{proposicao}\label{proposicao_base_simpletica_conjugada}
		Seja $B=\{a_{j}, f_{j} \}_{j=1}^{n}$ uma base simplética de $\complexificacaotensorialabrev{V}$, então $\overline{B}=\{\overline{a}_{j}, \overline{f}_{j}  \}_{j=1}^{n}$ é uma base simplética de $\complexificacaotensorialabrev{V}$.
	\end{proposicao}
	\begin{prova}
		Temos que a aplicação de conjugação $\complexificacaotensorialabrev{V} \ni z\otimes v \mapsto \overline{z}\otimes  \in \complexificacaotensorialabrev{V}$ é um isomorfismo. Logo, leva base em base. Como $B$ é uma base simplética, temos $\Omega(a_{i}, a_{j}) = \Omega(f_{i}, f_{j}) =0$ e $\Omega(a_{i}, f_{j}) = \delta_{ij}$. Com isso, fazendo $a_{i} = \complexificacaoelemento{z_{i}}{v_{i}}$, e pela Proposição $\ref{proposicao_complexificacao_espaco_simpletico}$ temos $\Omega(\overline{a}_{i}, \overline{a}_{j}) =	\overline{z_{i}}\overline{z_{j}}\formaSimpletica{v_{i}}{v_{j}}=\overline{z_{i}z_{j}\formaSimpletica{v_{i}}{v_{j}}} = \overline{\Omega(a_{i}, a_{j})} = 0$. Com argumento análogo pode-se ver que $\Omega(\overline{f}_{i}, \overline{f}_{j}) =0$ e $\Omega(\overline{a}_{i}, \overline{f}_{j}) =\delta_{ij}$. Logo, $\overline{B}$ é uma base simplética.
	\end{prova}
	...
	\begin{prova}
		\begin{enumerate}
			...
			\begin{aligned}
				...
				\left|
				\begin{array}{cc}
				Id-(\cos(\epsilon))_{n\times n}  & -(\sin(\epsilon))_{n\times n} 
				\\
				(\sin(\epsilon))_{n\times n}  & Id-(\cos(\epsilon))_{n\times n} 
				\end{array}
				\right| 
				\\
				&=(1-\cos(\epsilon))^{2n}+\sin^{2n}(\epsilon)\neq 0
			\end{aligned}
			$$
			...
			\end{aligned}
			$$
			onde $r(\epsilon)$ é a matriz de rotação no plano $\real{2}$ por de ângulo $\epsilon$. Com isso, $det(R(\epsilon)) = det(r(\epsilon))^{n}det(r(-\epsilon))^{n} = det(r(\epsilon))^{2n} = 1$. Além disso, temos que $R(\epsilon)^{t} = R(-\epsilon) = R(\epsilon)^{-1}$, portanto $R(\epsilon) \in \matrizortogonal{2n}$. Como $\exp(\epsilon\estruturacomplexa)$ e $R(\epsilon)$ são similares, então 
			
			é a matriz de rotação em $\real{2n}$ de um ângulo $t\epsilon$, portanto $\rho(\exp(t\estruturacomplexa S)) = exp(-it\epsilon)????????????????$
	...
	Na Seção $\ref{secao_fluxo_gradiente}$ vimos que, dada um função de Morse, construímos um complexo de cadeias com base nas linhas do fluxo do negativo do gradiente dessa função. Uma técnica análoga será adotada para a construção de um complexo de cadeias e operador bordo na qual resultará na Homologia de Floer.
	...
	Em Teoria de Morse Clássica vimos que as linhas do fluxo do negativo do gradiente de uma função de Morse definida em uma variedade fechada, $f\in \funcoesmorse{M}$, conectam dois pontos críticos (Lema $\ref{lema_conjunto_limite_funcao_morse}$). Entretanto, esse naõ é o caso das construções feitas até aqui. Temos uma variedade de Banach, isto é, estamos em dimensão infinita e a compacidade não esta garantida.
	...
	$$
	isto é, $\gamma$ é uma trajetoria do negativo do gradiente. defina o funcionaç energia
	$$
