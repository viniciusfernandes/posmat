...
\begin{document}
	...
	No caso em que a função Hamiltoniana $H$ dependente do tempo se reduz a uma Hamiltoniana autônoma, tem-se que os pontos críticos de $H$ são as soluções constantes dos sistema Hamiltoniano, o que está feito no Exemplo \ref{exemplo_funcao_hamiltoniana}.
	...
	Afirmo que a aplicação contínua $A:\reta \to \caminhosespeciais{\gruposimpletico{2n}}$ dada por $A(t) = D_{x(0)}\psi_{t}$ está bem definida. De fato, $A(0) = D_{x(0)}\psi_{0}=Id$, pois $\psi_{0}=Id$. Como $x$ é uma solução não-degenerada, então $\det(D_{x(0)}\psi_{1}-Id) \neq 0$, o que implica em $A(1)\in \gruposimpleticonaodegenerado{*}$. Além disso, escolhida uma outra base simplética $B'(t) = \{e'_{1}(t), \dots, e'_{2n}(t)\}$, tem-se que $A'(t)$ é a representação matricial de $D_{x(0)}\psi_{t}$ na base $B'(t)$ e é homotópicamente equivalente a $A(t)$, para todo $t \in \reta$. Seja $T:\reta \to \generalgroupreal{2}$ uma família de isomorfismos tal que $T(t)e_{j}(t) = e'_{j}(t)$. Então, supondo que $\omega_{x(t)}(e'_{j}(t), e'_{k}(t))=\omega_{x(t)}(e_{j}(t), e_{k}(t))=\delta_{jk}$, tem-se que
	$$
	\begin{aligned}
	...
	\end{aligned}
	$$
	O mesmo vale para os elementos das bases $B(t)$ e $B'(t)$ que são $\omega$-ortogonais. Portanto, $T(t)\in \gruposimpletico{2n}$ para todo $t\in \reta$. Com isso, pode-se escrever $A(t) = (T(t))^{-1}A'(t)T(t)$, e pela propriedade de homotopia dsecrita no Item \ref{item_naturalidade_maslov}, tem-se que 
	$$
	\mu(A) = \mu(T^{-1}A'T) = \mu(A').
	$$
	...
	\end{observacao}
	 ...
	 Seja $x\in \pontoscriticos{\funcionalH}$ e defina $Cr(x;k) = \{ y \in \pontoscriticos{\funcionalH}: \mu(u)=k,\; u\in \energiafinitaM(x,y)\}$. Considere $C_{k}(M, \inteiros_{2}; H)$ um espaço vetorial sobre o corpo $\inteiros_{2}$ e gerado pelos elementos $x \in \lacocontrateis$, isto é
	$$
	C_{k}(M, \inteiros_{2}; H) = \bigoplus_{x\in \pontoscriticos{\funcionalH}}n_{x}\gerador{x}.
	$$
	\vermelho{(Aqui devemos entender melhor em quais elementos estamos avaliando o índice de Maslov...!!!)}
	...
	\vermelho{	O caso de sistemas Hamiltonianos autônomos fica reduzido ao caso de uma função de Morse $H:M\to \real{}$ cujo conjunto de soluções 1-periódicas $\lacocontrateis$ será o conjunto de pontos críticos isolados de $H$. Com isso, o índice de Maslov terá uma relação direta com índice de Morse da seguinte forma: (Verificar o melhor local do texto para inserir esse resultado!!!)}
