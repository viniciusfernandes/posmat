...
\begin{document}
	...
	Sejam $(V, \omega)$ um 2n-espaço vetorial simplético e $L(V)$ é o conjunto dos subespaços Lagrangianos de $V$.  $\caminhossubespacoslagrangianos{a}{b}= \{\gamma:[a,b]\to \subespacoslagrangianos \times \subespacoslagrangianos \}$ o conjunto de todos os caminho contínuos e suaves por partes em $\subespacoslagrangianos \times \subespacoslagrangianos$. Como topologia de $\caminhossubespacoslagrangianos{a}{b}$ vamos adotar a topologia compacto-aberta (veja $\cite{munkres_topology}$). Um índice de Maslov é uma aplicação $\mu:\caminhossubespacoslagrangianos{a}{b}\to \inteiros$ satisfazendo as seguintes propriedades:
	\begin{enumerate}
		\item \textbf{\textit{(Invariância por Translação)}} Seja $g: [a,b] \to [ak+l, bk+l]$ tal que $g(t)=at+l$, para $k>0$ e $l\geq 0$. Então, dado $\gamma\in \caminhossubespacoslagrangianos{ak+l}{bk+l}$ temos que 
		$$
		\mu(\gamma) = \mu(\gamma\circ g).
		$$
		\item \textbf{\textit{(Invariância por Homotopia)}} Sejam $\gamma, \beta \in \caminhossubespacoslagrangianos{a}{arg2}$ e $h:[a,b] \times [0,1]$ uma homotopia entre $\gamma$ e $\beta$ tal que $h(t,0)= \gamma(t)$, $h(t,1) = \beta(t)$ e $h(s, a)$ e $h(s, b)$ constantes. Então
		$$
		\mu(\gamma) = \mu(\beta).
		$$
