...
\begin{document}
	...
	\begin{observacao}\label{observacao_identificacao_complexos}
			Sabemos que $\real{2n}$ e $\mathbb{C}^{n}$ são espaços vetoriais de dimensão $2n$, logo são isomorfos. Tal isomorfismo pode ser dado por $\real{2n}\ni (v_{1}, v_{2}) = v \mapsto z = v_{1}+iv_{2} \in \complexo{n}$. Desse modo temos $\estruturacomplexa v = (v_{2}, -v_{1}) \mapsto v_{2}-iv_{1} = -i(v_{1}+iv_{2}) = -iz \in \complexo{n}$. Portanto, aplicar $\estruturacomplexa$ é equivalente a multiplicação por $-i$. 
			Por definição temos $\formaSimpleticaPadrao{v}{u} = (\estruturacomplexa v)^{t}.u=v_{1}u_{2}-v_{2}u_{1}$. Então, $(\estruturacomplexa v)^{t}  \mapsto -i\overline{v}.u \in \reta$, portanto temos a associação $\formaSimpleticaPadrao{v}{u} \mapsto \parteImaginaria{\Omega(\overline{v},u)}$ com a parte imaginária de $\Omega$, uma forma bilinear, antí-simétrica e não degenerada nos complexos.
	\end{observacao} 
	...
	\begin{prova}
		\begin{enumerate}
			...
			\item Se $v, u \in E_{1}$, então pelo Lema $\ref{lema_auto_espaco_grupo_simpletico}$ temos $\formaSimpletica{v}{u} \neq 0$, logo a restrição  $\omega|_{E_{1}\times E_{1}}$ é não-degenerada. Com um argumento análogo podemos afirmar o mesmo para a restrição $\omega|_{E_{-1}\times E_{-1}}$.
	...
	\begin{corolario}
		Sejam $Q: \complexo{n} \times \complexo{n} \to \reta$ a forma quadrática definida por $Q(v) = B(v,v)$ e $m(Q) = m_{+}(Q) - m_{-}(Q) $, onde $m_{\pm}(Q) \in \inteiros$ são as dimensões dos subespaços em que $Q$ é positiva-definida, ou negativa-definida, respectivamente. Então $Q$ é não-degenerada e $m(Q) = 0$.
	\end{corolario}
	\begin{prova}
