...
\newcommand{\hessiana}{\text{Hess}_{p}(f)}
...
\newcommand{\orbitasConectantesZ}{\mathcal{Z}(x^{-}, x^{+}, J)}
...
\begin{document}
	...
	\title{Os Índices de Maslov, o Índice de Conely-Zehnder e suas Aplicações em Topologia Simplética: a homologia de Floer e a Conjectura de Arnold.}
...
\vermelho{	No caso em que a função Hamiltoniana $H$ dependente do tempo se reduz a uma Hamiltoniana autônoma, tem-se que os pontos críticos de $H$ são as soluções constantes dos sistema Hamiltoniano, o que está feito no Exemplo \ref{exemplo_funcao_hamiltoniana}.
	...
	Sob algumas condições, uma função Hamiltoniana pode ser vista como um função de Morse, e com isso, é natural se questionar sobre a relação entre o índice de Conley-Zehnder e o índice de Morse. O seguinte resultado esclarece essa questão.
	...
	Com isso, o diferencial $D_{x}\psi_{t}$ pode ser visto como um caminho de matrizes. Logo, seu vetori tangente é dado por
	$$
	\begin{aligned}
	...
	$$
	\exp(-\estruturacomplexa \hessianaponto{x}{H}) \in \caminhosespeciais{\gruposimpletico{2n}},
	$$o que pode ser encontrado na demonstração do Item \ref{item_assinatura_maslov} do Teorema \ref{teorema_indice_maslov}.
	...
	\begin{prova}
		...
		\begin{teorema}\label{teorema_indice_diferencial_operador_floer}
			O índice de Fredholm de $\diferencialfloer$ é \vermelho{(definir quem são $x^{\pm}$)}
			$$
	...
	Sejam $x^{-},x^{+}\in \pontoscriticos{\funcionalH}$, e considere o operador de Floer 
	$$
	...
	Seja 
	$
	\orbitasConectantesZ = \{ (u, \vermelho{H+h}): u \in  \energiafinitaM(x^{-}, x^{+}, J, H) \;\;\text{e}\;\; h\in \perturbacaoHamiltoniana{H} \}
	$ o conjunto de todas as soluções da equação de Floer conectando $x^{-}$ a $x^{+}$ de todas as perturbações da função Hamiltoniana $H$.
	...
	\begin{proposicao}
	Para todos $x^{-},x^{+}\in \pontoscriticos{\funcionalH}$ com $x^{+}\neq  x^{-}$ tem-se que $\mathcal{Z}(x^{-}, x^{+}, J)$ é uma variedade (de Banach). 
	\end{proposicao}
	...
	$$
	\energiafinitaM(x^{-}, x^{+}; H+h) \subset\pi^{-1}(h).
	$$
	
	Do Teorema da Regularidade Elíptica \ref{teorema_regularidade_eliptica}, pode-se concluir que 
	$$
	\pi^{-1}(h) \subset\energiafinitaM(x^{-}, x^{+}; H+h),
	$$
	e portanto $\energiafinitaM(x^{-}, x^{+}; H+h)=\pi^{-1}(h)$.
	
	\begin{teorema}
		Para cada $h \in \perturbacaoHamiltoniana{H}$ e para toda solução contrátil 1-periódica $x^{-}, x^{+}$ de $H$, $\energiafinitaM(x^{-}, x^{+}, H+h)$ é uma variedade de dimensão $\mu(x^{-})-\mu(x^{+})$.
	\end{teorema} 
