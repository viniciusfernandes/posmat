...
\alerta{	\begin{observacao} (Não sei onde colocar essa observação...!!!!)
	...
	\bigparenteses{
		...
		\\
		&=\underbrace{\bigparenteses{\derivadaparcial{}{s}+J_{u}\derivadaparcial{}{t}}}_{\overline{\partial}_{u}}(Y) 
		+ 
		\underbrace{\bigparenteses{(d_{u}J)(.)\bigparenteses{ \derivadaparcial{u}{t} - \campohamiltoniano{u}}-J_{u}(d_{u}\campohamiltonianoabrev)}}_{S_{u}}(Y)
		\\
		&=(\overline{\partial}_{u}+ S_{u})(Y).
		\end{aligned}
	$$
	...
	Seja $Y(s,t) = (y_{1}(s,t), \dots , y_{2n}(s,t)) \in \real{2n}\subset \real{m}$, que pode ser visto como um campo vetorial $Y=\sum y_{j}e_{j}$ em $\real{m}$ e tangente a $u(\retacartesianocirculo)\subset M$.
	
	\alerta{Comentar a escolha desse Y...ver como foram construidas as cartas em u!!!}
		
	$$
	\begin{aligned}
	\operadorFloerParametro{u+Y}-\operadorFloerParametro{u}
	&= \operadorFloerDefinicao{u+Y}
	\\
	&\;\;\;\; -\derivadaparcial{u}{s} - J_{u}\derivadaparcial{u}{t} + J_{u}\campohamiltoniano{u}
	\\
	&=\derivadaparcial{Y}{s}+J_{u+Y}\derivadaparcial{Y}{t}+\bigparenteses{J_{u+Y}-J_{u}}\derivadaparcial{u}{t}
	\\
	&\;\;\;\;-\bigparenteses{J_{u+Y}-J_{u}}\campohamiltoniano{u} 
	-J_{u+Y}\bigparenteses{\campohamiltoniano{u+Y}-\campohamiltoniano{u}}
	\\
	&=\derivadaparcial{Y}{s} + J_{u+Y}\derivadaparcial{Y}{t} + \bigparenteses{J_{u+Y}-J_{u}}\bigparenteses{\derivadaparcial{u}{t} - X_{H}(u)}  
	\\
	&\;\;\;\; -J_{u+Y}\bigparenteses{X_{H}(u+Y) - X_{H}(Y)}.
	\end{aligned}
	$$
	Como os termos $u, Y, J, X_{H}$ são de classe $C^{\infty}$, então o diferencial de $\operadorFloer$ avaliado no ponto $u\in \caminhosexponenciaisconectantesabrev$ é o limite 
	$$
	\begin{aligned}
	\diferencialfloerponto{u}(Y)
	&=\lim_{Y \to 0} \frac{	\operadorFloerParametro{u+Y}-\operadorFloerParametro{u}}{\norma{Y}}
	\\
	&= \derivadaparcial{Y}{s} + J_{u}\derivadaparcial{Y}{t}+ dJ_{u}(Y)\bigparenteses{\derivadaparcial{u}{t} - X_{H}(u)} - J_{u} \diferencialhamiltoniano{u}(Y)
	\\
	&= \underbrace{\Big( \derivadaparcial{}{s} + J_{u}\derivadaparcial {}{t}\Big)}_{\overline{\partial}_{u}}(Y)+ \underbrace{\Big(dJ_{u}(.)\bigparenteses{\derivadaparcial{u}{t} - X_{H}(u)} - J_{u} \diferencialhamiltoniano{u}\Big)}_{S_{u}}(Y)
	\\
	&= \operadorcauchyabrev{u} Y + S_{u}Y
	\\
	&= (\operadorcauchyabrev{u}  + S_{u})(Y).
	\end{aligned}
	$$
