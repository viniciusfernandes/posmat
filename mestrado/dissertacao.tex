...
\alerta{	\begin{observacao} (Não sei onde colocar essa observação...!!!!)
	...
	Logo, o operador de Floer $\operadorFloer$ pode ser tratado como uma aplicação $\operadorFloer: \caminhosexponenciaisconectantespadrao\to \espacoLpcontradominio{\retacartesianocirculo}{TM}$. 
	
	Tome uma carta $(D_{w}, \Exp_{w})$ em $u$ com raio $r_{w}>0$. Note que, todo $\tilde{u} \in D_{w}$ e suficientemente próximo a $u$ pode ser escrito como $\tilde{u}=\Exp_{w}(Y)$ para algum $Y\in \espacosobolev{\pullbackfibradotangenteM{w}}$ e que $\Exp_{w}(0)=w$. Seja $\epsilon>0$ tal que $\tilde{u}(\tau) = \Exp(w, \tau Y) \in D_{w}$ para todo $\tau \in (-\epsilon, \epsilon)$. O diferencial diferencial de $\operadorFloer$ em $u \in \caminhosexponenciaisconectantespadrao$ é determinado tomando o limite
	$$
	...
	Considere a aplicação $\pi_{2}\circ i \circ \operadorFloer: \caminhosexponenciaisconectantespadrao\to \espacoLpcontradominio{\retacartesianocirculo}{\real{m}}$ dada por 
	$$
	(\pi_{2}\circ i \circ \operadorFloer)(u)=\operadorFloerDefinicao{u},
	$$
	que por um abuso de notação será denotado $\pi_{2}\circ i \circ \operadorFloer$ por $\operadorFloer$. Seja $Y(s,t) = (y_{1}(s,t), \dots , y_{2n}(s,t)) \in \real{2n}\subset \real{m}$, que pode ser visto como um campo vetorial $Y=\sum y_{j}e_{j}$ em $\real{m}$.
	
	\alerta{Comentar a escolha desse Y...ver como foram construidas as cartas em u!!!}

	Logo, o operador de Floer $\operadorFloer$ pode ser tratado como uma aplicação $\operadorFloer: \caminhosexponenciaisconectantespadrao\to \espacoLpcontradominio{\retacartesianocirculo}{\real{m}}$. Tome uma carta $(D_{w}, \Exp_{w})$ em $u$ com raio $r_{w}>0$. Note que, todo $\tilde{u} \in D_{w}$ e suficientemente próximo a $u$ pode ser escrito como $\tilde{u}=\Exp_{w}(Y)$ para algum $Y\in \espacosobolev{\pullbackfibradotangenteM{w}}$ e que $\Exp_{w}(0)=w$. Seja $\epsilon>0$ tal que $\tilde{u}(\tau) = \Exp(w, \tau Y) \in D_{w}$ para todo $\tau \in (-\epsilon, \epsilon)$. O diferencial diferencial de $\operadorFloer$ em $u \in \caminhosexponenciaisconectantespadrao$ é determinado tomando o limite
	$$
		\lim\limits_{\tau\to 0}\frac{\operadorFloerParametro{\tilde{u}(\tau)} - \operadorFloerParametro{u}}{\tau}.
	$$
	...
	Considere a aplicação $i \circ \operadorFloer: \caminhosexponenciaisconectantespadrao\to \espacoLpcontradominio{\retacartesianocirculo}{\real{m}}$ definida por 
	$$
	(i \circ \operadorFloer)(u)=\operadorFloerDefinicao{u},
	$$
	onde $i$ é a a composição do mergulho $TM\hookrightarrow \real{m}\times \real{m}$ com a projeção $\pi_{2}$ anteriomente definida. Por um abuso de notação será denotado $i \circ \operadorFloer$ por $\operadorFloer$.
	...
	\\
	&=\derivadaparcial{Y}{s}+J(u+Y)\derivadaparcial{Y}{t}+\bigparenteses{J(u+Y)-J(u)}\derivadaparcial{u}{t}
	\\
	&\;\;\;\;-\bigparenteses{J(u+Y)-J(u)}\campohamiltoniano{u} 
	-J(u+Y)\bigparenteses{\campohamiltoniano{u+Y}-\campohamiltoniano{u}}
	\\
	&=\derivadaparcial{Y}{s} + J(u+Y)\derivadaparcial{Y}{t} + \bigparenteses{J(u+Y)-J(u)}\bigparenteses{\derivadaparcial{u}{t} - X_{H}(u)}  
	\\
	&\;\;\;\; -J(u+Y)\bigparenteses{X_{H}(u+Y) - X_{H}(Y)}.
	\end{aligned}
	$$
	...
	\\
	&= \derivadaparcial{Y}{s} + J(u)\derivadaparcial{Y}{t}+ dJ_{u}(Y)\bigparenteses{\derivadaparcial{u}{t} - X_{H}(u)} - J(u) \diferencialhamiltoniano{u}(Y)
	\\
	&= \underbrace{\Big( \derivadaparcial{}{s} + J(u)\derivadaparcial {}{t}\Big)}_{\overline{\partial}_{u}}(Y)+ \underbrace{\Big(dJ_{u}(.)\bigparenteses{\derivadaparcial{u}{t} - X_{H}(u)} - J(u) \diferencialhamiltoniano{u}\Big)}_{S_{u}}(Y)
	\\
	...
	\begin{definicao}
		...
		$$
		\diferencialfloerponto{u}(Y)= \Big( \derivadaparcial{}{s} + J(u)\derivadaparcial {}{t}\Big)(Y)+ \Big(dJ_{u}(.)\bigparenteses{\derivadaparcial{u}{t} - X_{H}(u)} - J(u) \diferencialhamiltoniano{u}\Big)(Y).
		$$
	\end{definicao}
