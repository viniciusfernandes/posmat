...
\begin{document}
	...
	\begin{prova}
		...
		Note que, $\rho(\psi^{-1}(t))$ é a produtória do inverso dos auto-valores de $\psi(t)$. Como a produtória do inverso é o inverso da produtória, então $\rho(\psi^{-1}(t)) = \rho(\psi(t))^{-1}$ para todo $t\in \intervalo$, como desejávamos. 
	\end{prova}
	...
	\begin{prova}
		\begin{enumerate}
			...
			\item \textbf{(Homotopia)} Sejam $\psi_{1}, \psi_{2}\in \caminhosespeciais{\gruposimpletico{2n}}$ e $h:\intervalo\times \intervalo \to \gruposimpletico{2n}$ uma homotopia entre eles tal que $h(t,0) = \psi_{1}(t)$ e $h(t,1) = \psi_{2}(t)$.
			...
			\item \textbf{(Inverso)} O Lema \ref{lema_inversa_caminho_especial} mostra que $\psi^{-1} \in \caminhosespeciais{\gruposimpletico{2n}}$. Além disso, se $\rho_{\psi}(t) = \exp(i\alpha_{\psi}(t))$, então $\rho_{\psi^{-1}}(t) = (\rho_{\psi}(t))^{-1} = \exp(-i\alpha_{\psi}(t))$. Logo 
			$$
	...
	\begin{enumerate}
		\item \textbf{\textit{(Invariância por Translação)}} 
