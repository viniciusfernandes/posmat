...
\begin{document}
	...
	\begin{enumerate}
		...
		\item \textbf{\textit{(Normalização)}} Seja $\mathbb{C} = \real{2}$ o espaço vetorial simplético munido do produto interno $\iprod{v}{u} = v_{1}u_{2}- v_{2}u_{1} = Re(i(v_{1}+iv_{2})\overline{(u_{1}+iu_{2})})$, onde $v=(v_{1},v_{2}),u (u_{1},u_{2})\in \real{2}$. Defina $\gamma \in \caminhos{-\pi/4}{\pi/4}{\real{2}}$ tal que $\gamma(t) = (\reta(1), \reta(e^{it}))$. Então
			...
			\end{enumerate}
			\begin{observacao}
				Uma discussão mais extensa e contendo diagramas esquemáticos pode ser encontrada em $\cite{cappell_maslov_index_equivalencia}$.
			\end{observacao}
	\end{enumerate}
	...
	\begin{thebibliography}{9}
		...
		\emph{On Masvlov Index}, Communication on Pure and Applied Mathematics, Vol. XLVII, 121-186 (1994).
