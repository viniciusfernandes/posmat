...
\newcommand{\distribuicoes}[1]{\mathcal{D}(#1)}
...
\begin{document}
	...
	\begin{prova}
		\begin{enumerate}
			...
			\item Sejam $A \in \gruposimpletico{2n}$ e $\psi \in \mathcal{L}(\gruposimpletico{2n})$ tal que $\psi(0) = Id $ e $\psi(1) = A$ e o caminho contínuo $\gamma:[0,1] \to \gruposimpleticonaodegenerado{*}$ tal que $\psi(1) = \gamma(0)$ e $\gamma(1)= W^{\pm}$. Se $\psi(1) \in \gruposimpleticonaodegenerado{+}$, então $det(Id - \psi(1))>0$, então $\gamma(1) = W^{+}$ e $\rho(W^{+}) = (-1)^{n}$. Com isso $\mu(\psi) - n$ é par \vermelho{(não entendi esse argumento. No michelle e no salamon estão iguais.)}
			
			\item Afirmo que, $\Psi(t)=\exp(t\estruturacomplexa S) \in\gruposimpletico{2n}$. Para isso é suficiente mostrar que $\Psi(t)$ comuta com $\estruturacomplexa$, ou  de forma equivalente, $\estruturacomplexa^{-1}\Psi(t)\estruturacomplexa = \Psi(t)$. Seja $k\geq 0$ um inteiro. Então
			$$
			\begin{aligned}
			\estruturacomplexa^{-1}(\estruturacomplexa S)^{k}\estruturacomplexa 
			&= \estruturacomplexa^{-1}\underbrace{(\estruturacomplexa S)\dots (\estruturacomplexa S)}_{k-vezes}\estruturacomplexa
			\\
			&=\underbrace{(S\estruturacomplexa )\dots (S\estruturacomplexa )}_{k-vezes}
			\\
			&=(S\estruturacomplexa )^{k},
			\end{aligned}
			$$
			que implica em 
			$$
				\begin{aligned}
				\estruturacomplexa^{-1}\exp(t\estruturacomplexa S)\estruturacomplexa &=\sum_{k=0}^{\infty}\frac{t^{k}}{k!}\estruturacomplexa^{-1}(\estruturacomplexa S)^{k}\estruturacomplexa
				\\
				&=\sum_{k=0}^{\infty}\frac{t^{k}}{k!}(S\estruturacomplexa)^{k}
				\\
				&=\exp(tS\estruturacomplexa).
				\end{aligned}
			$$
			Como $(\estruturacomplexa S )^{t}= S^{t}\estruturacomplexa^{t} = -S\estruturacomplexa$, temos $\exp(tS\estruturacomplexa)^{t} = \exp(t(S\estruturacomplexa)^{t}) = \exp(t\estruturacomplexa^{t}S^{t} = \exp(-t\estruturacomplexa S)$ \vermelho{(para mim essa afirmação vale se tivermos uma matriz anti-simétrica, iso é, $S^{t} =-S$...e agora?????).}
			...
			Notemos que $\exp(t\epsilon\estruturacomplexa )$ é a matriz de rotação em $\real{2n}$ de um ângulo $t\epsilon$, porntanto 
			
			
			Por fim, $\rho(\exp(t\estruturacomplexa S))$
	...
	\section{Teoria de Distribuições - Espaços de Schwartz}
	...
	Sejam $f:\Omega\to \reta$ uma função e $\Omega \subseteq \real{n}$ um aberto. O suporte de $f$ é o definido por $supp(f) = \overline{\{ x\in \Omega: f(x)\neq 0 \}}$. Denotaremos por $\funcoesdiferenciaveissupp{k}{\Omega} \subset \funcoesdiferenciaveis{k}{\Omega}$ o conjunto de todas as funções de classe $C^{k}$ com suporte compacto. Os elementos de tal conjunto receberão o nome de funções de teste.
	
	\begin{observacao}
		É possível se mostrar que o espaço das funções de teste $\funcoesdiferenciaveissupp{k}{\Omega}$ forma um espaço vetorial real, veja em $\cite{friedlander}$.
	\end{observacao}
	...
	\begin{definicao}
		(Espaço das distribuições) Seja $\Omega \subseteq \real{n}$ um aberto. Uma aplicação linear $f:\funcoesdiferenciaveissupp{\infty}{\Omega} \to \reta$ é chamada de distribuição se, para cada compacto $K\subset \Omega$, existe $c\geq 0$ e $m \in \mathcal{N}$ tal que 
		$$
			|\produtointerno{f}{\phi}| \leq c \sum_{|\alpha|\leq m} \sup|\partial^{\alpha}\phi|,
		$$
		para todo $\phi \in \funcoesdiferenciaveissupp{\infty}{\Omega}$ com $supp (\phi) \subseteq K$. O espaço das distribuições é denotado por $\distribuicoes{\Omega}$.
	\end{definicao}
	...
	\begin{definicao}
		(Espaço das funções p-integráveis) Sejam $\Omega\subset \real{n}$ um aberto e $1\leq p \leq \infty$. O espaço das funções p-integráveis em $\Omega$ é o conjunto 
		$$
		L^{p}(\Omega) = \{f:\Omega \to \real{} : \normaLp{f} < \infty \}, \; \normaLp{f} = \normaLpdefinicao{|f(x)|}{\Omega}.
		$$
		No caso em que $p=\infty$ temos $\norma{f}_{L^{\infty}} = inf\{C\geq 0: |f(x)|\leq C\}$.
	\end{definicao}
	...
	O espaço $L^{p}(\Omega)$ é um espaço de Banach, o que esta demonstrado em $\cite{breazis_sobolev_spaces}$. Como a equação de Floer envolve derivadas de primeira ordem, o espaço de Banach com o qual trabalharemos será munido de uma norma que leva em consideração o comportamento dessas derivadas. Tal espaço normado é chamado por espaço de Sobolev.
	...
	A demonstração será dividida em 4 etapas, sendo que algumas das construções feitas tiveram como base a referência $\cite{salamon_lecture}$ em sua maior parte e $\cite{audi_floer_homology}$ como apoio.
	...
	\textbf{Etapa 2:}
	O ponto de partida dessa etapa é o lema abaixo, o qual tem sua demonstração feita em $\cite{audi_floer_homology}$. Não a faremos pois é demasiadamente técnica e não trará entendimento adicional ao texto. Enunciaremos o teorema que estabelece a desigualdade de Holder (vide $\cite{breazis_sobolev_spaces}$), o qual será utilizado no lema a seguir.
	
	\begin{teorema}
		(Desigualdade de Holder) Sejam $1\leq p, q \leq \infty$ tais que $1/p + 1/q = 1$. Suponha que $f \in L^{p}(\Omega)$ e $g \in L^{q}(\Omega)$. Então $fg \in L^{1}(\Omega)$. Além disso, vale a desigualdade
		$$
		\int_{\Omega}|f(x)g(x)| \leq \normaLp{f}\norma{g}_{L^{q}}.
		$$ 
	...
	$$
	De fato, fazendo $f = g \in \espacoLp{\Omega}$ na desigualdade de Holder, teremos $\int $ dados $\xi:\retacartesianocirculo\to\real{2n}$ e $V \subseteq \reta$, então pela desigualdade de Holder temos que $\espacoLp{V\times \circulo} \subset \espacoLdois{V\times \circulo}$ e, analogamente, $\espacosobolev{V\times \circulo} \subset \espacosobolevgeral{2}{V\times \circulo}$. Pelo Teorema $\ref{teorema_sobolev}$ temos que $\espacosobolevgeral{2}{V\times \circulo} \subset \espacoLp{V\times \circulo}$ para 
