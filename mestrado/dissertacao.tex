...
\begin{document}
	...
	\section{Variedades Simpléticas}\vermelho{(Acho que devo remover toda essa seção. Não estou vendo importância alguma dela para o texto.)}
	\begin{definicao}
	...
	\section{Fibrados Vetoriais - preliminares}
	\begin{definicao}
	...
	\begin{definicao}
		(Operador de Floer) Sejam $\gamma^{-}, \gamma^{+}\in \pontoscriticos{\funcionalH}$. O operador de Floer é o operador diferencial $\operadorflor: \caminhosexponenciaisconectantesabrev \to T\caminhosexponenciaisconectantesabrev$ definido por
		$$
	...
	$$
	Logo $\operadorflorparametro{u} \in L^{p}(\pullbackfibradotangenteMpadrao)$ e o operador de Floer esta bem-definido.
	
	Lembrando que, dado $u \in \caminhosexponenciaisconectantesabrev$, tem-se que $u:\retacartesianocirculo\to M$. Com isso, $\operadorflorparametro{u}$ é tal que $\operadorflorparametro{u}(s, t) \in \espacotangenteponto{u(s,t)}{M}$ para todo $(s,t)\in \retacartesianocirculo$. É sob esse sentido que se obterá o diferencial de $\operadorflor$. Como $M$ é uma variedade Riemanniana, a variação de $\operadorflor$ pode ser obtida realizando o transporte paralelo desse campo ao longo de uma dada curva. O campo vetorial resultante é expresso em termos de conexões afins (veja a Definição \ref{definicao_conexao_afim} e a Observação \ref{observacao_transporte_paralelo}) e, de acordo com \cite{salamon_zehnder} e \cite{salamon_lecture}, é dado por
	
	$$
	\nabla\operadorflorparametro{u}(\xi) = \nabla_{s}\xi + J(u)\nabla_{t}\xi + dJ(u)(\xi)(\xi - X_{H}(u)) - J(u)(dX_{H})_{u}(\xi),
	$$
	...
	$$
	\begin{aligned}
	\operadorflorparametro{u+\xi}-\operadorflorparametro{u}
	&= \operadorflordefinicao{u+\xi}
	\\
	...
	\end{aligned}
	$$
	Como os termos $u, \xi, J, X_{H}$ são de classe $C^{\infty}$, então o diferencial de $\operadorflor$ avaliado no ponto $u\in \caminhosexponenciaisconectantesabrev$ é o limite 
	$$
	\begin{aligned}
	\diferencialfloer_{u}(\xi)
	&=\lim_{\xi \to 0} \frac{	\operadorflorparametro{u+\xi}-\operadorflorparametro{u}}{\norma{\xi}}
	\\
	...
	Daqui em diante, quando não houver ambiguidades, denotaremos $D\operadorflor_{u}$ por $\diferencialfloerabrev$, para um dado $u\in \caminhosexponenciaisconectantespadrao$.
	...
	\begin{prova}
		...
		\begin{teorema}
			O índice de Fredholm de $\diferencialfloer$ é 
			$$
