...
\begin{document}
	...
	\begin{definicao}
		(Variedade Simplética) Seja M uma $2n$-variedade diferenciável e $\omega \in \Omega^{2}(M)$ uma 2-forma fechada e não-degenerada, isto é, $d\omega = 0$. Se  para todo $x \in M$ tem-se que $(T_{x}M, \omega_{x})$ é um espaço vetorial simplético, então o par $(M, \omega)$ é uma $2n$-variedade simplética.
	\end{definicao}
	...
	\begin{definicao}
		(Gradiente simplético) Seja $f : M \times \real{} \to \real{}$ uma função suave, então o gradiente simplético de $f$ é o único campo vetorial $X \in \mathfrak{X}(M)$ tal que $\omega(X, Y) = df(Y)$ para todo $Y \in \mathfrak{X}(M)$.
	\end{definicao}
	
	\begin{definicao}
		(Campo hamiltoniano) Seja $(M, \omega)$ uma $2n$-variedade simplética. Uma função suave $H : M \times \real{} \to \real{}$ é chamada uma função hamiltoniana se satisfaz as equações diferenciais de Hamilton
		$$
		...
		$$
	\end{definicao}
	Ao campo Hamiltoniano $\campohamiltonianoabrev$ pode-se o associar o fluxo $\psi:M\times\reta\to M$, chamado fluxo Hamiltoniano, o qual satisfaz o seguinte sistema de equações diferenciais
	$$
	\label{sisHamilt}
	...
	\begin{proposicao}
		Seja $\psi:M\times\reta\to M$ o fluxo associado a função Hamiltoniana $H:M\times\reta\to M$. Então $\psi_{t}^{*}\omega=\omega$ para todo $t\in \reta$.
	\end{proposicao}
	\begin{prova}
		$$
		\begin{aligned}
	...
	\begin{exemplo}
		(Função hamiltoniana) Com base no exemplo anterior, defina $H:S^{2}\times \reta\to \reta$ por $H(q,p) = q^{2}/2+p^{2}/2$. Pode-se verificar que $q(t) = Asent(t)+Bcos(t)$ e $p(t) = Acos(t)-Bsent(t)$ são soluções do sistema Hamiltoniano. Logo, $H$ é uma função Hamiltoniana. Com isso, o fluxo Hamiltoniano é
		$$
		\psi(t)=(Asent(t)+Bcos(t), Acos(t)-Bsent(t)),
		$$
		e o campo Hamiltoniano é 
		$$
		\campohamiltonianoabrev(t) = (Acos(t)-Bsent(t))\partial_{q}-(Asent(t)+Bcos(t))\partial_{p}.
		$$
	\end{exemplo}
