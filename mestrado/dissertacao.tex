...
\begin{document}
	...
	No Capítulo \ref{capitulo_teoria_morse} mostrou-se que as linhas de fluxo do gradiente negativo de uma função de Morse, definida em uma variedade fechada, conectam dois pontos críticos (veja o Lema $\ref{lema_conjunto_limite_funcao_morse}$). Entretanto, esse naõ é o caso das construções feitas até aqui. Temos uma variedade de Banach, isto é, estamos em dimensão infinita e a compacidade não esta garantida.
	
	Ainda no caso clássico, seja $f \in \funcoesmorse{M}$ e --suponha que $\gamma:\reta \to M$ seja uma solução do sistema de equações diferenciais
	$$
	\derivada{}{s}\gamma(s)=-\gradiente(\gamma(s)),
	$$
	isto é, $\gamma$ é uma trajetoria do gradiente negativo. Defina o funcional energia
	$$
	...
	Seja $\cilindrosLM$ o conjunto dos caminhos de classe $C^{1}$ em $\lacocontrateis$. Então
	...
	Suponha que $u \in \cilindrosLM$ é uma trajetória de $-\gradientefuncional \in T\lacocontrateis$ e que conecte os pontos críticos $\gamma,\gamma' \in \pontoscriticos{\funcionalH}$, isto é, $\lim_{s\to -\infty}u(s)=\gamma'$ e $\lim_{s\to \infty}u(s)=\gamma$. Com isso
	$$
	E(u) = \funcionalH(\gamma') - \funcionalH(\gamma).
	$$
	
	Além disso, dado $Y \in T\reta$, tem-se que
	$$
	\begin{aligned}
	...
	\end{aligned}
	$$
	Como $Y\in T\reta$ é arbitrário, então $(u^{*}d\funcionalH)_{s} = -\norma{\gradientefuncional(u(s))}^{2}$. Voltando no funcional energia, tem-se
	$$
...
Pelo Teorema $\ref{teorema_sobolev}$ temos que $\espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}} \subset L^{\infty}(\retacartesianocirculo, \real{2n})$. Então dado $X \in \espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}}$, existem constantes $A, B >0$ tais que $A\normaWp{X}\leq \norma{X}_{\infty} \leq B\normaWp{X}$, portanto, ambas as normas geram a mesma topologia. Como $L^{\infty}(\retacartesianocirculo, \real{2n})$ é um espaço de Banach, então pela inclusão e compatibilidade das normas, $\espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}}$ é um espaço de Banach.
	...
	Denote a coleção $\{ (\espacosobolev{\gamma^{*}D}, \exp_{\gamma}) \}_{\gamma \in \lacocontrateis}$ por $\mathcal{A}$.
	
	\begin{proposicao}
		O conjunto $\cilindrosLM$  dos caminhos de classe $C^{1}$ em $\lacocontrateis$ é uma variedade de Banach.
	\end{proposicao}
