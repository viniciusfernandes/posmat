...
\newcommand{\distribuicoes}{\mathcal{D'}(\Omega)}
...
\begin{document}
	...
	\chapter{Introdução à Geometria Simplética}
	...
	É possivel se mostrar que $\complexificacao{V}$ é um espaço vetorial sobre $\complexo{}$. Vejamos que esse espaço é fechado pela operação de adição e multiplicação por escalares. Dados $a+ib \in \complexo{}$ e $v,u \in \complexificacao{V}$, então $(a+ib )v+u = (av_{1}-bv_{2}, bv_{1}+ av_{2}) + (u_{1}, u_{2}) = (av_{1}-bv_{2} + u_{1}, bv_{1}+ av_{2} + u_{2}) \in \complexificacao{V}$. As demais propriedades dos aximomas de espaços vetoriais resultam imediatamente dessa.
	
	\begin{exemplo}
		Definindo $V = \reta$ teremos $\complexificacao{\reta} = \reta\oplus \reta$. Assim, a identificação $(a+ib)(x, y ) = (ax-by, bx+ay) \mapsto ax-by+i(bx+ay) \in \complexo{}$. Portanto temos o isomorfimo $\complexificacao{\reta} \ni (x,y)\mapsto x +iy\in \complexo{}$.
	\end{exemplo}
	...
	Notemos que os espaços vetoriais $V\oplus\{0\}$ e $\{0\}\oplus V$ são isomorfos a $V$, pelos exemplos anteriores. Com isso, a inclusão $i :V \hookrightarrow \complexificacao{V}$ definida por $i(v) = (v, 0)$ é chamado de mergulho padrão de $V$ em $\complexificacao{V}$.
	...
	\begin{teorema}
		...
		$\reta$-linear entre espaços vetoriais reais de dimensão finita. Então existe uma única aplicação linear $\complexificacao{\varphi}:\complexificacao{V} \to \complexificacao{W}$ tal que $i\circ \varphi = \complexificacao{\varphi} \circ i$.
	\end{teorema}
	\begin{prova}
		...
		$$
			...
			\\
			&=\lambda 	\complexificacao{\varphi} (\varphi v_{1},\varphi v_{2})+\complexificacao{\varphi} (\varphi u_{1},\varphi u_{2}).
			\end{aligned}
		$$
	...
	Vendo o corpo dos complexos como um espaço vetorial sobre um corpo $\reta$, o seguinte produto tensorial é equivalente a construção da complexificação feita anteriormente.
	
	\begin{definicao}
		Seja $V$ um n-espaço vetorial sobre real. Então o produto tensorial $\complexificacaotensorial{V}$ é a complexificação de $V$.
	\end{definicao}
	
	Pode-se mostrar que o complexificação $\complexificacaotensorial{V}$ é um espaço vetorial sobre os complexos com a operação de multiplicação por escalares dada por $z'(z\otimes_{\reta} \alpha) = z'z\otimes_{\reta} \alpha$, onde $\alpha \in V$ e $z,z' \in \complexo{}$. Com isso, se $B = \{e_{j}\}_{j=1}^{n}$ é uma base de $V$, então $1\otimes_{\reta}B$ é uma base de $\complexificacao{V}$. O teorema seguinte afirma que ambas as complexificações são equivalentes.
	
	\begin{teorema}\label{teorema_isomorfismo_complexificacao}
		Para cada $V$ um espaço vetorial real de dimensão finita existe um único isomorfismo $f_{V}: \complexificacao{V} \to \complexificacaotensorial{V}$ entre os espaços vetoriais complexos que realiza a comutação do diagrama
		$$
			\xymatrix{
				& V\ar[dl]_{i}\ar[rd]^{j} &
				\\
		...
		$$
		onde $i$ indica o mergulho padrão, e $j(v) = 1\otimes_{\reta} v$ e $f_{V}(v_{1},v_{2}) = 1\otimes_{\reta} v_{1} + i \otimes_{\reta} v_{2}$.
	\end{teorema}
	\begin{prova}
		...
		Para todo $v \in V$ temos que $(f_{V}\circ i )(v) = f_{V}(v, 0) = 1\otimes_{\reta} v = j(v)$. Portanto o diagrama é comutativo. \vermelho{(Não fiz a de unicidade do isomorfismo.)}
	\end{prova}
	
	O conjugado de cada $z=(a+ib) \in \complexo{}$ é definido por $\overline{z} = a-ib$. Analogamente, o conjugado de $z \otimes_{\reta}v \in \complexificacaotensorial{V}$ é definido por $\overline{z} \otimes_{\reta}v \in \complexificacaotensorial{V}$.
	...
	\begin{corolario}
		Seja $V = V_{1}\otimes_{\reta} \dots \otimes_{\reta}V_{k}$ o produto tensorial dos k espaços vetoriais reais. A complexificação tensorial é dada por $\complexificacao{V} \cong  \complexificacaotensorial{\produtotensorialreal{V}{k}}$. 
	\end{corolario}
	\begin{prova}
		É imediato que $\complexificacao{V} \cong \complexificacaotensorial{(\produtotensorialreal{V}{k})} \cong  \complexificacaotensorial{\produtotensorialreal{V}{k}}$.
	\end{prova}
	
	\begin{observacao}\label{observacao_complexificacao_tensores}
		Se $\complexificacao{V}$ é a complexificação do produto tensorial $V = \produtotensorialreal{V}{k}$, então a complexificação $\complexificacao{T} \in \complexificacao{V}$ do tensor $T \in V$ é dada por $\complexificacao{T} = \complexificacaoelemento{z}{T}$.
	\end{observacao}
	...
	Seja $(V, \omega) $ um 2n-espaço vetorial simplético. Na seção anterior vimos que a complexificação de $V$ é o produto tensorial $\complexificacaotensorial{V}$ e que a complexificação do produto tensorial $V\otimes_{\reta} V$ é $\complexificacaotensorial{V\otimes_{\reta} V}$. Sabe-se que uma aplicação bilinear anti-simétrica é um tensor de ordem 2. Com isso podemos afirmar que $\omega \in V\otimes_{\reta} V$ e, pela Observação $\ref{observacao_complexificacao_tensores}$ temos $\complexificacao{\omega} = \complexificacaoelemento{1}{\omega} \in \complexificacaotensorial{V \otimes_{\reta }V}$ é a complexificação da forma simplética. Vamos denotar $\Omega = \complexificacao{\omega}$.
	
	\begin{proposicao} Seja $(V, \omega)$ um 2n-espaço vetorial simplético sobre $\reta$. Então a complexificação $(\complexificacao{V}, \Omega)$ é um 2n-espaço vetorial simplético sobre $\complexo{}$.
	\end{proposicao}
	\begin{prova}
		Pelo Teorema $\ref{teorema_isomorfismo_complexificacao}$ temos que $\complexificacao{V}$ é um 2n-espaço vetorial complexo. Por construção $\Omega = \complexificacao{\omega}$ é uma aplicação bilinear anti-simétrica. Afirmo que $\Omega$ é não degenerada. De fato, supondo que $\Omega$ seja degenerada, então existe $0\neq v \in \complexificacao{V}$ tal que para todo $u \in \complexificacao{V}$ temos $\formaSimpleticaExtendida{v}{u} = 0$. Com isso, $0=\formaSimpleticaExtendida{v}{u} = (\complexificacaoelemento{1}{\omega})(v,u) = \formaSimpletica{v}{u}$, o que implica que $\omega$ é degenerada, contradizendo a hipótese de que $\omega$ é não-degenerado. Portanto $\Omega$ é não-degenenada e o par $(\complexificacao{V}, \Omega)$ é um 2n-espaço vetorial simplético sobre $\complexo{}$.
	\end{prova}
	...
	O seguinte lema afirma a injetividade do mapa de Floer $\diferencialmapafloerabrev$, o que implicará diretamente que $\diferencialmapafloer$ é um operador compacto. Aqui vamos demonstrar apenas particularização $p=2$. O caso geral em que $p\geq 2$ pode ser encontrado em $\cite{salamon_lecture}$ \vermelho{(Caso sobre tempo podemos fazer o caso $p>2$)}. 
	...
	Faremos o uso do seguinte lema, demonstrado em $\cite{audi_floer_homology}$.
	...
	\begin{teorema}
		O operador $\diferencialmapafloer: \espacosobolevcontradominio{\caminhosexponenciaisconectantesabrev}{\real{2n}}\to \espacoLpcontradominio{\caminhosexponenciaisconectantesabrev}{\real{2n}}$ é compacto e é um operador de Fredholm.
	\end{teorema}
	\begin{prova}
		Temos que $\caminhosexponenciaisconectantesabrev \subset \real{k}$ é limitado. Então pelo Teorema $\ref{teorema_sobolev}$ (teorema de Sobolev), temos que a inclusão $\espacosobolevcontradominio{\caminhosexponenciaisconectantesabrev}{\real{2n}} \hookrightarrow \espacoLpcontradominio{\caminhosexponenciaisconectantesabrev}{\real{2n}}$ é compacta. Temos que $\diferencialmapafloer$ é um operador linear, logo é contínuo. Pelo Lema $\ref{lema_diferencial_floer_bijecao}$ é uma bijeção, portanto é um homeomorfismo, então $\diferencialmapafloer(\espacosobolevcontradominio{\caminhosexponenciaisconectantesabrev}{\real{2n}}) \subset \espacoLpcontradominio{\caminhosexponenciaisconectantesabrev}{\real{2n}}$ é um compacto. Dado um limitado $K \subset \espacosobolevcontradominio{\caminhosexponenciaisconectantesabrev}{\real{2n}}$, então $\overline{\diferencialmapafloer(K)} \subset \diferencialmapafloer(\espacosobolevcontradominio{\caminhosexponenciaisconectantesabrev}{\real{2n}})$ é um compacto, pois todo subconjunto fechado de compacto é compacto. Portanto $\diferencialmapafloer$ é um operador compacto e limitado, pela Observação $\ref{observacao_operador_compacto_limitado}$.
		
		Supondo $\xi \in \espacosobolevcontradominio{\caminhosexponenciaisconectantesabrev}{\real{2n}}$ temos, pelo Lema $\ref{lema_desigualdade_inclusao_sobolev}$, que existe uma constate $c>0$ tal que $\normaWp{\xi}\leq c(\normaLp{\diferencialmapafloer \xi} +\normaLp{\xi})$. Onde podemos identificar pela inclusão $i(\xi) = \xi$. Como $\diferencialmapafloer$ é um operador limitado e a inclusão é um operador compacto, então pelo Lema $\ref{lema_desigualdade_operador_compacto}$ temos que $\diferencialmapafloer(\espacosobolevcontradominio{\caminhosexponenciaisconectantesabrev}{\real{2n}})$ é fechado e $dim (Ker(\diferencialmapafloer))<\infty$. \vermelho{(Falta demonstrar a codimesão finita)}
	\end{prova}
