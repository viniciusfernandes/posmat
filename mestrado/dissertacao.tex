...
\begin{document}
	...
	\section{Variedades Riemannianas - algumas definições}\label{secao_variedade_riemanniana}
	...
	\begin{lema}\label{lema_conjunto_limite_funcao_morse}
		Sejam $f\in \funcoesmorse{M}$ e $p \in M$. Então $\alpha(p)  = \{q\}$ e $\omega(p) = \{r\}$ onde $q, r \in \pontoscriticos{f}$.
	\end{lema}
	...
	\section{Álgebra Linear - alguns resultados}
	...
	\begin{teorema}
		(Propriedade de Fredholm de $\diferencialfloerabrev$) Suponha que $det(Id - \Psi^{\pm}(1)) \neq 0$. Então o operador $\diferencialfloerabrev: \espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}} \to \espacoLpcontradominio{\retacartesianocirculo}{\real{2n}}$ é um operador de Fredholm para $1<p<\infty$. Além disso, seu índice de Fredholm é dado por
		$$
		ind(\diferencialfloerabrev) = \mu(\Psi^{+}) - \mu(\Psi^{-}).
		$$
	\end{teorema}
	\begin{prova}
		\vermelho{	Como $\diferencialfloerabrev$ é bijetor e limitado, então pelo Teorema $\ref{teorema_aplicacao_aberta}$ temos que $\diferencialfloerabrev^{-1}$ é continuo. Pela Observação $\ref{observacao_teorema_aplicacao_aberta}$ existe uma constante $c>0$ tal que $B_{W^{1,p}}(0,c)\subset\diferencialfloerabrev^{-1}(B_{L^{p}}(0,1))$, isto é, $\normaWp{\xi} \leq k \normaLp{\diferencialfloerabrev\xi}$ para algum $k>0$.(Acho que não é necessario esse argumento.)}
