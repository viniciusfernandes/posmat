...
\begin{document}
	...
	Tome $\lambda_{1}\in \sigma(A)$ tal que $|\lambda_{1}|<1$ e o escreva como $\lambda_{1} = |\lambda_{1}|\exp(i\theta_{\lambda_{1}})$, para algum $\theta_{\lambda_{1}} \in [0,2\pi]$. Tome um outro auto-valor $\lambda_{2}\neq \lambda_{1}$ tal que $|\lambda_{2}|<1$. Se $|\lambda_{1}| < |\lambda_{2}|$ ou  $|\lambda_{1}|= |\lambda_{2}|$ com $\theta_{\lambda} \leq \theta_{\beta}$, diremos que $\lambda_{1} \leq_{o} \lambda_{2}$. Repetindo esse procedimento n-vezes teremos a sequência $\lambda_{1} \leq_{0} \dots \leq_{0}\lambda_{n} \leq_{0} \lambda_{n}^{-1} \leq_{0} \dots \leq_{0} \lambda_{1}^{-1}$.
	...
	\begin{lema}
		A aplicação $B:\complexificado\times \complexificado \to \reta$ definida por $B(v, u ) = \parteImaginaria{\formaSimpleticaExtendida{\overline{v}}{u}}$ é uma forma $\reta$-bilinear simétrica e não-degenerada. Além disso
		$$
	...
	\begin{definicao}
		Seja  $Q: \complexificado{V} \to \reta$ a forma quadrática definida por $Q(v) = B(v,v)$. Então $m(Q) = m_{+}(Q)-m_{-}(Q)$ é chamada a assinatura de $Q$, onde $m_{\pm}(Q)$ são as dimensões dos aiores  subespaços de $\complexificado{V}$ nos quais $Q$ é positiva-definida ou negativa-definida, respectivamente.
	\end{definicao}
	...
	\end{corolario}
	\begin{prova} A forma quadrática $Q$ é não-degenerada pois $B$ é não-degenerada.
		Seja $\{\textbf{e}, \textbf{f}\}$ a base simplética de $\complexificado{V}$. Afirmo que $m(Q) = 0$. De fato, tome $v = v_{(1)}\textbf{e}+v_{(2)}\textbf{f}\in \complexificado{V}$. Pela Proposição \ref{proposicao_forma_simpletica_vetor_conjugado} tem-se que $Q(v)=\parteImaginaria{\formaSimpleticaExtendida{\overline{v}}{v}} = 2\parteImaginaria{\produtointerno{\overline{v}_{(1)}}{v_{(2)}}}$. Seja $\complexificado{V}^{+} = \{v \in \complexificado{V}: v_{(1)}=v_{(2)}\}$. Note que $0 \in \complexificado{V}^{+}$ e que, dados $\lambda \in \complexo{}$ e $v, u \in \complexificado{V}^{+}$, tem-se que 
		$$
		\lambda v + u  = \lambda v_{(1)}e+v_{(1)}f + u_{(1)}e+u_{(1)}f = (\lambda v_{(1)} + u_{(1)})e+(\lambda v_{(1)} + u_{(1)})f \in \complexificado{V}^{+}.
		$$
		
		Portanto $\complexificado{V}^{+} $ é um subespaço vetorial de $\complexificado{V}$.  Além disso, $dim(\complexificado{V}^{+}) = n$. Com isso, $Q(v) = 2\parteImaginaria{\produtointerno{\overline{v}_{(1)}}{v_{(1)}}} = 2\norma{v_{(1)}}^{2} >0$, para todo $v \in \complementar{\complexificado{V}^{+}}{\{0\}}$. Analogamente, definindo $\complexificado{V}^{-} = \{v \in \complexificado{V}: v_{(1)} =- v_{(2)}\}$, tem-se que $Q(v) = 2\parteImaginaria{\produtointerno{\overline{v}_{(1)}}{-v_{(1)}}} = -2\norma{v_{(1)}}^{2} <0$, para todo $v \in \complementar{\complexificado{V}^{-}}{\{0\}}$. Como $\complexificado{V}^{+}\cap \complexificado{V}^{-} = \{0\}$ e $dim(\complexificado{V}^{+}) = dim(\complexificado{V}^{-}) = n$, então $\complexificado{V}=\complexificado{V}^{+}\oplus\complexificado{V}^{-}$ e ambos os subespaços são máximais com respeito a $Q$ ser positiva ou negativa definida. Logo, $m(Q) = m_{+}(Q) -m_{-}(Q) = n-n = 0$.
	\end{prova}
	
	Sejam $A \in \gruposimpletico{2n}$ e $\lambda\in \sigma(A)$ tal que $\lambda \in \complementar{\circulo}{\reta}$ \vermelho{(não entendi essa condição...pode ser que envolva a $\omega$-ortogonalidade. Esta no começo das propriedades de $\rho$ do Michelle)}. Realizando a decomposição $E_{\lambda} = E_{\lambda}^{+} \oplus E_{\lambda}^{-}$, onde $E_{\lambda}^{\pm} $ são subespaços vetoriais tais que $Q$ é positivo-definida, ou negativo-definida, respectivamente, tem-se que a multiplicidade de $\lambda$ é dada por $m(\lambda) = m_{+}(\lambda)+m_{-}(\lambda)$, onde  $m_{\pm}(\lambda) = dim_{\complexo{}}(E_{\lambda}^{\pm} )$ e as dimensões de $E_{\lambda}^{\pm}$ são iguais, pela maximalidade. Com isso, $m(\lambda) = 2dim(E_{\lambda}^{+}) \in 2\mathbb{N}$.
	
	\begin{lema}\label{lema_identidade_aplicacao_rho}
		Sejam $A \in \gruposimpletico{2n}$, $m_{0} = \sum_{\lambda \in \sigma(A)} m_{-}(\lambda)$  e $\sigma'(A)=\sigma(A)\cap \complementar{\circulo}{\real{-}}$ \vermelho{(acredito que aqui tiramos os auto valores negativos para seguir com a ordem $\leq_{o}$)}. Então
		$$
