...
\begin{document}
	...
	\begin{teorema}
		(Darboux) Toda 2n-variedade simplética e localmente simplectomorfa a $(\real{2n}, \omega_{0})$.
	\end{teorema}
	...
	Sejam $H:M \to \reta$ uma função Hamiltoniana autônoma e $x\in M$ um ponto crítico não-degenerado de $H$. Fixe $V$ uma vizinhança corrdenada de $x$ e  $(q_{1}\dots, q_{n}, p_{1}\dots, p_{n})$ as coordenadas locais de $V$. Então o diferencial $D_{x}\psi_{t}:\espacotangenteponto{x(0)}{M} \to \espacotangenteponto{x(t)}{M}$ é associado a matriz $2n\times 1$
	$$
	D_{x}\psi_{t} = 
	\left(
		\begin{array}{cc}
	...
	Note que $\hessianaponto{x}{H}$ é uma matriz simétrica. Segue da demonstração do Item \ref{item_assinatura_maslov} do Teorema \ref{teorema_indice_maslov} que se $\norma{\hessianaponto{x}{H}}<2\pi$, então 
	$$
