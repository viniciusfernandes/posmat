...
\vermelho{	No caso em que a função Hamiltoniana $H$ dependente do tempo se reduz a uma Hamiltoniana autônoma, tem-se que os pontos críticos de $H$ são as soluções constantes dos sistema Hamiltoniano, o que está feito no Exemplo \ref{exemplo_funcao_hamiltoniana}.
	...
	\begin{prova}
		...
		\begin{enumerate}
			...
			Como $\norma{S}<2\pi$, então pode-se escolher $0<|a_{j}|<2\pi$ e, sem perda de generalidade, reduzir a análise ao caso em que $|a_{j}|=\pi$ para $1\leq j\leq 2n$ para os seguintes casos:
	...
	\begin{teorema}\label{teorema_orbitas_conectantes_finitas}
		Sejam $\gamma^{-}, \gamma^{+} \in \pontoscriticos{\funcionalH}$. Então, $\energiafinitaMconectante$ é uma variedade de dimensão $\mu(\gamma^{-})-\mu(\gamma^{+})$ e possui finitos elementos.
	\end{teorema}
