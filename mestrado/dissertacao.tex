...
\begin{document}
	...
	\section{Exemplos}
	\begin{exemplo}
		(Grupo fundamental $\grupofundamental{\real{n}}$) $\real{n}$ é simplesmente conexo, então pela Proposição $\ref{proposicao_grupo_fundamental_simplesmente_conexo}$ temos que $\grupofundamental{\real{n}}$ é trivial.
	\end{exemplo}
	\begin{exemplo}
	...
	\begin{exemplo}
		(Complexo de Morse-Witten) Considere variedade 2-dimensional $M$ mergulhada em $\real{3}$ e representada pela figura \vermelho{(inserir o número da figura)}. Seja $F:\real{3} \to \reta$ definida por $F(x, y, z) = z$. Pode-se mostrar que $f=F|_{M} \in \funcoesmorsesmale{M}$,  $\pontoscriticos{f} = \{p,q,r,s\}$ e que os índices de Morse dos pontos críticos de $f$ são $\lambda_{q} = 0$, $\lambda_{p} = 1$ e $\lambda_{r}=\lambda_{s} = 2$. 
		...
		\begin{figure}
			\centering
			\textbf{Your title}\par
			\includegraphics[width=5cm,height=3cm]{imagem/variedade_instavel_estavel}
