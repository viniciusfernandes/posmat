...
\begin{document}
	...
	\begin{prova}
		\begin{enumerate}
			...
			\item Sejam $A \in \gruposimpletico{2n}$ e $\psi \in \mathcal{L}(\gruposimpletico{2n})$ tal que $\psi(0) = Id $ e $\psi(1) = A$ e o caminho contínuo $\gamma:[0,1] \to \gruposimpleticonaodegenerado{*}$ tal que $\psi(1) = \gamma(0)$ e $\gamma(1)= W^{\pm}$. Se $\psi(1) \in \gruposimpleticonaodegenerado{+}$, então $det(Id - \psi(1))>0$, então $\gamma(1) = W^{+}$ e $\rho(W^{+}) = (-1)^{n}$. Com isso $\mu(\psi) - n$ é par \vermelho{(não entendi esse argumento. No michelle e no salamn estão iguais.)}
			
			\item Afirmo que, $\exp(t\estruturacomplexa S) \in\gruposimpletico{2n}$. De fato, vamos mostrar que, dada uma matriz simétrica $S$, temos que $\exp(t\estruturacomplexa S)$ comuta com $\estruturacomplexa$, de forma equivalente, $\estruturacomplexa^{-1}\exp(t\estruturacomplexa S)\estruturacomplexa = \exp(t\estruturacomplexa S)$. Seja um inteiro $k\geq 0$. Então
			$$
			\begin{aligned}
			...
			o que implica em 
			$$
				\estruturacomplexa^{-1}\exp(t\estruturacomplexa S)\estruturacomplexa =\sum_{k=0}^{\infty}\frac{\estruturacomplexa^{-1}(t\estruturacomplexa S)^{k}\estruturacomplexa}{k!} = \sum_{k=0}^{\infty}\frac{(tS\estruturacomplexa )^{k}}{k!} = \exp(tS\estruturacomplexa).
			$$
			logo $(\estruturacomplexa S )^{t}= S^{t}\estruturacomplexa^{t} = -S\estruturacomplexa$ \vermelho{(para mim essa afirmação vale se tivermos uma matriz anti-simétrica, iso é, $S^{t} =-S$...e agora?????).}
			
			Sejam um caminho contínuo $P:[0,1]\to \matrizortogonal{2n}$ e $S(\alpha) = P^{t}(\alpha)SP(\alpha)$ tais que $P(0) = Id$ e $S(1)$ é uma matriz diagonal. Temos que $S(\alpha)^{t}$ é simétrica, pois $S(\alpha)^{t} = (P^{t}(\alpha)SP(\alpha))^{t} = P^{t}(\alpha)S^{t}P(\alpha) = P^{t}(\alpha)SP(\alpha) = S(\alpha)$ \vermelho{(novamente temos o problema da simetria aqui)}, logo $\Psi_{\alpha}(t)=\exp(t\estruturacomplexa S(\alpha)) \in \gruposimpletico{2n}$.
			...
			\right| \neq 0
			$$
			pois $\epsilon \notin \{0, 2\pi\}$ e $\exp(\epsilon\estruturacomplexa ) \in \gruposimpleticonaodegenerado{*}$. Com isso, dado $v \in \real{2n}$ temos $\exp(t\estruturacomplexa S)v = \exp(t\estruturacomplexa \epsilon)v$. Como $v$ é arbitrário, então $ \exp(t\estruturacomplexa \epsilon) \in \gruposimpleticonaodegenerado{*}$. Por fim, $\rho(\exp(t\estruturacomplexa S))$
