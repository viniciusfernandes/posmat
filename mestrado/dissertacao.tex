...
\begin{document}
	...
	\begin{prova}
		...
		\begin{enumerate}
			...
			\item \textbf{\textit{Deteminante:}} Sejam $A \in \mathcal{U}$ e $U \in \matrizunitaria{n}$ com a identificação pelo isomorfismo descrito no Capítulo \ref{capitulo_grupo_simpletico} dado por
			$$
			\begin{aligned}
			...
			Na Observação \ref{observacao_quociente_grupo_simpletico_contratil} foi mostrado que $\matrizSimpleticaOrtogonal$ é um retrato por deformação de $\gruposimpletico{2n}$, portanto $\grupofundamental{\gruposimpletico{2n}} \cong \grupofundamental{\matrizSimpleticaOrtogonal} \cong \grupofundamental{\matrizunitaria{n}}$. Mostrou-se que $\rho|_{\matrizSimpleticaOrtogonal} = det: \matrizSimpleticaOrtogonal\to \circulo$, o que implica que $\rho|_{\matrizSimpleticaOrtogonal*} = Det_{*}$, onde $Det_{*}:\grupofundamental{\matrizunitaria{n}} \to \grupofundamental{\circulo}$ é o isomorfismo dado pelo Lema \ref{lema_isomorfismo_grupo_fundamental_Un}. Então $\rho_{*}: \grupofundamental{\gruposimpletico{2n}} \to \grupofundamental{\circulo}$ é um isomorfismo.
	...
	\begin{prova}
		...
		Afirmo que $W(\complementar{\reta}{\{0,1\}}) \subset \gruposimpleticonaodegenerado{*}$. De fato, $det(Id - W(a)) = (1-a)(1-a^{-1})4^{n-1} \neq 0$, pois $a\neq 1$. De fato, tomando $v,u \in \complexificado{V}$ tais que $v=v_{(1)}\textbf{e}+ v_{(2)}\textbf{f}$ e $u=u_{(1)}\textbf{e}+ u_{(2)}\textbf{f}$, temos 
		$$
		\begin{aligned}	
	...
	Na seção anterior mostrou-se que $\gruposimpleticonaodegenerado{\pm}$ são as duas componentes conexas de $\gruposimpleticonaodegenerado{*}$, logo $\gruposimpletico{2n}=\gruposimpleticonaodegenerado{+}\cup\gruposimpleticonaodegenerado{-}\cup \Sigma$.
	
	Sejam $\caminhosfechadospontobase{Id}{\gruposimpletico{2n}}=\{\gamma\in \caminhossempontobase{\gruposimpletico{2n}}:\gamma(0)=\gamma(1)=Id \}$ e $\caminhospontobase{Id}^{\pm}(\gruposimpletico{2n})=\{\alpha\in \caminhossempontobase{\gruposimpletico{2n}}: \alpha(0) =Id,\; \alpha(1)\in \gruposimpleticonaodegenerado{*}\}$. Defina $\caminhosespeciais{\gruposimpletico{2n}}$ como sendo o conjunto de todos os caminhos $\gamma*\alpha:\intervalo\to\gruposimpletico{2n}$, onde $\gamma \in \caminhosfechadospontobase{Id}{\gruposimpletico{2n}}$ e $\alpha\in \caminhospontobase{Id}^{\pm}(\gruposimpletico{2n})$.
	...
	\begin{definicao}
		(Índice de Maslov) Sejam $exp:\real{} \to \circulo \subset \mathbb{C}$ a aplicação exponencial. Para cada caminho $\psi \in \caminhosespeciais{\gruposimpletico{2n}}$ existe um único levantamento $\alpha:[0,1] \to \real{}$ do caminho $\rho_{\psi} = \rho \circ \psi:\intervalo\to \circulo$ que faz o diagrama abaixo comutar
		$$
		\xymatrix{
			& & \real{}\ar[d]\ar[d]^{\text{exp}}
			\\
		...
		$$	
		O índice de Maslov é definido por $\mu(\psi)= 2deg(\rho_{\psi}) = (\alpha(0) - \alpha(1))/\pi$.
	\end{definicao}
	
	\begin{observacao}
		Seja $\psi:\intervalo\to \gruposimpletico{2n}$ um caminho tal que $\psi(0)=W^{-}$ e $\psi(1)=W^{+}$. Com isso, tem-se $\rho_{\psi}(0) =\rho(W^{-}) = (-1)^{n-1}$ e $\rho_{\psi}(1) =\rho(W^{+}) = (-1)^{n}$. Suponha $n\in 2\mathbb{N}$ (o mesmo vale para $n$ ímpar). Então $\rho_{\psi}(0) =-1$ e $\rho_{\psi}(1)=1 $ e a imagem de $\rho_{\psi}$ envolve ao menos a metade do cícurlo, isto é, $\alpha(1)-\alpha(0) = k\pi$, para algum $k\in \inteiros$. Por esse fato defini-se o índice de Maslov com sendo $2deg(\rho_{\psi})$.
	\end{observacao}
	...
	\begin{definicao}
		...
		$$	
		Definamos $\varDelta(\gamma) = (\alpha(0) - \alpha(1))/\pi$. \vermelho{Dada $A \in Sp(2n)^{*}$ e uma determinada $\gamma_{A}:[0,1] \to Sp(2n)^{*}$ com $\gamma_{A}(0) = A$ e $\gamma_{A}(1) = W^{\pm}$, então temos o número $\varDelta(\gamma_{A}) \in \real{}$ fixo.
	...
	\begin{teorema}
		Seja $\psi\in \caminhosespeciais{\gruposimpletico{2n}} $. Então:
		\begin{enumerate}
			\item $\mu(\psi) \in \inteiros$.
			\item Dois caminhos $\psi_{1}, \psi_{2}\in \caminhosespeciais{\gruposimpletico{2n}} $ com $\psi_{1}(0) = \psi_{2}(0)$ e $\psi_{1}(1) = \psi_{2}(1)$ são homotopicamente equivalentes se, e somente se, $\mu(\psi_{1}) = \mu(\psi_{2})$.
	...
	\begin{prova}
		\begin{enumerate}
			\item Como $\gruposimpletico{2n}$ é conexo por caminhos, então exite $\psi \in \caminhosespeciais{\gruposimpletico{2n}}$ tal que $\psi=\gamma*\alpha$, onde $\alpha(\intervalo) \subset \gruposimpleticonaodegenerado{\pm}$ e $\alpha(1) = W^{\pm}$. Como $\gruposimpleticonaodegenerado{\pm}$ são contráteis, por consequêcia do Corolário \ref{corolario_homomorfismo_trivial_grupos_simpletico}, então $\alpha$ é homotópicamente equivalente ao caminho constante, portanto $deg(\rho\circ\alpha)=0$. Além disso, da Definição \ref{definicao_aplicacao_grau}, tem-se que o grau da justaposição de caminhos é a soma dos seus graus. Com esse resultado e usando o fato de que $\rho\circ(\gamma*\alpha) = (\rho\circ\gamma)*(\rho\circ\alpha)$, tem-se
			$$
			\mu(\psi)=2deg(\rho\circ\psi)=2deg(\rho\circ(\gamma*\alpha))=2deg(\rho\circ\gamma)+2deg(\rho\circ\alpha) = 2deg(\rho\circ\gamma) \in \inteiros.
			$$
			
			\item Sejam $\psi_{1}, \psi_{2}\in \caminhosespeciais{\gruposimpletico{2n}}$. Da Proposição \ref{proposicao_grau_aplicacao} tem-se que $\rho\circ\psi_{1} \sim \rho\circ\psi_{2}$ se, e somente se $deg(\rho\circ\psi_{1} )=deg(\rho\circ\psi_{2} )$. Suponha que $\psi_{1}\sim \psi_{2}$. Como $\rho$ é contínua, então $\rho\circ$ o que implica que $\mu(\psi_{1})=\mu(\psi_{2})$. 
			
			Como $\alpha \sim \beta$, então $\classe{\alpha} = \classe{\beta} \in \grupofundamental{\gruposimpletico{2n}}$ e, pelo Teorema $\ref{teorema_aplicacao_rho}$, $\rho_{*}$ é um isomorfismo, com isso $\rho_{*}[\alpha] = \rho_{*}[\beta] \in \grupofundamental{\circulo}$. Pela Proposição $\ref{proposicao_gerador_grupo_fundamental_ciruclo}$ temos $deg(\alpha) = (\theta(1)-\theta(0))/2\pi = \varDelta(\beta)/2$, o que implica em $\varDelta(\alpha) = \varDelta(\beta)$. Portanto
			$$
			\begin{aligned}
			\mu(\alpha) 
