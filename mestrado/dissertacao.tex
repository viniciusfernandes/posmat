...
\begin{document}
	...
	Considerando $\solucoesperiodicascontrateis$ como sendo o conjunto das aplicações $\psi : \circulo\to M$ de classe $C^{\infty}$, podemos tratar as soluções do sistema hamiltoniano 1-periódicas como elementos desse conjunto. Note que $\solucoesperiodicascontrateis$ não é conexo pois dois caminhos não-homotópicos não podem estar na mesma componente conexa. Isso nos leva a definição:
	
	\begin{definicao}
		(Laços contráteis) O conjunto dos laços contráteis $\solucoesperiodicascontrateis$ é o conjunto de todas as aplicações $\psi:\circulo \to M$ de classe $C^{\infty}$ e contráteis a aplicação constante.
	\end{definicao}
	...
	\appendix
	\chapter{Estrutura de variedade de $\solucoesperiodicascontrateis$}\label{apendice_estrura_variedade_LM}
	?????????????????????
