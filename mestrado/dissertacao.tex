...
\alerta{	\begin{observacao} (Não sei onde colocar essa observação...!!!!)
	...
	Por simplicidade, denote $\caminhosexponenciaisconectantespadrao$ por $\caminhosexponenciaisconectantesabrev$. Seja $\fibradocaminhosexponenciais$ um fibrado vetorial sobre $\caminhosexponenciaisconectantespadrao$ cujas fibras em $u \in \caminhosexponenciaisconectantespadrao$ é $\espacoLp{\pullbackfibradotangenteM{u}}$.
	
	\begin{definicao}\label{definicao_operador_floer}
		(Operador de Floer) Sejam $x^{-}, x^{+}\in \pontoscriticos{\funcionalH}$. O operador de Floer é a seção do fibrado $\operadorFloer: \caminhosexponenciaisconectantesabrev \to \fibradocaminhosexponenciais$ definida por
		$$
		\operadorFloerParametro{u} = \bigparenteses{u,\operadorFloerDefinicao{u}}.
		$$
	\end{definicao}
	
	Daqui em diante adotaremos a identificação da seção $\operadorFloer$ como operador diferencial $\partial_{s}+J\partial_{t}-J\campohamiltonianoabrev$ (veja o Apêndice \ref{apendice_fibrados}).
	...
	Dado $\exp(w,Y) \in \caminhosexponenciaisconectantesabrev$, pode-se mostrar que, identificando a fibra $\espacoLp{\pullbackfibradotangenteM{u}}$ com $\espacoLp{\retacartesianocirculo;\real{2n}}$, tem-se que $\operadorFloerParametro{\exp(w,Y)}\in \espacoLp{\retacartesianocirculo;\real{2n}}$ (veja \cite{audi_floer_homology}) \alerta{(se a próxima seção permanecer não precisarei dessa referência)}. Logo, o operador de Floer esta bem-definido.
...
\aviso{Brezis pag 203}
	...
	\begin{lema}\label{lema_diferencial_floer_bijecao}
		Suponha que $S(s,t) = S(t)$ e que $\det(\Psi(1) - Id) \neq 0 $, onde $\Psi:[0,1] \to \gruposimpletico{2n}$ é solução do sistema $\dot{\Psi}(t)=\estruturacomplexa S(t)\Psi(t)$. Então 
		$$
		\diferencialfloerabrev : \espacosobolevretacirculo \to \espacoLpretacirculo
		$$
		é bijetor para $1<p<\infty$.
	\end{lema}
	\begin{prova}
