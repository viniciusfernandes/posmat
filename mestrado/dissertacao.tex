...
\begin{document}
	...
	\end{proposicao}
	\begin{prova}
		Como $M$ é uma $2n$-variedade compacta, então $M\times M$ é uma $4n$-variedade compacta. Sejam $\psi_{t}: M\to M$ a família de difeomorfismos gerados pelo fluxo do campo Hamiltoniano $\campohamiltonianoabrev$, $A =\{(x, x):x\in M\}$ o gráfico da aplicação identidade e $B=\{(x, \psi_{1}(x)) :x\in M \}$ o gráfico do difeomorfismo $\psi_{1}$. Para todo $x \in M$ tem-se os espaços tangentes 
		$$\espacotangenteponto{(x,x)}{A} = \{(Y,Y):Y\in \espacotangenteponto{x}{M}\}\;\; \text{e}\;\; \espacotangenteponto{(x,x)}{B} = \{(Y,D_{x}\psi_{1}(Y)):Y\in \espacotangenteponto{x}{M}\}.
		$$
		Seja $S_{H}$ o conjunto das soluções $1-$periódica do sistema Hamiltoniano. Se $x\in S_{H}$, então $\psi_{1}(x(t)) = x(t)$ para todo $t\in \circulo$, e portanto $x(t)\in A\cap B$. Seja $S=\{(x(0), x(0)):x\in S_{H} \}$. Note que $S\subset A\cap B$. Além disso, $\espacotangenteponto{(x(0), x(0))}{S}=\{(Y,D_{x(0)}\psi_{1}(Y)):Y\in \espacotangenteponto{x}{M}\}$, onde $D_{x(0)}\psi_{1}(Y)\neq Y$, pois $x\in S_{H}$ é não-degenerada. Como 
		$$
		\espacotangenteponto{(x(0), x(0))}{S} \subset \espacotangenteponto{(x(0), x(0))}{A} \cap \espacotangenteponto{(x(0), x(0))}{B},
		$$
		então $D_{x(0)}\psi_{1}(Y) = Y$ se, e somente se, $Y=0$. Com isso, $\espacotangenteponto{(x(0), x(0))}{S}=\{(0,0)\}$ para todo $x \in S_{H}$. Logo $S$ é uma subvariedade de dimensão zero, e portanto é um conjunto de pontos isolados. A compacidade de $M\times M$ implica que $S$ é finito. Como os elementos de $S$ são distintos, então $S_{H}$ é finito.
	\end{prova}
	...
	\begin{teorema}\label{teorema_sard_smale}
		(Corolário de Sard-Smale) Como $\pi$ é uma aplicação de Fredholm, então o conjunto dos valores regulares de seus valores regulares é uma intersecção enumerável de conjuntos densos.
	\end{teorema}
	...
	Como $\diferencialfloerponto{u}$ é sobrejetor para todo $u \in \energiafinitaMHamiltoniana{H+h}$, então o operador de Floer $\operadorFloer$ satisfaz a propriedade de transversalidade para todo $u \in \energiafinitaMHamiltoniana{H+h}$.
	...
	\begin{prova} Tome $h \in \perturbacaoHamiltoniana{H}$. Pelo teorema da pré-imagem de valores regulares, tem-se que $\pi^{-1}(h)$ é uma variedade e sua dimensão é dada por
			$$
			\begin{aligned}
			\dim(\ker(D_{(u, H+h)}\pi)) &= \dim(\ker(\diferencialfloerponto{u}))\alerta{(falta mostrar o cokernel)}
			\\
	...
	Seja $(M,\omega)$ uma 2n-variedade simplética e $H: M\times \reta\to \reta$ uma função Hamiltoniana dependente do tempo não degenerada e uma estrutura complexa $J$. Fazendo uma perturbação dessa Hamiltoniana, caso seja necessário, pode-se supor que o par $(H,J)$ é regular. Suponha que $M$ satisfaças as condições de Asfericidade (veja em \ref{definicao_condicao_aesfericidade}) e defina
	$$
