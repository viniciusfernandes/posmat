...
\begin{document}
	...
	\begin{definicao}
		...
		$$
		para $1\leq j \leq n$ e $(q_{1}(t), \dots, q_{n}(t), p_{1}(t), \dots, p_{1}(t)) \in M$. Se a função hamiltoniana não depende do tempo, então diz-se que ela é uma função Hamiltoniana autônoma, isto é, $H(x(t), t) = H(x(t))$. 
	\end{definicao}
	
	\begin{observacao}
		Se $H$ é uma função Hamiltoniana autônoma, então $dH/dt = 0$, o que caracteriza um sistema conservativo.
	\end{observacao}
	...
	\end{proposicao}
	\begin{prova}
		Se $x\in \pontoscriticos{H}$, então pela não-degeneracidade da forma simpética tem-se que, $0=dH_{x}(Y)=\omega_{x}(\campohamiltonianoabrev, Y)$ para todo $Y\in T_{x}M$. Logo, $\campohamiltoniano{x} =0$. Por outro lado, se $\campohamiltoniano{x} =0$, então $\omega_{x}(\campohamiltonianoabrev, Y)=0$ para todo $Y\in T_{x}M$. Logo $dH_{x}=0$ e $x\in \pontoscriticos{H}$.
	\end{prova}
	
	O sistema de equaçoes diferenciais abaixo é chamado de Hamiltoniano, 
	$$
	\derivada{x(t)}{t}=\campohamiltoniano{x(t), t},
	$$
	e suas soluções determinam uma família de difeomorfismos $\psi_{t}:M\to M$ que satisfazem a as relações 
	$$
	...
	\begin{definicao}(Soluções $\tau$-periódicas não-degeneradas)
		Uma solução $\tau$-periódica de um sistema Hamiltoniano $x: \reta\to M$ é não-degenerada se $\det(Id - D_{x(0)}\psi_{\tau})\neq 0$, onde $D_{x(0)}\psi_{\tau}:T_{x(0)}M \to T_{x(0)}M$.
	\end{definicao}
	...
	\begin{exemplo}
		(Função Hamiltoniana) Com base no exemplo anterior, defina $H:S^{2}\times \reta\to \reta$ por $H(q,p) = q^{2}/2+p^{2}/2$. Pelas equações de Hamilton, pode-se verificar que 
		$$
		x(t)=(p_{0}sent(t)+q_{0}\cos(t), p_{0}\cos(t)-q_{0}sent(t))
		$$ 
		é a solução do sistema Hamiltoniano para a condição inicial $x(0) = (q_{0}, p_{0})$. Com isso, para um ponto arbitrário $(q,p) \in S^{2}$, o fluxo Hamiltoniano é
		$$
		\psi_{t}(q,p)=(psint(t)+q\cos(t), p\cos(t)-qsint(t)),
		$$
		e o campo Hamiltoniano é 
		$$
		\campohamiltonianoabrev(t) = (p\cos(t)-qsint(t))\partial_{q}-(psint(t)+q\cos(t))\partial_{p}.
		$$
		...
		Note que o solução $x$ para a condição inicial  $x(0) = (q_{0}, p_{0})$ é $2\pi$-periódica, pois $x(0)\neq x(2\pi)$. Contudo, $x$ é uma solução degenerada pois $\det(D_{x(0)}\psi_{2\pi}-Id) = 0$.
	\end{exemplo}
	...
	\vermelho{(Alterar para de p => 2 nos índices dos espaços normados pois estamos fazendo um caso particular.)}
	...
	\begin{prova}
		...
		$$ 
		Como $\eta$ é arbitrário, então $Q\circ \diferencialfloerabrev = Id$, e com isso, $\eta \in \espacosobolevgeral{2}{\retacartesianocirculo; \real{2n}}$.
	\end{prova}
	...
	Seja 
	$
	\orbitasConectantesZ = \{ (u, H+h):h\in \perturbacaoHamiltoniana{H} \;\;\text{e}\;\; u \in  \energiafinitaM(\gamma^{+}, \gamma^{-}, J, H)\}
	...
	\begin{proposicao}
		...
		$$
		\begin{aligned}
			\Gamma:\espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}}\times \perturbacaoHamiltoniana{H} 
			&\to \espacoLpcontradominio{\retacartesianocirculo}{\real{2n}}
			\\
			(Y, h) 
			&\mapsto \diferencialfloer^{H+h}_{u}(Y) + \nabla_{u}h
		\end{aligned}
		$$
