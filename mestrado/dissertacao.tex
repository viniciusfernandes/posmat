...
\begin{document}
	...
	\end{proposicao}
	\begin{prova}
		Seja $\psi:M\times \reta\to M$ o fluxo do campo Hamiltoniano. Se $x\in \pontoscriticos{H}$ é uma solução 1-periódica do sistema Hamiltoniano, então pela Proposição \ref{proposicao_exponencial_hessiana} tem-se que $D_{x}\psi_{1}=e^{-\estruturacomplexa \hessianaponto{x}{H}}$ em uma vizinhança coordenada $V$ de $x$. Seja $v\in \espacotangenteponto{x}{M}$ um auto-vetor de $D_{x}\psi_{1}$, então 
		$$
		\lambda v=
		D_{x}\psi_{1}v=e^{-\estruturacomplexa \hessianaponto{x}{H}}v.
		$$
		Escrevendo $\lambda v=e^{\alpha Id}v$ para algum $\alpha \in \complexo{}$, segue que 
		$$
		e^{-(\estruturacomplexa \hessianaponto{x}{H} + \alpha)}v=v,
		$$
		o que implica que $\hessianaponto{x}{H} = \alpha \estruturacomplexa$. Como $1\notin \espectrooperador{D_{x}\psi_{1}}$, então $\lambda \neq 1$, e portanto $\alpha\neq 0$. Com isso, $\hessianaponto{x}{H} =\alpha\estruturacomplexa \neq 0$ e $\hessianaponto{x}{H}$ é não-degenerada.
	\end{prova}
	...
	\begin{prova}
		...
		$$
		Seja $S_{H}$ o conjunto das soluções $1-$periódica do sistema Hamiltoniano. Se $x\in S_{H}$, então $\psi_{1}(x(t)) = x(t)$ para todo $t\in \circulo$, e portanto $(x(t), x(t))\in A\cap B$. Seja $S=\{(x(0), x(0)):x\in S_{H} \}$. Note que $S\subset A\cap B$. Com isso, $\espacotangenteponto{(x(0), x(0))}{S}=\{(Y,D_{x(0)}\psi_{1}(Y)):Y\in \espacotangenteponto{x}{M}\}$, onde $D_{x(0)}\psi_{1}(Y)\neq Y$, pois $x\in S_{H}$ é não-degenerada. Como 
		$$
	...
	\dot{x}(t) = X_{H}(x(t), t),
	$$
	busca-se um limitante inferior para o número de soluções 1-periódicas (isto é, $x(t+1)=x(t)$) e contráteis, onde $\campohamiltonianoabrev$ é o único campo vetorial tal que $\formaSimpletica{\campohamiltonianoabrev}{\xi} = -dH(\xi)$ para todo $\xi \in \campossuaves{M}$. Isto é equivalente a analisar as aplicações $x : \circulo\to M$ que satisfazem esse sistema. O conjunto das aplicações $x:\circulo \to M$ de classe $C^{\infty}$ e contráteis é denotado por $\lacocontrateis$.
	...
	O teorema seguinte garante que, assim como no caso Morse, as trajetórias com energia finita são exatamente as que conectam dois pontos críticos do funcional de ação.
	...
	\begin{teorema}\label{teorema_aplicacao_rho}
		...
		\begin{enumerate}
			...
			\right).
			$$
			Além disso, a aplicação induzida $\rho_{*}: \grupofundamental{\gruposimpletico{2n;\reta}} \to \grupofundamental{\circulo}$ é um isomorfismo.
