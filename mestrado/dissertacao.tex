...
\begin{document}
	...
	$$
	A aplicação $\psi: M\times \reta\to M$ acima é chamada de fluxo Hamiltoniano.
	
	Uma solução $\tau$-periódica do sistema Hamiltoniano é um caminho $x:\reta\to M$ tal que $x(t) = x(t+\tau)$ para todo $t\in \reta$. Desse modo, pode-se verificar que $\psi_{\tau}(x(0)) = x(\tau) = x(0)$. Portanto, $x(0)\in M$ é um ponto fixo do difeomorfismo $\psi_{\tau}:M\to M$. Supondo que a função Hamiltoniana $H$ também é $\tau$-periódica, então os difeomorfismos tem como pontos fixos, no mínimo, o total de soluções $\tau$-perídicas do sistema Hamiltoniano.
	
	\begin{definicao}(Soluções $\tau$-periódicas não-degeneradas)
		Uma solução $\tau$-periódica de um sistema Hamiltoniano $x: \reta\to M$ é não-degenerada se $\det(D_{x(0)}\psi_{\tau} - Id)\neq 0$, onde $D_{x(0)}\psi_{\tau}:T_{x(0)}M \to T_{x(0)}M$.
	\end{definicao}
	...
	A conjectura de Arnold tem como hipótese uma hamiltoniana dependente do tempo $H: \retacartesianovariedade \to \reta$ e, para o sistema Hamiltoniano,
	$$
	\derivada{\psi}{t}(t) = X_{H}(\psi(t), t),
	$$
	busca-se as soluções 1-periódicas (isto é, $\psi(t+1)=\psi(t)$) e contráteis, onde $\campohamiltonianoabrev$ é o único campo vetorial tal que $\formaSimpletica{\campohamiltonianoabrev}{\xi} = -dH(\xi)$ para todo $\xi \in \campossuaves{M}$. Isto é equivalente a analisar as aplicações $\psi : \circulo\to M$ que satisfazem esse sistema.
	...
	No Apêndice \ref{apendice_variedades_banach} mostra-se que $\lacocontrateis$, munido da $\cktopologia{\infty}$, é uma variedade conexa por caminhos, mais especificamente uma variedade de Banach. Com isso, dado $\gamma\in \lacocontrateis$ o espaço tangente $\espacotangenteponto{\gamma}{\lacocontrateis}$ está bem-definido.

	Seja $\gamma:\circulo\to M$ um caminho contínuo, então o pullback $\gamma^{*}TM$ é o conjunto $\{(\gamma(t), Y(\gamma(t)): Y\in \campossuaves{M} \; \text{e}\; t \in \reta \}$. é o $2n$-fibrado vetorial onde as fibras são $\espacotangenteponto{\gamma(t)}{M}$. Dado $(x, Y(x)) \in TM$, então 
	
	isto é, 
	
	e com isso, seus elementos são as restrições dos campos vetoriais $X \in \campossuaves{M}$ ao caminho $\gamma$. Essa descrição resulta na identificação 
	$$
	\espacotangenteponto{\gamma}{\lacocontrateis} \ni X \Rightarrow  X|_{\gamma} \in \pullbackfibradotangente{\gamma}{M}.
	$$
	
	Tome $\gamma \in \lacocontrateis$ e considere o caminho $u : \reta \to \lacocontrateis$ solução do sistema $u'(0)=X $ e $u(0) = \gamma$. Com isso, $u(s)\in \lacocontrateis$ para todo $s\in \reta$, isto é, $u(s):\circulo\to M$, onde $u(0,z) = \gamma(z)$ para todo $z\in \circulo$. Portanto
	$$
	\derivadaparcial{u}{s}(s,z)|_{s=0} \in \espacotangenteponto{\gamma(z)}{M}.
	$$
	...
	\begin{observacao}
		A primeira integral na Definição \ref{definicao_funcional_hamiltoniano} do funcional de ação depende explicitamente da extensão de $\gamma$ escolhida. Supondo que $v : D^{2}\to M$ é outra extensão de $\gamma$, então
		$$
	...
	Como $\omega$  é uma forma simplética não-degenerada, então deve-se ter $\dot{\gamma}(t) - \campohamiltonianoabrev(\gamma, t)=0$. Portanto, um dado $\gamma \in \lacocontrateis$ é um ponto crítico do funcional de ação $\funcionalH$ se, e somente se, $\gamma$ é uma solução 1-periódica e contrátil das equações de Hamilton.
	...
	O lema a seguir caracterisza o conjunto dos pontos críticos do funcional de ação e sua demonstração esta em $\cite{audi_floer_homology}$. 
	\begin{lema}
	...
	\begin{teorema}\label{teorema_indice_maslov}
		...
		\begin{enumerate}
			...
			\item \textbf{(Naturalidade)} Se $\phi\in \caminhossempontobase{\gruposimpletico{2n}}$, então o caminho $\phi\psi\phi^{-1}: \intervalo \to \gruposimpletico{2n}$ definido por $(\phi\psi\phi^{-1})(t) = \phi(t)\psi(t)\phi^{-1}(t)$ é um elemento de $\caminhosespeciais{\gruposimpletico{2n}}$ e $\mu(\phi\psi\phi^{-1}) = \mu(\psi)$.
			
			\item \textbf{(Homotopia)} \label{homotopia_caminhos_teorema_indice_maslov} Dois caminhos $\psi_{1}, \psi_{2}\in \dominioMaslov $ com $\psi_{1}(0) = \psi_{2}(0)$ e $\psi_{1}(1) = \psi_{2}(1)$ são homotopicamente equivalentes se, e somente se, $\mu(\psi_{1}) = \mu(\psi_{2})$.
	...
	\begin{prova}
		...
		\begin{enumerate}
			...
			\begin{figure}[!h]
				\centering
				\includegraphics[width=10cm,height=4cm]{imagem/caminho_naturalidade.pdf}
	...
	\begin{definicao}
		(O diferencial do operador de Floer) Seja $\operadorFloer: \caminhosexponenciaisconectantespadrao \to T\caminhosexponenciaisconectantespadrao $ o operador de Floer. Então o diferencial $\diferencialfloer_{u}: \espacosobolev{\pullbackfibradotangenteM{u}}\to \espacoLp{\pullbackfibradotangenteM{u}}$ é dado por
		$$
		\diferencialfloer_{u}(\xi)= \Big( \derivadaparcial{}{s} + J(u)\derivadaparcial {}{t}\Big)(\xi)+ \Big(dJ_{u}(.)\bigparenteses{\derivadaparcial{u}{t} - X_{H}(u)} - J(u) \diferencialhamiltoniano{u}\Big)(\xi).
		$$
	\end{definicao}
	
	\begin{observacao}
		Na próxima seção é demonstrado que $\diferencialfloer_{u}(\espacosobolev{\pullbackfibradotangenteM{u}})\subset \espacoLp{\pullbackfibradotangenteM{u}}$, e portanto o diferencial de $\operadorFloer$  está bem definido.
	\end{observacao}
	...
	$$
	D\sigma_{(u,h)}(Y,\eta) = \diferencialfloer_{u}(Y)+\nabla_{u}\eta.
	$$
	...
	\end{aligned}
	$$
	que por definição é uma aplicação suave. Além disso, $\ker(D\pi_{(u, H+h)})=\diferencialfloer_{u}(\espacosobolev{\pullbackfibradotangenteM{u}})$. De fato, como a aplicação $\Gamma$ definida na Proposição \ref{proposicao_gamma_sobrejetora} é uma sobrejeção, então todo $X\in \espacoLp{\pullbackfibradotangenteM{u}}$ pode ser escrito como $X=\diferencialfloer^{\vermelho{H+h}}_{u}(Y) + \nabla_{u}h$, para algum $(Y, h )\in \espacosobolev{\pullbackfibradotangenteM{u}}\times \perturbacaoHamiltoniana{H} $.
	
	Supondo $D\pi_{(u,H+h)}(Y,\eta) = 0$, então $\eta=0$, o que implica em 
	
	
	existe $(Y, \eta) \in \espacosobolev{\pullbackfibradotangenteM{u}}\times \perturbacaoHamiltoniana{H}$ tal que $\Gamma(Y, \eta) =  \diferencialfloer^{\vermelho{H+h}}_{u}(Y) + \nabla_{u}\eta = 0$, o que implica em $\diferencialfloer^{\vermelho{H+h}}_{u}(Y) =-\nabla_{u}\eta$. Como $(u,H+h) \in \orbitasConectantesZ$ é uma solução da equação de Floer para a Hamiltoniana perturbada $H+h$, então 
	$$
		D\pi_{(u,H+h)}(Y,\eta) = \eta = -\diferencialfloer^{\vermelho{H+h}}_{u}(Y) = 0.
	$$
	Portanto, $\diferencialfloer_{u}(\espacosobolev{\pullbackfibradotangenteM{u}})\subset \ker(D\pi_{(u, H+h)})$. Por outro lado, se $(Y,\eta) \in \ker(D\pi_{(u, H+h)})$, então 
