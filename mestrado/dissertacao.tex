...
\begin{document}
	...
	Vamos mostrar que $\rho$ pode ser extendida contínuamente para $\gruposimpletico{2n}$.
		...
		\end{lema}
		\begin{prova}
		A aplicação $B$ é bilinear pois é a parte imaginária da aplicação bilinear $\omega$. Sejam $v' = (v_{1}, - v_{2}), u' = (u_{1}, - u_{2}) \in \real{2n}$. Pela complexificação temos $v'\mapsto \overline{v} = v_{1}-iv_{2}$ e, analogamente, $u'\mapsto \overline{u}$. Por definição temos $\formaSimpleticaExtendida{\overline{v}}{u} = \formaSimpleticaPadrao{v'}{u} -ic(v',u)$. Vejamos que $c(v',u) = v_{1}u_{1} - v_{2}u_{2} = u_{1}v_{1} - u_{2}v_{2} = c(u',v)$. Afirmo que $B$ é simétrica. De fato,
		...
		$$
			...
			\end{aligned}
		$$
		Como $\omega_{0}$ é não degenerada e $\formaSimpleticaPadrao{v'}{u} = \parteReal{\formaSimpleticaExtendida{\overline{v}}{u}} = 0$, então $v = 0$ e $B$ é não-degenerada.
