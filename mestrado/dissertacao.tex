...
\begin{document}
	...
	$$
	\funcaocond{(f*g)(t)}{f(2t)}{0\leq t \leq 1/2}{g(2t-1)}{1/2 \leq t \leq 1}.
	$$
	...
	\begin{definicao}
		(Grupo Fundamental) O grupo fundamental é o grupo abeliano $\grupofundamentalpontobase{X}{p} = (\caminhospontobase{p}/\sim, .)$ munido do produto $.:\grupofundamentalpontobase{X}{p}  \times \grupofundamentalpontobase{X}{p}  \to \grupofundamentalpontobase{X}{p}$ tal que  $\classe{\gamma}.\classe{\gamma'} = \classe{\gamma*\gamma'}$.
	\end{definicao}
	
	Sejam $\gamma, \gamma' \in \caminhospontobase{p}$ e suponha que $\gamma':\circulo\to X$ seja um caminho constante, isto é, $\gamma'(t) =p$ para todo $t \in \circulo$. Pela definição de produto temos $(\gamma*\gamma')(t) = \gamma(2t)$ para $0\leq t \leq 1/2$ e  $(\gamma*\gamma')(t) = \gamma'(2t-1) = p$ para $1/2 \leq t \leq 1$, logo $(\gamma*\gamma')(t) = \gamma(t)$ para todo $t \in \circulo$. Com isso, podemos afirmar que $\classe{\gamma}.\classe{\gamma'} = \classe{\gamma*\gamma'} = \classe{\gamma}$, portanto $\classe{\gamma'}$ é o elemento neutro $Id_{p}$ do grupo $\grupofundamentalpontobase{X}{p}$. Note que aqui é necessário indexar o elemento neutro ao ponto base, bem como foi definido o seu grupo fundamental.
	...
	\subsection{Homomorfismos induzidos}
		...
		\subsection{Exemplo - $\grupofundamental{T^{2}}$}
		Seja $\varphi:U\to T^{2}$ uma parametrização do 2-toro dada por $\varphi(\theta,\phi) = ((a + bcos(\phi))cos(\theta), (a + bcos(\phi))sin(\theta), bsin(\phi))$,
		onde $\theta, \phi\in [0,2\pi)$.
		
		Definindo os caminhos $\gamma, \gamma' \in \caminhospontobase{(0,0)}{T^{2}}$ tais que $\gamma(\theta) = \varphi(\theta,0)= ((a + b)cos(\theta), (a + b)sin(\theta), 0)$ e $\gamma'(\phi) = 	\varphi(0,\phi) = ((a + bcos(\phi)), 0, bsin(\phi))$. Note que $\gamma$ e $\gamma'$ são os círculos maiores do tóro.
