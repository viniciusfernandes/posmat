...
\begin{document}
	...
	No desenvolvimento desse capítulo utilizaremos a identificação $\varphi:\real{}/[0,1] \to \circulo$, e assim, trabalharemos com os caminhos, $\gamma\circ\varphi : \real{}/[0,1] \to X$ onde $(\gamma\circ\varphi)(0) = p$. Por um abuso de notação escreveremos $\caminhospontobasegeral{p}{X} = \{\gamma :\circulo \to X : \gamma(0) =p\}$.
	...
	\subsection{Homomorfismos induzidos}
		...
		\begin{proposicao}\label{proposicao_grau_aplicacao}
			(O grau de uma caminhos fechado) Seja $\caminhosfechadoscirculo{0}{1}$ o conjunto de todos os caminhos contínuos e fechados em $S^{1}$, então a aplicação $deg:\caminhosfechadoscirculo{0}{1} \to \mathbb{Z}$ onde $deg(\gamma) = (\alpha(1)-\alpha(0))/2\pi$ esta bem-definida e $\alpha:[0,1] \to \real{}$ é o levantamento de $\gamma$ da Proposição $\ref{proposicao_levantamento_curvas}$. Além disso
			\begin{enumerate}
				\item Se $\gamma, \beta$ possuem o mesmo ponto base, isto é, $\gamma(0)=\beta(0 )=p$ para um dado $p \in \circulo$, então $deg(\gamma.\beta)=deg(\gamma)+deg(\beta)$.
				\item Tem-se que $\gamma\sim \beta$ se, e somente se, $deg(\gamma)=deg(\beta)$
			\end{enumerate}
			
			
			
			
			, se $\gamma$ e $\beta$ são caminhos homotópicos, então $deg(\gamma)=deg(\beta)$ e para o produto $\gamma.\beta \in \caminhosfechadoscirculo{0}{1}$ temos $deg(\gamma.\beta) = deg(\gamma)+deg(\beta)$.
		\end{proposicao}
		\begin{prova}
			\begin{enumerate}
				\item Seja $p \in \circulo$ um ponto base para $\gamma$ e $\beta$ tal que $\gamma(1)= \beta(0) = p$. Suponha que $\alpha$ e $\lambda$ sejam os levatamentos de $\gamma$ e $\beta$, então o vínculo do ponto base implica que $\alpha(1) = \lambda(0)$. Pelo produto entre caminhos temos $(\gamma*\beta)(t) = e^{i\alpha(2t)}$ para $0\leq t\leq 1/2$ e  $(\gamma*\beta)(t) = e^{i\lambda(2t - 1)}$ para $1/2\leq t\leq 1$. Logo 
				$$
				\begin{aligned}
				...
				\end{aligned}
				$$  
				\item Suponha que $h:\circulo \times [0,1]\to S^{1}$ seja uma homotopia entre $\gamma$ e $\beta$ tal que $h(p,0) = \gamma(\lambda)$ e $h(p,1) = \beta(\lambda)$. Com isso, para cada $t \in [0,1]$ fixo temos $h_{t} \in \caminhosfechadoscirculo{0}{1}$. Pela Proposição $\ref{proposicao_levantamento_curvas}$ o caminho fechado $h_{t}$ possui um único levantamento $\alpha_{t}$ tal que $(\alpha_{t}(1)-\alpha_{t}(0))/2\pi = q \in \mathbb{Z}$, portanto $deg(h_{t})=q$ é constante, logo $deg(\gamma)=deg(h_{0})=deg(h_{1})=deg(\beta)$, isto é, caminhos homotópicos possuem o mesmo grau. 
			\end{enumerate}
		\end{prova}
		\begin{prova}
