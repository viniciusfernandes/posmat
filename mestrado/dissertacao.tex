...
\begin{document}
	...
	A relação entre a topologia de $M$ e os pontos críticos de uma função de Morse $f:M \to \reta$, originalmente foi data em termos de desigualdades, chamadas desigualdades de Morse.
	...
	\begin{lema}\label{lema-caracterizacao_espectro_semelhante}
	...
	Seja $Det:\caminhossempontobase{\matrizunitaria{n}} \to \circulo$ dada por $Det(\gamma)(t) = \det(\gamma(t))$. Essa aplicação esta bem-definida pois $\det(\matrizunitaria{n}) \subseteq \circulo$, pela Observação \ref{observacao_determinante_matriz_unitaria}. Note que, para um dado $\gamma \in \caminhossempontobase{\matrizunitaria{n}}$, tem-se que $Det(\gamma) \in \caminhossempontobase{\circulo}$. Tome $\gamma, \beta \in\caminhospontobasegeral{p}{\matrizunitaria{n}} $ com $\gamma(0)=\beta(0)$ e $\gamma(1)=\beta(1)$, e defina $h : \intervalo \times \intervalo \to \matrizunitaria{n}$ como sendo uma homotopia entre $\gamma$ e $\beta$ tal que $h(t,0) = \gamma(t)$ e $h(t,1)=\beta(t)$.
	
	Definindo a função contínua $H = Det \circ h:\intervalo \times \intervalo \to \circulo$ por $H(t,s) = Det(h(t,s))$, temos que $H(t,0) = Det(h(t,0)) = Det(\gamma(t)) = Det(\gamma)(t)$ e $H(t,1) = Det(h(t,1)) = Det(\beta(t)) = Det(\beta)(t)$. Logo $H$ é uma homotopia entre $Det(\gamma)$ e $Det(\beta)$. Portanto, se $\gamma \sim \beta$ então $Det(\gamma) \sim Det(\beta)$.
	
	Além disso, temos que a justaposição dos caminhos $Det(\gamma),Det(\beta) \in \caminhossempontobase{\circulo}$ é 
	$$
	...
	\end{lema}
	\begin{prova}
		Seja $Det_{*}: \grupofundamental{\matrizunitaria{n}} \to \grupofundamental{\circulo}$ a aplicação definida por $Det_{*}(\classe{\gamma}) = \classe{Det(\gamma)}$, onde $Det:\caminhossempontobase{\matrizunitaria{n}} \to \circulo$, foi definida anteriormente. Afirmo que $Det_{*}$ é um homomorfismo. De fato, se $\classe{\gamma}, \classe{\beta} \in \grupofundamental{\matrizunitaria{n}}$, então $Det_{*}(\classe{\gamma}.\classe{\beta}) = Det_{*}(\classe{\gamma *\beta}) =\classe{Det(\gamma *\beta)} = \classe{Det(\gamma) *Det(\beta)} = Det_{*}(\classe{\gamma})*Det_{*}(\classe{\beta})$, portando 
		
		Como $Det$ é sobrejetora, por construção, então $Det_{*}$ é sobrejetora. Suponha que $Det_{*}$ não é injetora. Então existem $\classe{\gamma} , \classe{\beta} \in \grupofundamental{\matrizunitaria{n}}$ tais que $\classe{\gamma} \neq \classe{\beta}$ e $Det_{*}(\classe{\gamma})= Det_{*}(\classe{\beta} )$, isto é, $\classe{Det(\gamma)}= \classe{Det(\beta)}$. O que implica que $Det(\gamma) \sim Det(\beta)$ e $\gamma \sim \beta$, logo $\classe{\gamma} = \classe{\beta}$, contradizendo a hipótese de que $\classe{\gamma} \neq \classe{\beta}$. Portanto $Det_{*}$ é injetora e sobrejetora, logo é um isomorfismo e $\grupofundamental{\matrizunitaria{n}} \cong \grupofundamental{\circulo} \cong \inteiros$.
	\end{prova}
	...
	\end{lema}
	\begin{prova}
		Sejam $(V, \omega)$ um 2n-espaço vetorial simplético e $\{e_{1}, \dots, e_{n}, f_{1}, \dots, f_{n}  \}$ a base simplética de $V$. Defina $W: \complementar{\reta}{\{0,1\}} \to \generalgroupreal{2n}$ por
		$$
		W(a) = diag\{\underbrace{a, -1, \dots, -1}_{n}, \underbrace{a^{-1}, -1, \dots , -1}_{n}\}.
		$$
		
		Afirmo que $W(\complementar{\reta}{\{0,1\}}) \subset \gruposimpleticonaodegenerado{*}$. De fato, $det(Id - W(a)) = (1-a)(1-a^{-1})4^{n-1} \neq 0$, pois $a\neq 1$. De fato, tomando $x,y \in V$ tais que $x=(x_{(1)}, x_{(2)})$, onde $x_{(1)} = (x_{1}, \dots, x_{n})$ e $x_{(2)} = (x_{n+1}, \dots, x_{2n})$ e analogamente para $y$, temos 
		$$
		\begin{aligned}	
		W(a)x &= (ax_{1}, -x_{2}, \dots, -x_{n}, a^{-1}x_{n+1}, -x_{n+2}, \dots, -x_{2n})
		\\
		&= ax_{1}e_{1}  -\sum_{j=2}^{n} x_{j}e_{j} + a^{-1}x_{n+1}f_{1} -\sum_{j=2}^{n} x_{n+j}f_{j},
		\\
		\formaSimpleticaPadrao{W(a)x}{W(a)y} 
		&=x_{1}y_{n+1}\formaSimpleticaPadrao{e_{1}}{f_{1}} + \sum_{j=2}^{n}x_{j}y_{n+j}\formaSimpleticaPadrao{e_{j}}{f_{j}}
		\\
		&+x_{n+1}y_{1}\formaSimpleticaPadrao{f_{1}}{e_{1}} + \sum_{j=2}^{n} x_{n+j}y_{j}\formaSimpleticaPadrao{f_{j}}{e_{j}}
		\\
		&=\sum_{j=1}^{n}\bigparenteses{x_{j}y_{n+j} - 	x_{n+j}y_{j}}
		\\
		&= \produtointerno{x_{(1)}}{y_{(2)}} - \produtointerno{x_{(2)}}{y_{(1)}}
		\\
		...
		Seja $A \in \gruposimpleticonaodegenerado{*}$. Pelo Lema $\ref{lema_conectividade_grupo_simlpetico_nao_degenerado}$ existe $B \in \gruposimpleticonaodegenerado{*}$ com todos os seus auto-valores distintos  e um caminho contínuo $\alpha:[0,1] \to \gruposimpleticonaodegenerado{*}$ tal que $\alpha(0)=A$ e $\alpha(1) = B$. Seja $\{v_{\lambda_{1}}, \dots , v_{\lambda_{2n}} \}$ uma base simplética de $V$ invariante por conjugação complexa \vermelho{(Ainda não sei a relevância dessa afirmação , isto é, $\{\overline{v}_{\lambda_{1}}, \dots , \overline{v}_{\lambda_{2n}} \}$ também é uma base simplética (veja $\ref{proposicao_base_simpletica_conjugada}$))}, onde $v_{\lambda_{j}}$ auto-vetores de $B$.
		...
		com $\Gamma(0) = B$. Por abuso de notação, escreveremos $\Gamma(s)v_{\lambda}=\gamma(s)v_{\lambda}$. No caso dos auto-valores $\overline{\lambda}$ e $1/\lambda$ teremos $\overline{\gamma}(s)$ e $1/\gamma(s)$. Com isso, temos $\Gamma(s)(E_{\lambda}) = E_{\lambda}$. Afirmo que $\Gamma(s) \in \gruposimpleticonaodegenerado{*}$. De fato, supondo $\lambda,\lambda' \in \sigma(B)$ tais que $\lambda\lambda' \neq 1$ teremos $\formaSimpleticaPadrao{\Gamma(s)v_{\lambda}}{\Gamma(s)v_{\lambda'}}= \gamma(s)\gamma'(s)\formaSimpleticaPadrao{v_{\lambda}}{v_{\lambda'}} = 0$, pela $\omega$-ortoganalidade. No caso em que $\lambda\lambda'=1$ teremos $\lambda'=\lambda^{-1}$, o que implica em $\gamma'(s) = 1/\gamma(s)$, logo $\omega(\Gamma(s)v_{\lambda},\Gamma(s)v_{\lambda'}) = \omega(v_{\lambda},v_{\lambda'})$. Por fim, caso $\lambda = \lambda'$, teremos $\omega(\Gamma(s)v_{\lambda},\Gamma(s)v_{\lambda}) = \gamma^{2}(s)\omega(v_{\lambda},v_{\lambda}) =0$. Portanto, $\Gamma^{*}(s)\omega = \omega$ e $\Gamma(s) \in \gruposimpletico{2n}$. Como $1 \notin \gamma(\intervalo)$, então $det(Id - \Gamma(s))\neq 0 $, logo $\Gamma(s)\in \gruposimpleticonaodegenerado{*}$ para todo $s\in \intervalo$.
		...
		\begin{enumerate}
			...
			\item Se $\alpha([0,1]) \subset \gruposimpleticonaodegenerado{-}$, então $B$ possui apenas 2 auto-valores positivos. Supondo que $\lambda \in \sigma(A)$ tal que $\lambda > 0$, então $\lambda^{-1} > 0$, pois os auto-valores de $A$ são determinados aos pares $\lambda, \lambda^{-1}$. Podemos reordenar a base simplética tal que $\lambda_{1}=\lambda$ e $\lambda_{n+1}=\lambda^{-1}$ sejam esses auto-valores positivos. Assim, tem-se que $\Gamma(0) = B$ e $\Gamma(1) = diag\{\lambda, -1, \dots, -1, \lambda^{-1}, -1, \dots, -1\}$. Seja $\Gamma': \intervalo \to \gruposimpleticonaodegenerado{-}$ definida por $\Gamma'(t)=diag\{ \theta(t), -1,\dots, -1, 1/\theta(t) ,-1,\dots, -1\}$,
			onde $\theta(t)=(2- \lambda)t + \lambda$. Note que $\Gamma'$ é um caminho contínuo que conecta $\Gamma(1)$ a $W^{-}$. Com isso, a justaposição $\Gamma'*\Gamma$ é um caminho conectando $B$ a $W^{-}$ e a justaposição $\alpha * \Gamma'*\Gamma: \intervalo \to \gruposimpleticonaodegenerado{-}$ é um caminho contínuo conectando $A$ a $W^{-}$. Com uma construção análoga a do item anterior concluimos que qualquer $A' \in \gruposimpleticonaodegenerado{-}$ pode ser conectado a $A$ por um caminho contínuo que passa por $W^{-}$. Portanto, $\gruposimpleticonaodegenerado{-}$ é conexo por caminhos, logo é conexo.
		\end{enumerate}
		
		Defina $g:\gruposimpletico{2n}\to \reta$ por $g(A) = det(Id-A)$. Como o determinante é uma função contínua, então $g$ é contínua. Definindo os abertos $\real{\pm} = \{x \in \reta:\pm x >0 \}$, tem-se que $g^{-1}(\real{\pm}) = \gruposimpleticonaodegenerado{\pm}$. Logo, pela continuidade de $g$, tem-se que $\gruposimpleticonaodegenerado{\pm}$ são abertos e $\gruposimpleticonaodegenerado{*} =\gruposimpleticonaodegenerado{+} \cup \gruposimpleticonaodegenerado{-}$ é um aberto.
	\end{prova}
	
	\begin{corolario}
		A inclusão $\gruposimpleticonaodegenerado{*} \hookrightarrow \gruposimpletico{2n}$ induz o homomorfismo trivial entre os grupos fundamentais. \vermelho{(Estou revendo onde 
		esse resultado para a construção do índice de maslov. Aparentemente, ele deve ser feito após a demonstração das propriedades de $\rho$.)}
	\end{corolario}
	\begin{prova}
		
		Suponha $A \in \gruposimpleticonaodegenerado{*}$ e $\sigma(A) \subset \complexo{}$. Note que, dado um auto-valor $\lambda$, teremos  $\lambda/|\lambda| \in \circulo$. Sejam $\lambda_{j} \in \sigma(A)$ com $1\leq j\leq n $ auto-valores tais que $\alpha_{j}:\gruposimpleticonaodegenerado{+} \to [0,1]$ são funções contínuas definidas por $\exp(i 2\pi\alpha_{j}(A)) = \lambda_{j}/|\lambda_{j}|$ para $0\leq j \leq n$,
		onde $\lambda_{j}\neq \pm 1$ e $0\leq \alpha_{1}(A) \leq \dots \leq \alpha_{n}(A)\leq 1$. Definindo $\caminhosfechados{\gruposimpleticonaodegenerado{+}} \subset \caminhossempontobase{\gruposimpleticonaodegenerado{+}}$ como sendo o conjunto de caminhos contínuos e periódicos em $\gruposimpleticonaodegenerado{+}$ temos
		$$
	...
	Sejam $(V, \omega)$ um 2n-espaço vetorial simplético, $L_{V}$ o conjunto dos subespaços Lagrangianos de $V$ e   $\caminhoslagrangianosV{a}{b}= \{\gamma:[a,b]\to L_{V}\times L_{V}\}$ o conjunto de todos os caminhos contínuos e suaves por partes em $L_{V} \times L_{V}$. Como topologia de $\caminhoslagrangianosV{a}{b}$ vamos adotar a topologia compacto-aberta (veja $\cite{munkres_topology}$). Um índice de Maslov é uma aplicação $\mu:\caminhoslagrangianosV{a}{b}\to \inteiros$ satisfazendo as seguintes propriedades:
	\begin{enumerate}
		\item \textbf{\textit{(Invariância por Translação)}} Seja $g: [a,b] \to [ak+l, bk+l]$ definida por $g(t)=at+l$, para $k>0$ e $l\geq 0$. Então, dado $\gamma\in \caminhoslagrangianosV{ak+l}{bk+l}$ temos que 
		$$
	...
	Outros detalhes sobre a axiomatização e demonstração da equivalência entre diferentes construções do índice de Maslov pode ser encontrada em $\cite{cappell_maslov_index_equivalencia}$.
	
	\section{Construção de $\rho: \gruposimpletico{2n} \to \circulo$}
	Para a construção do índice de Maslov, vamos adotar as complexificações $(\complexificado, \Omega) $ e $\gruposimpleticocomplexo{2n}$ do 2n-espaço vetorial simplético $(V, \omega)$ e do grupo simplético$\gruposimpletico{2n}$, respectivamente. Os detalhes da complexificação podem ser encontrados na Seção $\ref{secao_complexificacao_espacos_vetoriais}$. 
	
	O primeiro passo é a construção de uma aplicação contínua $\rho: \gruposimpletico{2n}\to \circulo$ tal que, definido o determinante $det:\matrizSimpleticaOrtogonal\to \circulo$, tem-se que $\rho|_{\matrizSimpleticaOrtogonal} = det$.
	...
	Na Seção $\ref{secao_complexificacao_espacos_vetoriais}$ mostrou-se que, dados $\lambda \in \sigma(A)$, temos que $\lambda, \lambda^{-1}, \overline{\lambda}, \overline{\lambda}^{-1}  \in \sigma(A)$. Suponha que $\lambda, \overline{\lambda} \in \circulo$. Então $0 \neq \formaSimpleticaExtendida{E_{\lambda}}{E_{\overline{\lambda}}} \in i\reta$. De fato, dados $v \in E_{\lambda}$ e $\overline{v} \in E_{\overline{\lambda}}$, a Proposição \ref{proposicao_forma_simpletica_vetor_conjugado} garante que  $\formaSimpleticaExtendida{\overline{v}}{v} \in i\reta$.
	
	Para se construir uma aplicação que extenda contínuamente o determinante se introduz uma ordenação nos auto-valores de $A$. Sabe-se que um dado $\lambda \in \espectrooperador{A}$ se pode assumir um dos valores $|\lambda|\leq 1$ ou $|\lambda|>1$, além disso, são determimados em quádruplas $\lambda, \overline{\lambda}, \lambda^{-1}, \overline{\lambda}^{-1}$. Com isso, basta analisar os casos em que $|\lambda| \leq 1$, o que implica que $|\overline{\lambda}|\leq 1$, $|\lambda^{-1}|\geq 1$ e $|\overline{\lambda}^{-1}|\geq 1$.
	
	Tome $\lambda_{1}\in \sigma(A)$ tal que $|\lambda_{1}|<1$ e o escreva como $\lambda_{1} = |\lambda_{1}|\exp(i\theta_{\lambda_{1}})$, para algum $\theta_{\lambda_{1}} \in [0,2\pi]$. Tome um outro auto-valor $\lambda_{2}\neq \lambda_{1}$ tal que $|\lambda_{2}|<1$. Se $|\lambda_{1}| < |\lambda_{2}|$ ou  $|\lambda_{1}|= |\lambda_{2}|$ com $\theta_{\lambda} \leq \theta_{\beta}$, diremos que $\lambda_{1} \leq_{o} \lambda_{2}$. Repetindo esse procedimento n-vezes teremos a sequência $\lambda_{1} \leq_{0} \dots \leq_{0}\lambda_{n} \leq_{0} \lambda_{n}^{-1} \leq_{0} \dots \leq_{0} \lambda_{1}^{-1}$. Essa ordenação será chamada de ordem de primeiro tipo.
	...
	Considere $\{\textbf{e}, \textbf{f} \}$ como sendo uma base simplética de $\complexificado{V}$. Sejam $v, u \in \complexificado{V}$ e $v_{j}, u_{j} \in \complexo{}$ os coeficientes desses vetores nessa base, onde $v_{j} = a_{j}+ib_{j}, u_{j} =c_{j}+id_{j}$. Então
	
	$$
	\begin{aligned}
	\formaSimpleticaExtendida{\overline{v}}{u} 
	&= \sum_{j=1}^{n} (\overline{v}_{j}u_{n+j} - \overline{v}_{n+j}u_{j})
	\\
	&=\sum_{j=1}^{n} \bigparenteses{(a_{j} - ib_{j})(c_{n+j} +id_{n+j}) - (a_{n+j} - ib_{n+j})(c_{j} +id_{j})}
	\\
	&=\underbrace{\sum_{j=1}^{n} \bigparenteses{(a_{j}c_{n+j}+b_{j}d_{n+j})-(a_{n+j}c_{j}+b_{n+j}d_{j})}}_{\parteReal{\formaSimpleticaExtendida{\overline{v}}{u}}}
	\\
	&+ i\underbrace{\sum_{j=1}^{n} \bigparenteses{(b_{n+j}c_{j}+a_{j}d_{n+j})-(b_{j}c_{n+j}+a_{n+j}d_{j})}}_{\parteImaginaria{\formaSimpleticaExtendida{\overline{v}}{u} }}
	\\
	&=\parteReal{\formaSimpleticaExtendida{\overline{v}}{u}}+i\parteImaginaria{\formaSimpleticaExtendida{\overline{v}}{u}}.
	\end{aligned}
	$$
	...
	\begin{prova} 
		...
		Tomando $z \in \complexo{}$, pode-se verificar que $\parteReal{z} = \parteImaginaria{-iz}$. Com isso, escreva Suponha que $B$ seja degenerada, isto é, existe $v \in \complexificado{V}$ com $v\neq 0$ tal que $B(v, u ) = 0$ para todo $u \in \complexificado{V}$. Então
	...
	\begin{definicao}\label{definicao_forma_quadratica_Q}
		Seja  $Q: \complexificado{V} \to \reta$ a forma quadrática definida por $Q(v) = B(v,v)$. Então $m(Q) = m_{+}(Q)-m_{-}(Q)$ é chamada a assinatura de $Q$, onde $m_{\pm}(Q)$ são as dimensões dos maiores  subespaços de $\complexificado{V}$ nos quais $Q$ é positiva-definida ou negativa-definida, respectivamente.
	\end{definicao}
	
	Note que, a forma quadrática $Q$ é não-degenerada pois $B$ é não-degenerada.
	
	Seja $\{\textbf{e}, \textbf{f}\}$ a base simplética de $\complexificado{V}$ e tome $v = v_{(1)}\textbf{e}+v_{(2)}\textbf{f}\in \complexificado{V}$. Pela Proposição \ref{proposicao_forma_simpletica_vetor_conjugado} tem-se que $Q(v)=\parteImaginaria{\formaSimpleticaExtendida{\overline{v}}{v}} = 2\parteImaginaria{\produtointerno{\overline{v}_{(1)}}{v_{(2)}}}$. Afirmo que o conjunto $\complexificado{V}^{+} = \{v \in \complexificado{V}: v_{(1)}=v_{(2)}\}$ é um subespaço vetorial de $\complexificado{V}$. De fato, é imediato que $0 \in \complexificado{V}^{+}$. Dados $\lambda \in \complexo{}$ e $v, u \in \complexificado{V}^{+}$, tem-se que 
	$$
		\begin{aligned}
		\lambda v + u  &= \lambda (v_{(1)}\textbf{e}+v_{(1)}\textbf{f}) + u_{(1)}\textbf{e}+u_{(1)}\textbf{f} 
		\\
		&= (\lambda v_{(1)} + u_{(1)})\textbf{e}+(\lambda v_{(1)} + u_{(1)})\textbf{f} 
		\\
		&= (\lambda v_{(1)} + u_{(1)})(\textbf{e}+ \textbf{f}) \in \complexificado{V}^{+}.
		\end{aligned}
	$$
	
	Portanto $\complexificado{V}^{+} $ é um subespaço vetorial de $\complexificado{V}$.  Além disso, $\{\textbf{e}+\textbf{f}\} = \{e_{1}+f_{1}, \dots, e_{n}+f_{n}\}$ é uma base de $\complexificado{V}^{+}$, logo $dim(\complexificado{V}^{+}) = n$. Com isso, $Q(v) = 2\parteImaginaria{\produtointerno{\overline{v}_{(1)}}{v_{(1)}}} = 2\norma{v_{(1)}}^{2} \geq 0$, para todo $v \in \complexificado{V}^{+}$. Analogamente, tem-se que $\complexificado{V}^{-} = \{v \in \complexificado{V}: v_{(1)} =- v_{(2)}\}$ também é um subespaço vetorial de $\complexificado{V}$, cuja base é $\{\textbf{e}-\textbf{f}\}$, e portanto $dim(\complexificado{V}^{-}) = n$. Então $Q(v) = 2\parteImaginaria{\produtointerno{\overline{v}_{(1)}}{-v_{(1)}}} = -2\norma{v_{(1)}}^{2} \leq 0$, para todo $v \in \complexificado{V}^{-}$. Como $\complexificado{V}^{+}\cap \complexificado{V}^{-} = \{0\}$ e $dim(\complexificado{V}^{+}) = dim(\complexificado{V}^{-}) = n$, então $\complexificado{V}=\complexificado{V}^{+}\oplus\complexificado{V}^{-}$.
	
	Sejam $A \in \gruposimpletico{2n}$, $\lambda\in \sigma(A)$ e $E_{\lambda}$ o auto-espaço generalizado associado a $\lambda$, cuja multiplicidade é denotada por $m(\lambda)$. Suponha que $\lambda \in \complementar{\circulo}{\{\pm 1\} }$ e repita a decomposição $\autoespaco{\lambda} = \autoespaco{\lambda}^{+}\oplus \autoespaco{\lambda}^{-} $ anteriormente feita para $\complexificado{V}$. Pelo Corolário \ref{corolario_restricao_forma_simpletica} tem-se que a restrição $\Omega|_{\autoespaco{\lambda} \oplus \autoespaco{\lambda}}$ é não-degenerada, logo a restrição da forma quadrática $Q|_{\autoespaco{\lambda}}$ é não-degenerada. Além disso, $dim(\autoespaco{\lambda}^{+}) = dim(\autoespaco{\lambda}^{-})$ e $dim(\autoespaco{\lambda})=2dim(\autoespaco{\lambda}^{+})$. Sejam $m_{+}(\lambda) = dim(\autoespaco{\lambda}^{+})$ e $\rho: \gruposimpletico{2n}\to \complexo{}$ definida por
	$$
	...
	Afirmo que a restrição $\rho|_{\gruposimpleticoespecial{2n}}$ coincide com a aplicação $\rho$ anteriormente definida no Lema \ref{lema_aplicacao_rho}. De fato, tomando $A \in \gruposimpleticoespecial{2n}$ tem-se que $m_{+}(\lambda) = 1$ para todo $\lambda\in \espectrooperador{A}$. Suponha uma ordem de primeiro tipo para os auto-valores de $A$. Com isso, tem-se o produto $\lambda_{1}\dots \lambda_{n} = \lambda_{1}^{m_{+}(\lambda_{1})}\dots \lambda_{1}^{m_{+}(\lambda_{1})}$. Sem perda de generalidade, pode-se supor que $\lambda_{j} \in \reta$ e que $\lambda_{k}\in \circulo$ para $1\leq j \leq r $ e $r \leq k \leq n$ para algum $r \leq k$. Com isso, $\lambda_{j}/|\lambda_{j}|=\pm 1$ e $\lambda_{k}/|\lambda_{k}| = \lambda_{k}$. Seja $m_{0}$ seja a quantidade de auto-valores reais negativos de $A$. Então tem-se o produto 
	$$
	\begin{aligned}
	\rho(A) 
	&=
	...
	\\
	&= (-1)^{m_{0}}\frac{\lambda_{r+1}}{|\lambda_{r+1}|}^{m_{+}(\lambda_{r+1})}\dots \frac{\lambda_{k}}{|\lambda_{k}|}^{m_{+}(\lambda_{k})} \underbrace{\frac{\lambda_{k+1}}{|\lambda_{k+1}|}^{m_{+}(\lambda_{k+1})}\dots \frac{\lambda_{n}}{|\lambda_{n}|}^{m_{+}(\lambda_{n})}}_{\vermelho{???}}
	\\
	...
	\begin{prova}
		...
		\begin{enumerate}
			\item \label{item_naturalidade_rho} \textbf{\textit{Naturalidade:}} Sejam $T : \complexificado{V}_{1} \to \complexificado{V}_{2}$ um isomorfismo simplético e $A \in \gruposimpletico{\complexificado{V}}$. Pelo Lema \ref{lema-caracterizacao_espectro_semelhante} tem-se que $\espectrooperador{T} = \espectrooperador{T^{-1}}$. Definindo $\sigma'(A) = \lambda\in \espectrooperador{A}\cap\complementar{\circulo}{\reta}$ e analogamente $\sigma'(T)$, tem-se
			$$
				\begin{aligned}
						\rho(TAT^{-1}) 
						&=(-1)^{m_{0}^{T}} (-1)^{m_{0}^{A}}(-1)^{m_{0}^{T^{-1}}}\prod_{\lambda\in \sigma'(A), \beta\in \sigma'(T) }\beta^{m_{+}(\beta)}\lambda^{m_{+}(\lambda)}(\beta^{-1}) ^{m_{+}(\beta^{-1})}
						\\
						&=(-1)^{2m_{0}^{T}} (-1)^{m_{0}^{A}}\prod_{\lambda\in \sigma'(A), \beta\in \sigma'(T) }\beta^{m_{+}(\beta)}(\beta^{-1}) ^{m_{+}(\beta)}\lambda^{m_{+}(\lambda)}
						\\
						&=(-1)^{m_{0}^{A}}\prod_{\lambda\in \sigma'(A)}\lambda^{m_{+}(\lambda)}
						\\
						&=\rho(A),
				\end{aligned}
			$$
			onde foi usado o fato de que a multiplicidade de $\beta, \beta^{-1} \in \espectrooperador{T}$ são iguais.
			
			\item \textbf{\textit{Produto:}} Sejam $(\complexificado{V}, \Omega) = (\complexificado{V}_{1}\times \complexificado{V}_{2}, \Omega_{1}\times \Omega_{2})$ um $2k$-espaço vetorial simplético, onde $\complexificado{V}_{1}, \complexificado{V}_{2}$ são $2n_{1}$ e $2n_{2}$-espaços vetoriais simpléticos, respectivamente, e $k=2n_{1}+2n_{2}$. Dado $A \in \gruposimpletico{\complexificado{V}}$ tem-se que $Av = (A_{1}v_{1}, A_{2}v_{2})$, onde $v=(v_{1}, v_{2}) \in \complexificado{V}$, $A_{1}\in \gruposimpletico{\complexificado{V}_{1}}$ e $A_{2}\in \gruposimpletico{\complexificado{V}_{2}}$. Com isso, um auto-espaço $E_{\lambda}$ de $A$ pode ser escrito como $E_{\lambda} = E_{\lambda_{1}}\times E_{\lambda_{2}}$, onde $E_{\lambda_{1}}$, $E_{\lambda_{2}}$ são auto-espaços de $A_{1}$ e $A_{2}$, porespectivamente. Desse modo, a aplicação $\hat{\rho}: \gruposimpletico{\complexificado{V}} \to \circulo$ definida por $\hat{\rho}(A) = \rho(A_{1})\rho(A_{2})$ satisfaz o axioma do produto.
			
			\item \textbf{\textit{Deteminante:}} Sejam $A \in \mathcal{U}$ e $U \in \matrizunitaria{n}$ com a identificação pelo isomorfismo descrito na Seção \ref{secao_grupo_simpletico} dado por
			$$
			\begin{aligned}
				\real{2n} \ni (x,y) &\mapsto x+iy \in \complexo{n},
				\\
				\mathcal{U} \ni 
				\left(
					\begin{array}{cc}
			...
			\end{aligned}
			$$
			Se $(x,y) \in \real{2n}$ é um auto-vetor de $A$ com auto-valor $\lambda$ em $\complexo{}$, então, pelo isomorfismo, tem-se que $A(x,y)=\lambda (x,y) \mapsto U(x+iy) = \lambda(x+iy)$. Logo se tem $\espectrooperador{A} \subseteq \espectrooperador{U}$. Analogamente, a identificação dos auto-valores de $U$ com os auto-valores de $A$ em $\complexo{}$ implica que $\espectrooperador{U} \subseteq\espectrooperador{A}$. Portanto $\espectrooperador{A} =\espectrooperador{U}$ em $\complexo{}$.
			
			Sejam $v$ um auto-vetor de $A$ com auto-valor $\lambda$ e $\autoespaco{\lambda}$ seu auto-espaço. Sabendo que $\estruturacomplexa: \real{2n} \to \real{2n}$ é um endomorfismo, que $A\estruturacomplexa = \estruturacomplexa A$ e que ambos $A$ e $\estruturacomplexa$ são diagonalizáveis, onde $\espectrooperador{\estruturacomplexa} = \{\pm i\}$, então $A\estruturacomplexa v = \estruturacomplexa Av = \lambda \estruturacomplexa v$, logo $\estruturacomplexa v \in \autoespaco{\lambda}$, o que implica que $\estruturacomplexa(\autoespaco{\lambda}) = \autoespaco{\lambda}$. Suponha que $u$ seja um auto-vetor de $\estruturacomplexa$ com auto-valor $\beta\in \espectrooperador{\estruturacomplexa}$. Então $\estruturacomplexa A u = A \estruturacomplexa u = \beta Au$. Logo $Au \in \autoespaco{\beta}^{\estruturacomplexa}$, e pode-se afirmar que $A(\autoespaco{\beta}^{\estruturacomplexa} )=\autoespaco{\beta}^{\estruturacomplexa}$. Como ambos $A$ e $\estruturacomplexa$ são diagonalizáveis, então existe ao menos um $\lambda\in \espectrooperador{A}$ tal que $\autoespaco{\lambda}\cap\autoespaco{\beta}^{\estruturacomplexa} \neq \emptyset$, onde $\beta\in \espectrooperador{\estruturacomplexa}$.
			
			Note que os auto-espaços de $ \pm i \in \espectrooperador{\estruturacomplexa}$ são $\autoespaco{\pm i}^{\estruturacomplexa} = \{u\textbf{e}\pm iu\textbf{f}: u\in \complexo{n} \}$, e que $dim(\autoespaco{\pm i}^{\estruturacomplexa})=n$. Tomando $v \in \autoespaco{i}^{\estruturacomplexa}$, tem-se que 
			$$
			\begin{aligned}
				\formaSimpleticaExtendida{\overline{v}}{v} 
			...
			Portanto a restrição da forma quadrática $Q$, dada pela Definição \ref{definicao_forma_quadratica_Q}, ao auto-espaço $\autoespaco{ i}^{\estruturacomplexa}$, é negativa definida. Analogamente pode-se verificar que a restrição de $Q$ ao auto-espaço $\autoespaco{ -i}^{\estruturacomplexa}$ é positiva definida.
			
			Com isso, pode-se decompor $\autoespaco{\lambda} = \autoespaco{\lambda}^{+} \oplus \autoespaco{\lambda}^{-}$, onde $\autoespaco{\lambda}^{\pm} = \autoespaco{\lambda}\cap \autoespaco{\mp i}^{\estruturacomplexa}$. Consequentemente, tem-se
			$$
			\autoespaco{\lambda}^{+}=\{v=v_{(1)}\textbf{e}-iv_{(1)}\textbf{f}\in \complexificado{ V} : Av = \lambda v\} \mapsto \{v_{(1)} \in \complexo{n} : Uv_{(1)}=\lambda v_{(1)} \} = \autoespaco{\lambda}^{U}.
			$$
			
			Como os auto-espaços $\autoespaco{\pm i}^{\estruturacomplexa} \subseteq \complexificado{V}$ são maximais em relação a positividade e negatividade definida de $Q$, então os subespaços vetoriais $\autoespaco{\lambda}^{\pm} \subseteq \autoespaco{\lambda}$ também o são. Com isso, $m_{+}(\lambda) = m^{U}(\lambda)$.
			
			A aplicação $\rho|_{\mathcal{U}}: \mathcal{U} \to \circulo$ quando avaliada em $A\in \mathcal{U}$ é dada em termos do produto dos auto-valores $\lambda \in \espectrooperador{A}$ e seus respectivos $m_{+}(\lambda)$ e $m_{0}$. Como $\espectrooperador{A} = \espectrooperador{U}$ e $m_{+}(\lambda)  = m_{\lambda}^{U}$, então as multiplicidades dos auto-valores são as mesmas e $\rho(A)=det(U)$.
	...
	Um caminho $\gamma:[0,1]\to \gruposimpletico{2n}$ partindo de uma matriz $A$ pertencente a uma das componentes conexas $\gruposimpleticonaodegenerado{\pm}$ cruza $\Sigma$ um número finito de vezes, sendo que esse número de cruzamentos será determinado pelo índice de Maslov de $\gamma$. Para efetuar a contagem dos cruzamentos entre $\gamma$ e $\Sigma$, vamos fixar dois pontos $W^{\pm} \in \gruposimpleticonaodegenerado{\pm}$. A definição do índice não dependerá dessa escolha, pois vimos na Seção $\ref{secao_grupo_simpletico}$ que $\gruposimpleticonaodegenerado{\pm}$ são ambas conexas por caminhos.
	...
	\begin{definicao}
		...
		$$
		Esse número indica quantas vezes a imagem da aplicação $\rho$ executa uma meia-volta.
	\end{definicao}
	...
	Na Seção $\ref{secao_fluxo_gradiente}$ vimos que, dada um função de Morse, construímos um complexo de cadeia com base nas linhas do fluxo do gradiente negativo dessa função. Uma técnica análoga será adotada para a construção de um complexo de cadeia e operador bordo na qual resultará na Homologia de Floer.
