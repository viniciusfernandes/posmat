...
\begin{document}
	...
	Foi demonstrado na Seção \ref{secao_variedade_banach_P} que $\caminhosexponenciaisconectantespadrao$ é uma variedade e $T\caminhosexponenciaisconectantespadrao$ é seu fibrado tangente. 
	
	Seja $s	: \caminhosexponenciaisconectantespadrao \to T\caminhosexponenciaisconectantespadrao$ a seção nula de $T\caminhosexponenciaisconectantespadrao$. Pela Definição \ref{definicao_aplicao_transversal} tem-se $\operadorFloer \pitchfork s$ se, e somente se, $\diferencialfloer_{u}$ é sobrejetor para todo $u\in \caminhosexponenciaisconectantespadrao$. De fato, como $s_{u} = 0$, então $Ds_{u} = 0$. Supondo que $\operadorFloer \pitchfork s$, então  
	$$
	\pullbackfibradotangenteM{u}=\diferencialfloer_{u}(\pullbackfibradotangenteM{u})+Ds_{u}(\pullbackfibradotangenteM{u}) = \diferencialfloer_{u}(\pullbackfibradotangenteM{u}).$$ 
	...
	\vermelho{
		Pode-se mostrar que, dada uma função Hamiltoniana $H:M\times \reta\to \reta$, existe uma função Hamiltoniana $H_{0} $ suficientemente próxima a $H$, no sentido da $\cktopologia{\infty}$. Além disso, ambas têm as mesmas órbitas periódicas e para as quais $\energiafinitaMconectante$ são variedades com dimensão determinada.
		}
	
	Seja $h :M\times \reta\to \reta$ uma função suave, fixe uma sequência $\epsilon=(\epsilon_{j})$ tais que $\epsilon_{j}\in \real{+}$ e defina
	$$
	\norma{h}_{\epsilon} = \sum_{k\geq 0}\epsilon_{k}\sup_{(x,t)\in M\times \reta}\max_{|\alpha|=k}|\partial^{\alpha}h(x,t)|.
	$$
	...
	\begin{definicao}
		(Espaço das Perturbação Hamiltoniana) Seja $H :M\times \reta\to \reta$ uma função Hamiltoniana. O conjunto $\perturbacaoHamiltoniana{H}$ é o conjunto das funções $h \in C^{\infty}_{\epsilon}$ tal que $h(x,t) = 0$ em alguma vizinhança das órbitas $1$-periódicas de $H$.
	\end{definicao}

	\begin{teorema}
		Fixe uma função Hamiltoniana não-degenerada $H$. Então existe uma vizinhança de $0 \in \perturbacaoHamiltoniana{H}$ e uma intersecção enumerável de abertos densos de $\paresregulares$ $\hamiltonianasRegulares$ nessa vizinhança tal que, se $h\in \paresregulares$, então $H+h$ é não-degenerada e o diferencial $\diferencialfloer_{u}$ é sobrejetor para todo $u \in \energiafinitaM(H+h; J)$\vermelho{(Não sei onde o Michelle introduziu essa notação. Devo rever isso!!!)}.
	\end{teorema}
	...
	Anteriormente, foi mostrado que a propriedade de transversalidade do operador de Floer esta diretamente relacionada a sobrejetividade de seu diferencial $\diferencialfloer$.
	...
	Considere o conjunto $\caminhosexponenciaisconectantespadrao\times \perturbacaoHamiltoniana{H}$, que é uma variedade, pois é o produto cartesiano de variedades. Defina o fibrado vetorial $\mathcal{E} \to \caminhosexponenciaisconectantespadrao\times \perturbacaoHamiltoniana{H}$, onde 
	$$
	...
	$$
	\sigma(u,h)=\derivadaparcial{u}{s}+J\derivadaparcial{u}{t}+\nabla_{u}(H+h)\in \espacoLp{\pullbackfibradotangenteM{u}}, 
	$$
	é uma seção do fibrado vetorial e esta bem definida, o que pode ser visto na Seção \ref{secao_operador_floer}. Note que as singularidades da seção $\sigma$ são os elementos de $\orbitasConectantesZ$. Com isso, dado $(u,H+h)\in \orbitasConectantesZ$, tem-se que $\sigma(u,H+h) = 0$. Além disso, pode-se mostrar que 
	$$
	D_{(u,H+h)}\sigma(Y,\eta) = \diferencialfloer_{u}Y+\nabla_{u}\eta,
	$$
	\vermelho{(Pq $\eta $ e não h???)} pois a seção $\sigma$ é um operador linear em $h$.
