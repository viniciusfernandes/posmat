...
\newcommand{\espacoLpdualdois}{(L^{2})^{*}(\retacartesianocirculo;\real{2n})}
...
\begin{document}
	...
	\begin{definicao}
		...
		\begin{enumerate}
			\item existe uma família de abertos $\{U_{i}\}_{i\in I}$ de $N$ tal que $N=\bigcup_{i\in I}U_{i}$,
	...
	A seguir serão definidos alguns espaços de Sobolev, os quais serão utilizados como base para a construção de um sistema local cartas para o conjunto $\caminhosexponenciaisconectantespadrao$, e como homeomorfismo local será utilizada a aplicação exponencial (veja o Apêndice \ref{apendice_variedade_riemanniana}). Com isso, demonstra-se que esse conjunto possui um atlas e que satisfaz as propriedades de uma variedade de Banach. Os espaços de Sobolev são apresentados no Apêndice \ref{secao_espaco_sobolev}.
	...
	\begin{observacao}
	A análise realizada nesta seção fica simplificada se identificarmos os espaços de Sobolev $\espacosobolev{\pullbackfibradotangenteM{u}}$ e $\espacosobolev{\pullbackfibradotangenteM{x}}$. Em \cite{audi_floer_homology} pode ser encontrada a identificados desses espaços com os espaços de Sobolev $\espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}}$ e  $\espacosobolevcontradominio{\circulo}{\real{2n}}$, respectivamente. Para detalhes desta identificação, veja \cite{audi_floer_homology}.
	\end{observacao}
	
	Os espaços de Banach  $\espacoLp{\pullbackfibradotangenteM{u}}$, $\espacoLp{\pullbackfibradotangenteM{x}}$ e seus  equivalentes  $\espacoLpcontradominio{\retacartesianocirculo}{\real{2n}}$ e $\espacoLpcontradominio{\circulo}{\real{2n}}$, são definidos de forma análoga às construções anteriores.
	...
	\begin{proposicao}\label{proposicao_caminhos_variedade_banach}
		O conjunto $\caminhosexponenciaisSobolev$ é uma variedade de Banach de classe $C^{\infty}$ munida com o atlas $\{(\espacosobolev{x^{*}D_{x}}, \Exp_{x})\}_{x \in \lacocontrateis }$, onde $D_{x} \subset TM$ é uma vizinhança aberta da seção nula de $TM$. Além disso, 
		$$
	...
	\begin{observacao}
		Uma consequência da construção anterior é que o espaço tangente a $x\in \caminhosexponenciaisSobolev$ da variedade de banach $\caminhosexponenciaisSobolev$ é
		$$
	...
	\begin{definicao}\label{definicao_caminhos_decaimentos_exponenciais}
		...
		$$
		\normagrande{\derivadaparcial{w}{s}w(s,t)} \leq Ke^{-\delta|s|} \;\; \text{e} \;\; \normagrande{\derivadaparcial{w}{t}(s,t) -X_{H}(w(s,t))} \leq Ke^{-\delta|s|}
		$$
		para $\delta \in \reta_{+}$, é chamado de conjunto dos caminhos com decaimento exponencial conectando $x^{-}$ a $x^{+}$ e é denotado por $\caminhosdecaimentoexponencialpadrao$.
	\end{definicao}
	...
	\begin{proposicao}\label{proposicao_variedade_banach}
		Sejam $x^{-}, x^{+} \in \pontoscriticos{\funcionalH}$. Então $\caminhosexponenciaisconectantespadrao$ é uma variedade de Banach para $p>2$ \alerta{(acho que a condição de p>2 é utilizada na demonstração de algum outro teorema)}.
	\end{proposicao}
	
	\begin{prova}
		Sejam $w \in \caminhosdecaimentoexponencialpadrao$, um número $r_{w}>0$ e o disco $D_{w}=\{Y \in \espacosobolev{\pullbackfibradotangenteM{w}}: \normaWp{Y}<r_{w} \}$. Além disso, suponha que $r_{w}$ seja um raio tal que a aplicação 
		$$
	...
	O seguinte teorema e sua observação caracterizam o comportamento assintótico das soluções da equação de Floer
	...
	\end{observacao}
		...
		A seguinte proposição, que é consequência co Teorema \ref{teorema_decaimento_exponencial_solucao_floer} e da Observacao \ref{observacao_decaimento_exponencial_solucao_floer}, afirma que todo solução de energia finita $u \in \energiafinitaMconectante$ satisfaz as condições de decaimento exponencial e, além disso, pode ser escrita como $u=\Exp(w,Y)$ para algum $w\in \caminhosdecaimentoexponencialpadrao$ e algum $Y\in \espacosobolev{\pullbackfibradotangenteM{w}}$.
	...
	\begin{definicao}\label{definicao_operador_floer}
		(Operador de Floer) Sejam $x^{-}, x^{+}\in \pontoscriticos{\funcionalH}$. O operador de Floer é a seção $\operadorFloer: \caminhosexponenciaisconectantesabrev \to \fibradocaminhosexponenciais$ definida por
		$$
		\operadorFloerParametro{u} = \bigparenteses{u,\operadorFloerDefinicao{u}}.
		$$
	\end{definicao}
	
	Daqui em diante adotaremos a identificação da seção $\operadorFloer$ com o operador diferencial $\partial_{s}+J\partial_{t}-J\campohamiltonianoabrev$ (veja o Apêndice \ref{apendice_fibrados}).
	
	\begin{observacao}
		 Com essa identificação o operador de Floer pode ser visto como um elemento de $\campossuaves{\caminhosexponenciaisconectantespadrao}$.
	\end{observacao}
	...
	Realizando a composição $\pi_{2}\circ i \circ \operadorFloer$, o operador de Floer $\operadorFloer$ pode ser visto como uma aplicação $\operadorFloer: \caminhosexponenciaisconectantespadrao\to \espacoLpcontradominio{\retacartesianocirculo}{\real{m}}$. Essa composição permite que se calcule do diferencial de $\operadorFloer$ de forma análoga ao que se faz em análise em espaços euclidianos, pois $u(s,t)$ pode ser visto como elemento de $\real{m}$ para todo $(s,t)\in \retacartesianocirculo$ e todo $u\in \caminhosexponenciaisconectantespadrao$.
	...
	\begin{definicao}
		(O diferencial do operador de Floer) Seja $\operadorFloer: \caminhosexponenciaisconectantespadrao \to \fibradocaminhosexponenciais$ o operador de Floer. Então o diferencial $\diferencialfloerponto{u}: \espacosobolev{\pullbackfibradotangenteM{u}}\to \espacoLp{\pullbackfibradotangenteM{u}}$ é dado por
		$$
	...
	A seguir o diferencial $\diferencialfloer$ será escrito como a soma de dois operadores lineares e tal decomposição é utilizada na demonstração de que $\diferencialfloer$ é uma bijeção de $\espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}}$ em $\espacoLpcontradominio{\retacartesianocirculo}{\real{2n}}$.
	
	Sejam $\tilde{x}^{-}:D^{2}\subset \complexo{} \to M$ uma extensão suave de $x^{-}$ ao disco unitário $D^{2}$ tal que  $\tilde{x}^{-}|_{\circulo} = x^{-}$ e, de forma análoga,  $\tilde{x}^{+}:D^{2}\subset \complexo{} \to M$ é uma extensão de suave de $x^{+}$. Defina uma extensão contínua $\tilde{u}:S^{2}\to M$ do cilíndro $u:\retacartesianocirculo\to M$ para a esfera $S^{2}$ como ilutrado na Figura \ref{figura_colagem_disco_cilindro}. Como $\tilde{u}(S^{2})$ é homotópico a $S^{2}$ e todo fibrado vetorial sobre $S^{2}$ é trivial, então $\pullbackfibradotangenteM{\tilde{u}}$ é um $2n$-fibrado vetorial trivial (veja o Apêndice \ref{apendice_fibrados}).
	...
	O referencial $\{e_{1}, \dots, e_{2n}\}$ define uma carta em $u$ da varidade de Banach $\caminhosexponenciaisconectantespadrao$ dada por
	$$
	...
	Nessa carta o diferencial de $\diferencialfloerponto{u}$ avaliado em $Y=\sum_{j=1}^{2n}y_{j}e_{j}$ é escrito como
...
\alerta{
	...
	Como o diferencial $\diferencialfloerponto{u}$ está definido de $\espacosobolev{\pullbackfibradotangenteM{u}}$ em $\espacoLp{\pullbackfibradotangenteM{u}}$, e esses espaços podem ser identificados com $\espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}}$ e $\espacoLpcontradominio{\retacartesianocirculo}{\real{2n}}$, respectivamente, então o diferencial do operador de Floer pode ser visto como uma aplicação
	$$
	\diferencialfloerponto{u}:\espacosobolevcontradominio{\retacartesianocirculo}{\real{2n}}\to \espacoLpcontradominio{\retacartesianocirculo}{\real{2n}},
	$$
	onde $\diferencialfloerponto{u}=\overline{\partial}+ S$. Seja  $\text{Endo}(\real{2n})$ o conjunto dos endomorfismos de $\real{2n}$ em $\real{2n}$, então $S$ pode ser vista como um uma aplicação de $\retacartesianocirculo$ em $\text{Endo}(\real{2n})$. Por uma questão de simplicidade, o operador $\diferencialfloerponto{u}$ será denotado por $\diferencialfloerabrev$. 
	
	Os resultados dessa seção serão demonstrados para o caso particular em que $p=2$, isto é, os espaços $\espacosobolevdois{\retacartesianocirculo}{\real{2n}}$ e $\espacoLdoiscontradom{\retacartesianocirculo}{\real{2n}}$. O caso geral em que $1 <p<\infty$ pode ser encontrados em \cite{audi_floer_homology} e \cite{salamon_lecture}.
	
\aviso{Brezis pag 203}
	
	Os espaços  $\espacoLdoiscontradom{\retacartesianocirculo}{\real{2n}}$ e $\espacosobolevdois{\retacartesianocirculo}{\real{2n}}$ munidos dos produtos internos 
	$$
	\produtointernoLdois{Y}{X}=\int_{\retacartesianocirculo}\produtointerno{Y(s,t)}{X(s,t)}
	$$
	e
	$$
	\produtointernoWdois{Y}{X}=\int_{\retacartesianocirculo}\bigparenteses{\produtointerno{Y(s,t)}{X(s,t)} + \produtointerno{\derivadaparcial{Y}{s}(s,t)}{\derivadaparcial{X}{s}X(s,t)} + \produtointerno{\derivadaparcial{Y}{t}(s,t)}{\derivadaparcial{X}{t}X(s,t)}},
	$$
	onde $\produtointernoabrev$ é o produto interno usual do espaço euclidiano $\real{2n}$, são espaços de Hilbert. Além disso, $\espacosobolevdois{\circulo}{\real{2n}}$ é denso em  $\espacoLdois{\circulo;\real{2n}}$ (tais resultados podem ser encontrados em \cite{breazis_sobolev_spaces}).

	O seguinte resultado, que é uma particularização do teorema de Reisz (veja \cite{kreyszig_analise_funcional}) ao espaços das aplicações p-integráveis e pode ser encontrado em \cite{breazis_sobolev_spaces}.
	
	\begin{teorema}\label{teorema_representacao_reiz}
		Sejam $1<p<\infty$ e $f \in \espacoLpdual$. Então existe uma único elemento $Y \in \espacoLpGeral{q}{\retacartesianocirculo;\real{2n}}$ 
		tal que 
		$$
		f(X) = \produtointernoLdois{Y}{X}
		$$
		para todo $X \in \espacoLpretacirculo$. Além disso, $\norma{f}_{(L^{p})^{*}} = \norma{Y}_{L^{q}}$.
	\end{teorema}

	\begin{observacao}
		No caso $p=2$ todo elemento de $\espacoLdoiscontradom{\retacartesianocirculo}{\real{2n}}$ pode ser visto como um único elemento de $\espacoLpdualdois$.
	\end{observacao}
	
	Um operador $T:D(T)\subset H_{1}\to H_{2}$ entre espaços de Hilbert é dito densamente definito se seu domínio $D(T)$ é denso em $H_{1}$. 
	
	\begin{definicao}
		Sejam $H_{1}, H_{2}$ espaços de Hilbert e $T:D(T)\subset H_{1}\to H_{2}$ um operador linear densamente definido. O operador Hilbert-adjunto de $T$ é o operador linear $T^{*}:D(T^{*})\subset H_{2}\to H_{1}$ tal que
		$$
		\produtointerno{TX}{Y}_{H_{2}} = \produtointerno{X}{T^{*}Y}_{H_{1}}
		$$ 
		para todo $X\in H_{1}$ e todo $Y\in H_{2}$. O operador $T^{*}$.
	\end{definicao}
	
	\begin{proposicao}\label{proposicao_diferencial_floer_adjunto}
		Em termos de sitribuição, o operador $\diferencialfloerabrev^{*} :\espacoLdoiscontradom{\retacartesianocirculo}{\real{2n}} \to \espacosobolevdois{\retacartesianocirculo}{\real{2n}}$ é dado por
		$$
		\diferencialfloerabrev^{*}=-\derivadaparcial{}{s}+\estruturacomplexa \derivadaparcial{}{t}+S^{t}.
		$$
	\end{proposicao}
	\begin{prova} Tome $f \in \espacoLpdualdois$ tal que $f=\produtointernoLdois{Y}{.}$ para algum $Y\in \espacoLdoiscontradom{\retacartesianocirculo}{\real{2n}}$, de acordo com o Teorema \ref{teorema_representacao_reiz}. Denote sua restrição a imagem do operador $\diferencialfloerabrev$ por $f|_{\diferencialfloerabrev}$. Seja $\funcoessupcompactcontradom{\retacartesianocirculo}{\real{2n}}$ o conjunto das aplicações suaves e de suporte compacto de $\retacartesianocirculo$ em $\real{2n}$ e defina o espaço de funções teste $\mathcal{W}=\espacosobolevdois{\retacartesianocirculo}{\real{2n}} \cap \funcoessupcompactcontradom{\retacartesianocirculo}{\real{2n}}$ e os espaço de distribuições  $\mathcal{W}'=\espacoLpdualdois$. Então $		f|_{\diferencialfloerabrev}$ pode ser visto como um elemento de $\mathcal{W}'$, e com isso
		$$
		\begin{aligned}
		f|_{\diferencialfloerabrev}(X)
		&=
		\produtointernoLdois{Y}{\diferencialfloerabrev X}
		\\
		...
		Efetuando integração por partes da primeira integral e usando o fato de que $\lim_{s\to \pm \infty} X(s,t) = 0$, pois tem suporte compacto, tem-se que
		$$
		\begin{aligned}
		...
		Usando o fato que $X(s, 0)=X(s, 1)$ para todo $s\in \reta$, e aplicando integração por parte na segunda integral, tem-se que
		$$
		\begin{aligned}
		...
		\\
		&=\produtointernoLdois{\diferencialfloerabrev^{*}Y}{X}.
		\end{aligned}
		$$
	\end{prova}

	\begin{lema}\label{lema_diferencial_floer_bijecao}
		Suponha que $S(s,t) = S(t)$. Se $\Psi:[0,1] \to \gruposimpletico{2n}$ é solução do sistema $\dot{\Psi}(t)=\estruturacomplexa S(t)\Psi(t)$ com $\Psi(0)=Id$, tal que $\det(\Psi(1) - Id) \neq 0 $, então 
		$$
		\diferencialfloerabrev : \espacosobolevdois{\retacartesianocirculo}{\real{2n}} \to \espacoLdoiscontradom{\retacartesianocirculo}{\real{2n}}
		$$
		é bijetor.
	\end{lema}
	\begin{prova}
		Considere o operador $A: \espacosobolevdois{\circulo}{\real{2n}} \to \espacoLdois{\circulo;\real{2n}}$ definido por $A = \estruturacomplexa \derivadaparcialabrev{t} + S$. Tomando $\Psi \in \espacosobolevdois{\circulo}{\real{2n}}$ tem-se que 
		$$
		\diferencialfloerabrev\Psi  = \derivadaparcial{\Psi}{s}+A\Psi = A\Psi
		$$
		e
		$$
		\diferencialfloerabrev^{*}\Psi  = -\derivadaparcial{\Psi}{s}+A^{*}\Psi = A^{*}\Psi.
		$$
		Como $S$ não depende do parâmetro $s$, então $S^{t} = S$.
	...
	Aplicando a regularidade elíptica, que pode ser encontrada em $\cite{audi_floer_homology}$, pode-se mostrar a continuidade do diferencial do operador de Floer $\diferencialfloerabrev$.
	...
	\begin{prova}
		...
		\begin{teorema}\label{teorema_indice_diferencial_operador_floer}
			...
			$$
			\ind(\diferencialfloerponto{u}) = \mu(x^{-}) - \mu(x^{+}).
			$$
		\end{teorema}
	...
	$$
	\diferencialfloerponto{u}(\xi)= \Big( \derivadaparcial{}{s} + J_{u}\derivadaparcial {}{t}\Big)(\xi)+ \Big(dJ_{u}(.)\bigparenteses{\derivadaparcial{u}{t} - X_{H}(u)} - J_{u} \diferencialhamiltoniano{u}\Big)(\xi).
	$$
