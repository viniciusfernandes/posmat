...
\begin{document}
	...
	\chapter*{Resumo}
	%	A topologia simplética é o ramo da matemática que trata das questões topológicas das variedades simpléticas, e é a esse ramo que pertence esse texto. 
	Nesse trabalho é apresentada uma detalhada construção do índice de Maslov, bem como as demonstrações de suas propriedades. Além disso, é demonstrado que ele é uma generalização do índice de Morse para dimensões infinitas.
	
	Utilizando uma abordagem funcional, é estudada a forma com que as soluções 1-periódicas de um sistema Hamiltoniano, definido em uma variedade compacta, são conectadas pelas linhas do fluxo gradiente do funcional de ação $\funcionalH$. Também é demonstrada uma relação entre seus pontos críticos de $\funcionalH$ e as soluções 1-periódicas do sistema Hamiltoniano, e como a equação de Floer é obtida da definição do campo gradiente negativo do funcional de ação $-\gradientefuncional$.
	
	Algumas aplicações do índice de Maslov são feitas, sendo elas: a primeira aplicação está na definição do índice Conley-Zehnder $\iconleyabrev$. A segunda é está na demonstração de que o espaço das soluções da equação de Floer que conectam dois pontos críticos do funcional de ação $\funcionalH$ é uma variedade de dimensão dada pela diferença dos índices de Conley-Zehnder $\iconleyabrev$ de seus pontos críticos associados. A terceira está na definição do complexo de Floer, cuja graduação é dada pelo índice de Conley-Zehnder. 
	
	Por fim, a Homologia de Floer e sua relação com a Homologia Singular da variedade simplética são apresentadas. Esse resultado é utilizado na demonstração da Conjectura de Arnold, encerrando o trabalho.
	\\
	\\	
	...
	Symplectic Topology is a branch of mathematics concerning about topological information of manifolds, and this is which the text belongs to. In this work, we present a detailed construction of the Maslov Index and its properties as well. Furthermore, we demonstrate this object is a generalization of Morse Index to infinite-dimensional spaces.
	
	By a functional strategy, we investigate how that 1-periodic solutions of Hamiltonian systems, defined on compact manifolds, are connected by the lines of the gradient flow of the action functional $\funcionalH$. Also, we demonstrate a relation between critical points of the action funcional $\funcionalH$ and 1-periodic solutions of Hamiltonian systems, and how the Floer Equation can be derived from negative gradient field of action functional $-\gradientefuncional$.
	
	Here, some Maslov Index application has been done, they are: the first one was done in the Conley-Zehnder Index $\iconleyabrev$ definition. The second was done in the demonstration that all solutions of Floer Equation connecting a couple of critical points of the action functional define a finite-dimensional manifold, and his dimension is the subtraction of Conley-Zehnder indexes of both critical points. And the last one, was done in the Floer Complex definition, which is graduated by the Conley-Zehnder index.
	
	Finally, the Floer Homology and his relation between Singular Homology of the symplectic manifolds are presented. This result is applyied in the Arnold Conjecture's demonstration, and doing that, we complete this text. 
	\\
	\\
	...
	definido no espaço de todas as curvas que conectam um par de pontos críticos do funcional de ação. Mostra-se que esse espaço é uma variedade de dimensão infinita (uma variedade de Banach) e que o diferencial de tal operador é um operador de Fredholm. A todo operador de Fredholm associa-se um número inteiro, chamado índice de Fredholm, que no caso de interesse, coincide com a diferença dos índices de Conley-Zehnder de cada um dos pontos críticos. 
	
	Feito isso, pode-se mostrar que, no caso de uma função Hamiltoniana independente do tempo e cuja Hessiana é limitada, tal índice coincide com o índice de Morse, a menos de uma constante. Tem-se aqui uma generalização da Homologia de Morse-Witten. Essa abordagem é uma consequência da definição do complexo de Floer e sua homologia. Analogamente ao complexo de Morse-Witten, as k-cadeias do complexo de Floer são geradas pelos pontos críticos do funcional de ação $\funcionalH$ de índice $k \in \inteiros_{+}$. A graduação do grupo gerado pelos pontos críticos do funcional de ação é dada pelo índice de Conley-Zehnder $\iconleyabrev$.
	
	Por essas razões, o ponto central do trabalho está na construção detalhada do Índice de Maslov $\mu$ e nas demonstrações de suas propriedades. Feito isso, define-se o Índice de Conley-Zehnder $\iconleyabrev$ via o índice de Maslov e demonstra-se que esse objeto é o índice de Morse a menos de uma constante. Com isso, mostra-se que o índice de de Maslov é uma generalização do índice de Morse para dimensões infinitas.
	
	O capítulo \ref{capitulo_topologia_algebrica} expõe o conteúdo prelimiar sobre topologia algébrica necessário para o entendimento dos capítulos posteriores. Esse capítulo está segmentado em duas seções: a primeira seção é dedicada à definição do grupo fundamental, da aplicação grau e demonstração de algumas de suas propriedades. São feitos alguns exemplos de cálculo dos grupos fundamentais para alguns espaços topológicos utilizando a aplicação grau. A segunda seção é dedicada a álgebra homológica. Mais espeficicamente, à Homologia Singular e Homologia Celular. Exemplos e figuras ilustrando a estrutura celular de espaços topológicos dão ênfase a Homologia celular.
	
	O capítulo \ref{capitulo_teoria_morse} a primeira seção expõe rapidamente os principais conceitos da Teoria de Morse Clássica, tais como as propriedades de uma função de Morse, o teorema de deformação, o qual associa um CW-complexo a uma função de Morse, e as Desigualdades de Morse. A segunda seção é reservada para a Teoria de Morse-Smale-Witten e para a definição do complexo de Morse-Smale-Witten e sua homologia. Exemplos e figuras complementam o capítulo.
	
	O capítulo \ref{capitulo_espacos_vetoriais_simpleticos}  expõe os principais conceitos sobre espaços vetoriais simpleticos e simplectomorfismos, bem como suas propriedades, tais como: a caracterização da forma simlética e dos subespaços simpléticos, lagrangianos e isotrópicos. Por fim, é demonstrado que todo espaço vetorial simplético pode ser munido com uma base simplética.
	
	O capítulo \ref{capitulo_grupo_simpletico} define o grupo das transformações simpléticas, chamado grupo simplético. É demonstrada uma de suas caracterizações e que as transformações simpléticas ortogonais e as matrizes unitárias são homeomorfas. Visando estudar alguns aspectos topológicos do grupo simplético, é realizada uma investigação dos auto-valores das transformações simpléticas via complexificação dos espaços vetoriais simpléticos. Por fim, um cálculo do grupo fundamental do grupo simpletico é realizado.
	
	O capítulo \ref{capitulo_indice_maslov} é o de maior relevância do trabalho. Utilizando os resultados obtidos nos capítulos anteriores, tais como, as propriedades das transformações simpléticas e a topologia do grupo simplético, propriedades da aplicação grau e a definição do grupo fundamental, é definido o Índice de Maslov e suas propriedades, as quais são demonstradas em detalhes. Figuras ilustram as definições desse capítulo, o que facilita a compreensão dos conceitos.
	
	O capítulo \ref{capitulo_equacao_floer} é o mais extenso do trabalho por conta do volume de álgebra e análise nas demonstrações dos resutlados. Na primeira parte são apresentadas a definição e propriedades das variedades simpléticas, os sistemas Hamiltonianos e algumas características do conjunto de suas soluções. Na segunda parte, é definido o funcional de ação, é demonstrada uma caracterização de seus pontos críticos e é determinada a expressão de seu campo gradiente. Em seguida, é demonstrado que as linhas de fluxo gradiente desse funcional são as soluções de energia finita da Equação de Floer. Na terceira e ultima parte, é definido o Índice de Conley-Zehnder de uma solução periódica do sistema Hamiltonia.
	
	O capítulo \ref{capitulo_aplicacoes} trata da condição de transversalidade do operador de Floer é apresentada brevemente, e um esquema de demonstração da compacidade do espaço moduli é feito. É definido complexo de Floer e algumas questões sobre sua homologia são apresentadas. Encerrando o trabalho, o conjectura de Arnold é demonstrada utilizando a Homologia de Floer e suas propriedades.	
	
	\chapter{Preliminares de Topologia Algébrica}\label{capitulo_topologia_algebrica}
	...
	%	Na definição de grupo fundamental se mantém a escolha do ponto base, contudo, 
	
	Pode-se mostrar que, para espaços topológicos conexos por caminhos, a definição de grupo fundamental independe da escolha do ponto base. A demonstração deste resultado pode ser encontrada em $\cite{massey}$.
	...
	\begin{definicao}
		(k-módulo de homologia singular) Seja $X$ um espaço topológico. O $k$-ésimo módulo de homologia singular de $X$ com coeficientes em $A$ é o módulo quociente
		$$
		\homologia{k}{X} = \frac{\kernelbordo{k}{X}}{\imagembordo{k}{X}}
		$$	
		para cada $k\geq 0$ e $\homologia{k}{X}=0$ para $k<0$.
	\end{definicao}
	...
	%	$$
	%	
	%	Um submódulo $H\subseteq G$ de um módulo graduado é um módulo graduado $H=\{H_k\}_{k\in\mathbb{Z}_+}$, onde $H_{k} \subseteq G_{k}$ é um submódulo para $k \in \inteiros_{+}$. Nesse caso, o módulo quociente $G/H$ é o módulo graduado $\{G_{k}/H_{k} \}_{k\in \inteiros}$.
	%	
	...
	$$
	é chamado de homologia singular do espaço $X$.
	...
	\begin{definicao}
		(Homologia do complexo de cadeia) Sejam $C=(C_{*}, \bordo{*})$ um complexo de cadeia, $Z_{k}(C) = \ker(\bordo{k})$ e $B_{k}(C) = \imagem{\bordo{k}}$, onde $k\in \inteirospos$. O $k$-ésimo módulo de homologia do complexo de cadeia $C$ é o módulo quociente
	...
	Analogamente a homotopia de caminhos definida na seção anterior, é possível definir homotopia entre quaisquer duas funções contínuas. Sejam $f, g: X\to Y$ duas aplicações contínuas entre espaços topológicos. Diz-se que $f$ e $g$ são homotópicas se existe uma aplicação contínua $h:\intervalo\times X \to Y$, tal que $h(0, x) = f(x)$ e $h(1, x) = g(x)$ para todo $x\in X$. A aplicação $h$ é chamada homotopia entre $f$ e $g$ e denota-se essa propriedade por $f\sim g$.
	...
	\begin{definicao}
		Uma tripla $\xymatrix{C\ar[r]^{f} & D\ar[r]^{g} & E }$ de módulos e homomorfismos é chamada exata se $\imagem{f} = \ker(g)$. Uma sequência de módulos e homomorfismos
		$$
		\xymatrix{\dots \ar[r]& G_{1}\ar[r]^{f_{1}} & G_{2}\ar[r]^{f_{2}} & G_{3}\ar[r]^{f_{3}} & \dots \ar[r]\ar[r]^{f_{k-1}} & G_{k}\ar[r]^{f_{k}} &\dots}
		$$
		é chamada de sequência exata longa se cada tripla é exata, isto é, $\imagem{f_{k}} = \ker(f_{k+1})$ para todo $k\in \inteiros$.
	\end{definicao}
	...
	$$
	Como $g$ é epimorfismo, dado $e \in \kernelbordoabrev{k}{E}$ existe $d \in D_{k}$ tal que $g_{k}(d) = e$. Pela comutatividade do diagrama, tem-se que $g_{k-1}(\bordo{k}'(d)) = \bordo{k}''(g_{k}(d)) = \bordo{k}''(e) = 0$. Portanto, $\bordo{k}'(d) \in \ker(g_{k-1}) = \imagem{f_{k-1}}$, pela exatidão. Com isso, pode-se tomar $c \in C_{k-1}$ tal que $f_{k-1}(c)=\bordo{k}'(d)$. Então $f_{k-2}(\bordo{k-1}(c)) = \bordo{k-1}'(f_{k-1}(c)) = \bordo{k-1}'(\bordo{k}'(d)) = 0$. Como $f$ é monomorfismo, então $c\in \kernelbordoabrev{k-1}{C}$. Logo, tem-se uma correpondência $\kernelbordoabrev{k}{E} \ni e \mapsto c \in \kernelbordoabrev{k-1 }{C}$, a qual é denotada por $\delta_{k}(e) = c$. Portanto a aplicação correspondente induzido em homologia $\delta_{*}:H_{*}(E) \to H_{*}(C)$ está bem definida e é chamada homomorfismo de conexão.
	...
	\begin{teorema}\label{teorema_homomorfismo_conectante}
		...
		$$
		\xymatrix{\dots \ar[r]^{f_{*}\hspace{3mm}} & \homologiaabrev{k}{D}\ar[r]^{g_{*}} & \homologiaabrev{k}{E}\ar[r]^{\delta_{k}\hspace{3mm}}& \homologiaabrev{k-1}{C}\ar[r]^{f_{*}} & \homologiaabrev{k-1}{D} \ar[r]^{\hspace{5mm} g_{*}}& \dots} 
		$$
		é exata.
	\end{teorema}
	...
	$$ 
	\xymatrix{0 \ar[r] & C_{k}(U\cap V)\ar[r]^{f\hspace{3mm}} & C_{k}(U)\oplus C_{k}(V)\ar[r]^{\hspace{8mm}g} & C_{k}(X) \ar[r]& 0},
	$$
	...
	\begin{teorema}
		...
		\[
		\xymatrix{\dots \ar[r]^{\delta_{k+1}\hspace{8mm}} & \homologiaabrev{k}{U\cap V}\ar[r]^{f_{*}\hspace{3mm}} & \homologiaabrev{k}{U}\oplus \homologiaabrev{k}{V}\ar[r]^{\hspace{10mm}g_{*}}& \homologiaabrev{k}{X}\ar[r]^{\delta_{k}\hspace{5mm}} & \homologiaabrev{k-1}{U \cap V} \ar[r]^{\hspace{10mm}f_{*}}& \dots} 
		\]
	...
	\begin{teorema}
		...
		$$
		\xymatrix{\dots \ar[r]^{\delta_{k+1}\hspace{3mm}} & \homologiaabrev{k}{U}\ar[r]^{i_{*}} & \homologiaabrev{k}{X}\ar[r]^{\pi_{*}\hspace{5mm}}& \homologiaabrev{k}{X, U}\ar[r]^{\delta_{k}} & \homologiaabrev{k-1}{U} \ar[r]^{\hspace{8mm}i_{*}}& \dots},
		$$
	...
	\begin{teorema}
		...
		$$
		\xymatrix{\dots \ar[r]^{\delta_{k+1}\hspace{8mm}} & \homologiaabrev{k}{U, U'}\ar[r]^{i_{*}} & \homologiaabrev{k}{X, U'}\ar[r]^{\pi_{*}}& \homologiaabrev{k}{X, U}\ar[r]^{\delta_{k}\hspace{3mm}} & \homologiaabrev{k-1}{U, U'} \ar[r]^{\hspace{8mm}i_{*}}& \dots},
		$$
		onde $\delta_{k}$ é o homomorfismo conectante para $k \in \inteirospos$.
	\end{teorema}
	...
	\begin{figure}[!h]
		\centering
		\includegraphics[height=3.5cm]{imagem/colagem_celula.pdf}
	...
	\begin{exemplo}\label{exemplo_cw_n_esfera}
		...
		\begin{figure}[!h]
			\centering
			\includegraphics[height=5cm]{imagem/cw_esfera.pdf}
	...
	\begin{exemplo}
		(Disco com alça) Sejam $\skeleton{0}=\{a,b\}$, $D = D^{1}_{1}\cup D^{1}_{2}\cup D^{1}_{3}$, onde $D^{1}_{1} = [0,1],D^{1}_{2} = [2,3]$ e $D^{1}_{3} = [4,5]$. Defina $f_{\partial_{1}}: D \to \skeleton{0}$ tal que 
		$$
		...
		\begin{figure}[!h]
			\centering
			\includegraphics[height=4.5cm]{imagem/disco_alca.pdf}
	...
	\begin{figure}[!h]
		\centering
		\includegraphics[height=3cm]{imagem/cw_2_toro.pdf}
	...
	\end{lema}
	\begin{prova}
		Por definição, tem-se $\skeleton{n} = \skeleton{n-1}\dot{\bigcup}_{\sigma}(\bigcup_{f_{\partial \sigma} } \celula{n}{\sigma})$ e também $\celula{n}{\sigma} \subset \skeleton{n}$, onde $f_{\sigma}:\celula{n}{\sigma} \to \skeleton{n}$ é a aplicação característica. Sejam $C_{\sigma}$ e $A_{\sigma}$ discos fechado e abertos, respectivamente, contendo o hemisfério norte de $f_{\sigma}(\celula{n}{\sigma})$. Definindo $N_{\sigma} = f_{\sigma}(\celula{n}{\sigma}) - C_{\sigma}$, $M_{\sigma} = f_{\sigma}(\celula{n}{\sigma}) - A_{\sigma}$ se tem $\overline{N_{\sigma}} \subset M_{\sigma}$, e considerando $U = \skeleton{n} - \bigcup_{\sigma} C_{\sigma}$, $Y = \skeleton{n} - \bigcup_{\sigma} A_{\sigma}$ se tem $U \subseteq Y$. Note que $\skeleton{n-1}$ é um retrato de deformação de $Y$, logo, pela invariância homotópica tem-se $\homologiarel{k}{\skeleton{n}}{\skeleton{n-1}} \cong  \homologiarel{k}{\skeleton{n}}{Y}$. Como $\skeleton{n} - U = \bigcup_{\sigma} C_{\sigma}$ e $Y - U $ é homotópico a $\bigcup S^{n-1}_{\sigma}$, pelo teorema da excisão $\homologiarel{k}{\skeleton{n} - U}{Y- U} \cong \homologiarel{k}{\skeleton{n}}{Y}$. Portanto $\homologiarel{k}{\skeleton{n} - U}{Y- U} = \homologiarel{k}{\dot{\bigcup }_{\sigma}C_{\sigma}}{\dot{\bigcup }_{\sigma}S^{n-1}_{\sigma}} \cong \homologiarel{k}{\dot{\bigcup}_{\sigma} \celula{n}{\sigma} }{\dot{\bigcup}_{\sigma} \celulabordo{n}{\sigma}} \cong \somadir{\sigma} \homologiarelcel{k}{n}{\sigma}$, pois $C_{\sigma} \approx \celula{n}{\sigma}$ e $S^{n-1}_{\sigma} = \celulabordo{n}{\sigma}$. Enfim, tem-se o diagrama comutativo:
		\[
		\xymatrix{
			...
			\\
			\homologiarel{k}{\skeleton{n} - U}{Y- U} \ar[r]^{\hspace{5mm}\cong} & \homologiarel{k}{\skeleton{n}}{Y} \ar[r]^{\cong\hspace{5mm}} & 
	...
	\begin{definicao}
		...
		\xymatrix{
			...
			\homologiarelskelesimpl{n}{n} \ar[r]^{\delta_{*}\hspace{3mm}} & 
			\homologiarelskele{n-1}{n-1}{n-2} \ar[r]^{\hspace{10mm}\Phi_{n-1}}&
			\mathcal{C}_{n-1}(X)
		}
		\]
	...
	\begin{prova}
		...
		\xymatrix{
			...
			\\
			\homologiarelskele{n}{n}{n-1} \ar[r]^{\hspace{5mm}\delta_{*} \qquad}\ar[rd]_{\delta_{n}} &
		...
		$$
		Note que $\delta_{*} \circ \delta_{*} = j_{*} \circ \delta_{n-1} \circ j_{*} \circ \delta_{n}$. Pela exatidão da sequência vertical tem-se $\imagem{j_{*}} = \ker(\delta_{n-1})$, logo $\delta_{*}^{2}=0$. Com isso, tem-se a composição do CW-bordo $\partial_{n-1}\circ \partial_{n} = \Phi_{n-2} \circ \delta_{*} \circ \Psi_{n-1} \circ \Phi_{n-1} \circ \delta_{*} \circ \Psi_{n} = \Phi_{n-2} \circ \delta_{*}^{2} \circ \Psi_{n} =0$, pois $\Psi_{n-1} \circ \Phi_{n-1}=1$ e $\delta_{*} ^2= 0$.
	...
	\begin{prova}
		...
		$$
		\xymatrix{
			\mathcal{C}_{k+1}(X) \ar[r]^{\partial_{k+1}} & \mathcal{C}_{k}(X) \ar[r]^{\partial_{k}\hspace{3mm}} & \mathcal{C}_{k-1}(X).
		}
		$$
		Por definição, tem-se $\homologia{k}{\mathcal{C}_{*}(X)} = \ker(\partial_{k})/\imagem{\partial_{k+1}}$, e com isso será provado que 
		$$
		\ker(\partial_{k})/\imagem{\partial_{k+1}} \cong \homologia{k}{X}.
		$$ 
		...
		Pode-se afirmar que $\phi: \ker(\delta_{2*}) \to \homologiarelskele{k}{k+1}{k-2}$, dado por $\phi(\classe{\alpha}) = \classe{\beta}$, é um epimorfismo. De fato, seja $\classe{\beta'} \in \homologiarelskele{k}{k+1}{k-2}$, então existe um $\classe{\alpha'} \in \homologiarelskele{k}{k}{k-1}$ tal que $i_{*} \classe{\alpha'} = j_{*} \classe{\beta'}$. Com isso $\delta_{2*}\classe{\alpha'} = \delta_{3*}\circ i_{*}\classe{\alpha'} = \delta_{3*}\circ j_{*}\classe{\beta'} = 0$, pois $\imagem{j_{*}} = \ker(\delta_{3*})$, logo $\classe{\alpha'} \in \ker(\delta_{2*})$ e $\phi$ é epimorfismo.
		
		Como $\phi$ é sobrejetor, existe $\classe{\alpha} \in \ker(\delta_{2*})$ tal que $\phi(\classe{\alpha}) = 0$. Pela comutatividade do diagrama tem-se que $i_{*}{\classe{\alpha}} = j_{*}\classe{0} = 0$, pois $j_{*}$ é monomorfismo, portanto $\classe{\alpha} \in \ker(i_{*})$ e, pela exatidão, tem-se $\ker(i_{*})=\imagem{\delta_{1*}}$, logo $\classe{\alpha} \in \imagem{\delta_{1*}}$. Com isso, conclui-se que $\ker(\phi) = \imagem{\delta_{1*}}$. Pelo teorema fundamental do isomorfismo de grupos, tem-se que $\ker(\delta_{2*})/\ker(\phi) \cong \homologiarelskele{k}{k+1}{k-2}$, ou seja, $\ker(\delta_{2*})/\imagem{\delta_{1*}} \cong \homologiarelskele{k}{k+1}{k-2}$.
		...
		Assim, $\homologia{k}{X} \cong \homologiarel{k}{\skeleton{k+1}}{\skeleton{k-2}} \cong \ker(\delta_{2*})/\imagem{\delta_{1*}}$. Mas como $\partial_{k} = \Phi_{n-1}\circ\delta_{*}\circ\Psi_{n}$, então $\ker(\delta_{2*}) \cong \ker(\partial_{k})$ e $\imagem{\delta_{1*}} \cong \imagem{\partial_{k+1}}$. Logo $\ker(\delta_{2*})/\imagem{\delta_{1*}} \cong \ker(\partial_{k})/\imagem{\partial_{k+1}} = \homologia{k}{\mathcal{C}_{*}}$. Portanto, $\homologia{k}{X} \cong \homologia{k}{\mathcal{C}_{*}}$ que é a equivalência entre as homologias.
	\end{prova}
	...
	\begin{exemplo}
		...
		\begin{figure}[!h]
			\centering
			\includegraphics[height=5cm]{imagem/cw_n_esfera.pdf}
			\caption{Estrutura de CW-complexo da n-esfera $S^{n}$.}
			\label{figura_cw_n_esfera}
		\end{figure} 
	\end{exemplo}
	
	\begin{exemplo}\label{exemplo_homologia_2_toro}
	...
	\begin{observacao}\label{observacao_conjunto_nivel}
	...
	\begin{observacao}\label{observacao_cw_complexo_pontos_criticos}
	...
	\begin{exemplo}
		...
		\right).
		$$
		Portanto, os índices $\lambda_{(\phi, \theta)}$ dos pontos críticos são: $\lambda_{(0,0)} = 2$, $\lambda_{(0,\pi)} = 0$, $\lambda_{(\pi,0)} = 1$ e $\lambda_{(\pi,\pi)} = 1$. 
		
		Pela Observação \ref{observacao_cw_complexo_pontos_criticos}, pode-se escrever
		$$
		T^{2} = f^{-1}([0, 2]) \approx \{\varphi(\pi,0)\}\cup_{f_{1\partial}}D^{\lambda_{(\pi, 0)}}\cup_{f_{2\partial}}D^{\lambda_{(\pi, \pi)}}\cup_{f_{3\partial}}D^{\lambda_{(0, 0)}},
		$$
		o que está de acordo com o Exemplo \ref{exemplo_cw_toro}, e sua homologia foi determinada no Exemplo \ref{exemplo_homologia_2_toro}.
		
		\begin{figure}[!h]
			\centering
			\includegraphics[height=5.2cm]{imagem/2_toro_funcao_largura.pdf}
	...
	Considere $\gradiente$ o campo gradiente da função de Morse $f$. Munindo $M$ com uma métrica Riemanniana $g_{p}: T_{p}M \times T_{p}M \to \reta$ (veja o Apêndice \ref{apendice_variedade_riemanniana}), tem-se que $df_{p}(v) = g_{p}(\nabla f(p), v)$ para todo $p\in M$, onde $v \in T_{p}M$. Sejam $-\gradiente \in \campossuaves{M}$ e $\gamma$ a curva integral de $-\gradiente $ tal que $\gamma(0) = p\in M$. Então
	$$
	\begin{aligned}
	...
	$$
	\funcaocond{h(t,q)}{\phi(t/(1-t), q)}{t\neq 1}{c(q)}{t=1},
	$$ 
	é uma homotopia entre $Id$ e $c$, pois é contínua e $h(0, q) = Id(q)$ e $\lim_{t \to 1}h(t, q) = c(q)$, logo a variedade instável é contrátil. Com uma argumento análogo mostra-se que $\variedadeestavel{p}$ é contrátil.
	...
	\begin{exemplo}
		...
		\begin{figure}[!h]
			\centering
			\includegraphics[height=6cm]{imagem/pontos_cristicos_morse_smale.pdf}
		...
		\begin{figure}[!h]
			\centering
			\includegraphics[height=5cm]{imagem/variedade_estavel_instavel.pdf}
	...
	%	Os espaços vetoriais simpléticos têm sua origem na sistematização do formalismo Hamiltoniano, o qual é uma descrição alternativa à formlução Newtoniana de sistemas físicos. 
	
	Nesse capítulo são introduzidos os espaços vetoriais simpléticos e demonstradas as suas principais propriedades. Tais definições e propriedades serão estendidas às variedades diferenciáveis, no Capítulo \ref{capitulo_equacao_floer}. Mais detalhes sobre os espaços vetoriais simpléticos podem ser encontrados em \cite{berndt}, \cite{bursztyn_macarini} e \cite{cannas_anna}.
	...
	Mais detalhes sobre o formalismo matemático da mecânica clássica podem ser encontrados em \cite{arnold} e \cite{artur_lopes}.
	...
	Com isso, o par $(\real{2n}, \formaSimpleticaabrev)$ é chamado 2n-espaço vetorial simplético. 
	
	Mais detalhes sobre a geometrizalção da mecânica clássica podem ser encontrados em \cite{arnold} e \cite{artur_lopes}.
	...
	Neste capítulo o grupo simplético será introduzido e será apresentado um estudo aprofundado das suas propriedades.  O grupo simplético e suas características topológicas exercem papel central na construção do índice de Maslov, introduzido  no Capítulo \ref{capitulo_indice_maslov}. Inicialmente serão apresentadas  suas principais propriedades, bem como a caracterização auto-valores de seus elementos e, por fim, será apresentado o cálculo de seu grupo fundamental. Mais detalhes sobre o grupo simplético podem ser encontrados em \cite{audi_floer_homology}, \cite{gonsson} e \cite{mcduff_salamon}.
	...
	Para estudar os auto-espaços generalizados de uma transformação simplética $A\in \gruposimpletico{\real{2n}}$, será considerada a complexificação do espaço $\real{2n}$.
	
	\begin{proposicao}\label{proposicao_auto_espaco_grupo_simpletico}
		Sejam $A \in \gruposimpletico{\real{2n}} \subset \generalgroupcomplexo{2n}$ e $r,s \in \inteiros$ tais que $r\geq 1$ e $s\geq 1$. Se $\lambda, \mu \in \sigma(A)$ tais que $\lambda\mu \neq 1$, então seus auto-espaços generalizados $E_{\lambda}=\ker(A-\lambda Id)^{r}$ e  $E_{\mu}=\ker(A-\mu Id)^{s}$ são $\Omega$-ortogonais, isto é, $\Omega(E_{\lambda}, E_{\mu}) = 0$.
	\end{proposicao}
	\begin{prova}
		Denote por $P(r,s)$ a propriedade que se quer mostrar. Note que $P(1,1)$ é verdadeira. De fato, dados $v\in E_{\lambda}$ e $u\in E_{\mu}$ tem-se que $\formaSimpleticaExtendida{v}{u} = \formaSimpleticaExtendida{Av}{Au} = \lambda\mu\formaSimpleticaExtendida{v}{u}$, o que implica que $\formaSimpleticaExtendida{v}{u} = 0$, pois $\lambda\mu\neq 1$. Por indução será mostrado que $P(r,s)$ é verdadeira para quaisquer $r,s$. Para aplicar a hipótese de indução no índice $s$, assuma que $P(1,s)$ é verdadeira e que $u \in \ker(A-\mu Id)^{s+1}$. Com isso, $0=(A-\mu Id)^{s+1}u = (A-\mu Id)^{s}(A-\mu Id)u $, logo $(A-\mu Id)u \in \ker(A-\mu Id)^{s}$, o que implica em
		$$
		\begin{aligned}
		\formaSimpleticaExtendida{v}{u}
		&=\formaSimpleticaExtendida{Av}{Au}
		\\
		&= \formaSimpleticaExtendida{Av}{Au -\mu u +\mu u} 
		\\
		&= \lambda\underbrace{\formaSimpleticaExtendida{v}{(A-\mu Id)u}}_{=0}+\lambda\mu\formaSimpleticaExtendida{v}{u}
		\\
		&=\lambda\mu\formaSimpleticaExtendida{v}{u}.
		\end{aligned}
		$$
		Como $\lambda\mu \neq 1$, então $\formaSimpleticaExtendida{v}{u}=0$ e $P(1, s+1)$ é verdadeira.
	...
	\begin{proposicao}\label{proposicao_potenciacao_grupo_simpletico}
		(Potênciação em $\gruposimpletico{\real{2n}}$) Seja $\gruposimpleticopositdef = \gruposimpletico{\real{2n}} \cap \matrizsimetricapositiva{2n}$ o conjunto das matrizes simpléticas simétricas e positivas-definidas. Dado $A \in \gruposimpleticopositdef$, então $A^{\alpha} \in \gruposimpletico{\real{2n}}$ para qualquer $\alpha \in \real{}$.
	\end{proposicao}
	\begin{prova}
		Se $A \in \gruposimpleticopositdef$, então $A$ é simétrica, logo é normal, e pelo Lema $\ref{lema_caracterizacao_matriz_normal}$ é diagonalizável. Além disso, pela Observação $\ref{observacao_matriz_positiva_definida}$ seus auto-valores são todos positivos. Pode-se decompor $V$ na soma direta de seus auto-espaços $E_{\lambda}$, onde $\lambda \in \espectrooperador{A}$. Se $u \in E_{\lambda_{u}}$ e $v \in V_{\lambda_{v}}$, então
		$$
	...
	\end{teorema}
	\begin{prova}
		Se $A \in \gruposimpletico{\real{2n}}$, então pelo Lema $\ref{lema_decomposicao_grupo_simpletico_positivo}$ é possível escrever $A=PO$, onde $P \in \gruposimpleticopositdef$ e $O\in \matrizSimpleticaOrtogonal$ são únicas. Pela unicidade da decomposição anterior, a aplicação $G: \gruposimpletico{\real{2n}} \to \gruposimpleticopositdef \times \matrizSimpleticaOrtogonal$ definida por $G(A) = (P,O)$ é injetora. Por outro lado, dado $(P,O) \in \gruposimpleticopositdef \times \matrizSimpleticaOrtogonal$ tem-se $(PO)^{t}\estruturacomplexa PO = O^{t}P^{t}\estruturacomplexa PO = O^{t}\estruturacomplexa O = \estruturacomplexa$, logo pela Proposição $\ref{proposicao_caracterizacao_Sp2n}$, tem-se $PO \in \gruposimpletico{\real{2n}}$ e $G$ é sobrejetora. De fato, definindo $A=PO \in \gruposimpletico{\real{2n}}$ tem-se que $G(A) = (P,O)$. Portanto $G$ é bijetora. 
		
		Denotando por $G^{-1}:\gruposimpleticopositdef \times \matrizSimpleticaOrtogonal\to \gruposimpletico{\real{2n}}$ a inversa de $G$, tem-se que $G^{-1}(P,O) = PO$, o que implica que $G^{-1}$ é contínua, pois o produto de matrizes é uma operação contínua.
		
		Veja que $\gruposimpleticopositdef\times \matrizSimpleticaOrtogonal$ é conexo por caminhos, pois é o produto cartesianos de espaços topológicos conexos por caminhos. Além disso, como a conexidade é preservada por aplicações contínuas, então $\gruposimpletico{\real{2n}}$ é conexo por caminhos, logo é conexo.
	\end{prova}
	
	\begin{observacao}\label{observacao_decomposicao_Sp2n}
		No teorema anterior, foi exibida uma bijeção entre $\gruposimpletico{\real{2n}} $ e o produto cartesiano $\gruposimpleticopositdef \times \matrizSimpleticaOrtogonal$, isto é, toda $A \in \gruposimpletico{\real{2n}}$ pode ser decomposta unicamente como $A=PO$, onde $P\in \gruposimpleticopositdef$ e $O \in \matrizSimpleticaOrtogonal$. Essa afirmação será utilizada na demonstração de alguns resultados adiante.
	\end{observacao}
	...
	\end{teorema}
	\begin{prova}
		Na Observação $\ref{observacao_decomposicao_Sp2n}$ foi mostrado que se $A \in \gruposimpletico{\real{2n}}$ então $A=PO$, onde $P \in \gruposimpleticopositdef$ e $O \in \matrizSimpleticaOrtogonal$. Com isso tem-se $AA^{t} = POO^{t}P^{t} = PP^{t}=P^{2}$, e ela Proposição $\ref{proposicao_potenciacao_grupo_simpletico}$, é possível afirmar que $P^{\alpha} \in \gruposimpletico{\real{2n}}$ para todo $\alpha \in \real{}$. Considere a aplicação $r:\gruposimpletico{\real{2n}}\times [0,1] \to \gruposimpletico{\real{2n}}$ tal que $r(A, \alpha) = (AA^{t})^{-\alpha/2}A$. Note $r$ que é contínua pois é o produto de matrizes, que é uma operação contínua em $\gruposimpletico{\real{2n}}$. Além disso, $r$ é um retrato de deformação de $\gruposimpletico{\real{2n}}$ sobre $\matrizSimpleticaOrtogonal$ pois $r(A, 0) = A$, $r(A, 1) = (AA^{t})^{-1/2}A = P^{-1}A = O \in \matrizSimpleticaOrtogonal$ e, tomando $B \in \matrizSimpleticaOrtogonal$, $r(B, 1) = (BB^{t})^{-1/2}B = B$, pois $BB^{t} = Id$.
	...
	O primeiro passo para a construção do índice de Maslov é a construção de uma aplicação contínua $\rho: \gruposimpletico{\real{2n}} \to \circulo$, que é uma extenção contínua do determinante complexo e induz o isomorfismo entre os grupos fundamentais, além de satisfazer propriedades que serão herdadas pelo índice de Maslov.
	
	\begin{teorema}\label{teorema_aplicacao_rho}
		Para todo $n\ \in \mathbb{N}^{*}$ existe uma aplicação contínua $\rho:\gruposimpletico{\real{2n}} \to S^{1}$ satisfazendo as seguintes propriedades:
		\begin{enumerate}
			...
			\item \label{item_produto_rho} \textbf{Produto:} Se  $A_{1} \in\gruposimpletico{\real{2m}}$ e $A_{2} \in \gruposimpletico{\real{2n}}$, então
			$$
			\rho 
			\left(
			\begin{array}{cc}
			...
			\end{array} 
			\right)
			= \rho_{1}(A_{1})\rho_{2}(A_{2}),
			$$
			onde $\rho_{1} = \rho|_{\gruposimpletico{\real{2m}}}$ e $\rho_{2} = \rho|_{\gruposimpletico{\real{2n}}}$.
	...
	\begin{corolario}\label{corolario_nao_degeneracidade_Q}
		Se $\lambda\in \espectrooperador{A} \cap \complementar{\circulo}{\{\pm 1 \} }$, então a forma quadrática $Q$ é não-degenerada e $Q(E_{\lambda})\subset \reta_{+}$ ou $Q(E_{\lambda})\subset \reta_{-}$.
	\end{corolario}
	\begin{prova} Pelo Lema \ref{lema_forma_simetrica_B} segue que $Q$ é não-degenerada. Seja $\lambda\in \espectrooperador{A} \cap \complementar{\circulo}{\{\pm 1 \} }$. Pela Proposição \ref{proposicao_forma_quadratica_Q} tem-se que $Q(v)>0$ ou $Q(v)<0$, para todo $v\in \autoespaco{\lambda}$. 
	\end{prova}
	...
	Sejam $m(\lambda)$ a multiplicidade do auto-valor $\lambda \in \espectrooperador{A}$  e $\gruposimpleticoespecial{\real{2n}} =\{A\in \gruposimpletico{\real{2n}} : m(\lambda) = 1,\;\forall \lambda\in \espectrooperador{A} \}$ o conjunto das matrizes simpléticas cujos auto-valores têm multiplicidade igual a 1.
	
	Considere a aplicação $\hat{\rho}:\gruposimpleticoespecial{\real{2n}} \to \circulo$ definida por 
	$$
	...
	Pode-se afirmar que a restrição $\rho|_{\gruposimpleticoespecial{\real{2n}}}$ coincide com a aplicação $\hat{\rho}$ anteriormente definida. De fato, tomando $A \in \gruposimpleticoespecial{\real{2n}}$ tem-se que $m(\lambda)=1$ e portanto $m_{+}(\lambda) \in \{0,1\}$ para todo $\lambda\in \espectrooperador{A}$. Suponha uma ordem de primeiro tipo para os auto-valores de $A$. Sem perda de generalidade, pode-se supor que $\lambda_{j} \in \reta$ para $1\leq j\leq r$ e $\lambda_{j}\in \complexo{}$ para $r+1 \leq  j\leq n$. Se $\lambda_i\in \real{+}$ então $\lambda_i/|\lambda_i|=1$. Se $\lambda_i\in \real{-}$ então $\lambda_i/|\lambda_i|=-1$. Além disso, $m_{+}(\lambda)=m_{-}(\bar{\lambda})$ e $m_{-}(\lambda)=m_{+}(\bar{\lambda})$, logo $m_{+}(\lambda)=1$ se, e somente se, $m_{+}(\bar{\lambda})=0$. Então  
	$$
	\begin{aligned}
	\hat{\rho}(A) 
	&=
	...
	\begin{proposicao}\label{proposicao_continuidade_rho}
		A aplicação $\rho: \gruposimpletico{\real{2n}}\to \circulo$ é contínua e $\rho|_{\gruposimpleticoespecial{\real{2n}}} = \hat{\rho}$.
	\end{proposicao}
	...
	\begin{prova}
		...
		Primeiramente, será mostrado que $W(\complementar{\reta}{\{0,1\}}) \subset \gruposimpleticonaodegenerado{*}$. De fato, 
		$$
	...
	Considere as matrizes diagonais $W^{+}\in \gruposimpleticonaodegenerado{+}$ e $W^{-}\in \gruposimpleticonaodegenerado{-}$, definidas por
	$$
	...
	\begin{definicao}
		...
		\begin{figure}[!h]
			\centering
			\includegraphics[height=4cm]{imagem/caminhos_especiais_em_Sp2n.pdf}
	...
	\begin{prova}
		...
		\begin{enumerate}
			\item Tem-se que $\rho_{\Psi}(0) = \rho(\Psi(0)) = 1$ e $\rho_{\Psi}(1) = \rho(\Psi(1)) = \rho(W^{\pm})$, onde $\rho(W^{+})=(-1)^{n}$ e $\rho(W^{-})=(-1)^{n-1}$, o que implica em $\rho(W^{\pm}) \in \{ -1,1\}$. Sabe-se que $\rho_{\Psi}(t) = \exp(i\alpha_{\Psi}(t))$. Então $\alpha_{\Psi}(0)=0$ e $\alpha_{\Psi}(1)=k\pi$. De fato, para $t=0$ é imediato que $\alpha_{\Psi}(0)=0$. Para $t=1$ tem-se que $\rho(W^{\pm}) = \pm 1$, o que implica que $\alpha_{\Psi}(1) = k\pi$, para algum $k\in \inteiros$. Logo $\mu(\psi) = 2\deg(\rho_{\Psi}) = (k\pi-0)/\pi \in \inteiros$.
			...
			Seja $\phi'\in \caminhos(\gruposimpletico{\real{2n}})$ tal que $\phi'(0) = \phi(1)$ e $\phi'(1) = Id$, e defina $\gamma'=\phi'\gamma\phi'^{-1}$. Então $\Psi'=\psi'*\gamma'$ é um prolongamento de $\psi'$. De fato, 
			$$
			...
			\begin{figure}[!h]
				\centering
				\includegraphics[height=4cm]{imagem/caminho_naturalidade.pdf}
			...
			\item \textbf{(Assinatura)} O primeiro passo é mostrar que, $\exp(\estruturacomplexa S) \in\gruposimpletico{\real{2n}}$. Para isso é suficiente mostrar que $(\exp(\estruturacomplexa S))^{t}\estruturacomplexa \exp(\estruturacomplexa S)= \estruturacomplexa$. Note que $(\estruturacomplexa S)^{t} 
			...
			\item \textbf{(Inverso)} Pelo Lema \ref{lema_inversa_caminho_especial}, tem-se que $\psi^{-1} \in \caminhosespeciais{\gruposimpletico{\real{2n}}}$. Segue que $\Psi^{-1}= \psi^{-1}*\gamma^{-1} \in \caminhosespeciais{\gruposimpletico{\real{2n}}}$. De fato, se $\lambda(t)$ é um auto-valor de $\gamma(t)$, então $1/\lambda(t)$ é um auto-valor de $\gamma^{-1}(t)$. Como $1 \notin\espectrooperador{\gamma(t)}$, então $1 \notin\espectrooperador{\gamma^{-1}(t)}$
			...
			\begin{figure}[!h]
				\centering
				\includegraphics[height=5.5cm]{imagem/prolongamento_homotopico.pdf}
			...
			\begin{figure}[!h]
				\centering
				\includegraphics[height=5cm]{imagem/prolongamento_homotopico_2.pdf}
	...
	\begin{observacao}Nem toda $2n$-variedade é uma variedade simplética. De fato, se $(M,\omega)$ é uma $2n$-variedade simplética, então  $M$ é orientável.  Logo, as variedades diferenciáveis não-orientáveis não são variedades simpléticas.
	\end{observacao}
	...
	\begin{figure}[!h]
		\centering
		\includegraphics[height=5cm]{imagem/caminho_espaco_laco.pdf}
	...
	\begin{observacao}
		...
		\begin{figure}[!h]
			\centering
			\includegraphics[height=3cm]{imagem/colagem_discos.pdf}
	...
	\begin{figure}[!h]
		\centering
		\includegraphics[height=5.5cm]{imagem/extensao_solucao.pdf}
	...
	\begin{observacao}\label{observacao_uniao_orbitas_conectantes}
		...
		$$
		Além disso, segue do Teorema $\ref{teorema_compacidade_gromov}$ que $\energiafinitaM$ é um conjunto compacto.
	\end{observacao}
	
	\begin{observacao}
		Em \cite{audi_floer_homology} está demonstrado que, se $u:\reta\times \circulo\to M$ é uma solução de classe $C^{1}$ da equação de Floer, então $u$ é de classe $C^{\infty}$. Além disso, em $\energiafinitaM$, as topologias $C^{0}_{\text{loc}}$, $C^{1}_{\text{loc}}$ e $C^{\infty}_{\text{loc}}$ coincidem.
	\end{observacao}
	...
	\begin{corolario}
		Sejam $(M,\omega)$ uma 2n-variedade simplética, $H:M\to \reta$ uma função Hamiltoniana autônoma e $x \in \pontoscriticos{H}$. Se $\norma{\hessianaponto{x}{H}}<2\pi$ (assumindo a $\cktopologia{2}$), então o índice de Conley-Zehnder $\iconley{x}$ de $x$ como uma solução periódica do sistema Hamiltoniano e seu índice $\ind(x)$ como ponto crítico da função $H$ são relacionados por
		$$
		\iconley{x} = \ind(x)-n.
		$$
	\end{corolario}
	...
	\begin{figure}[!h]
		\centering
		\includegraphics[height=4.0cm]{imagem/colagem_disco_cilindro.pdf}
	...
	\subsection{A Transversalidade do Operador $\operadorFloer$ e a Compacidade do Espaço Moduli $\quocientetrajetorias$}\label{secao_transversalidade_compacidade_M}
	...
	\begin{prova} Tome $h \in \perturbacaoHamiltoniana{H}$. Pelo teorema da pré-imagem de valores regulares, tem-se que $\pi^{-1}(h)$ é uma variedade e sua dimensão é dada por 
		$$
		\begin{aligned}
		\dim(\ker(D_{(u, H+h)}\pi)) &= \dim(\ker(\diferencialfloerponto{u}))-\underbrace{\dim(\coker(\diferencialfloerponto{u}))}_{=0}
		\\
		...
		\end{aligned}
		$$
		onde foi usado o fato que $\diferencialfloerponto{u}: \espacosobolev{\pullbackfibradotangenteM{u}} \to \espacoLp{\pullbackfibradotangenteM{u}}$  é sobrejetor, o que implica em
		$$
	...
	O espaço moduli de $x^{-}, x^{+}$ é o espaço quociente 
	...
	\begin{prova}
		...
		\end{aligned}
		$$
		Portanto $f_{V}$ é $\complexo{}$-linear. Além disso $f_V$ é injetora. De fato, dado $u \in \complexificacao{V}$ tal que $f_{V}(u) = 0$,  então $1\otimes_{\reta}u_{1}+i\otimes_{\reta} u_{2}=0$ e, portanto, $u_{1}=u_{2}=0$.
	...
	\begin{prova}
		Pelo Teorema $\ref{teorema_isomorfismo_complexificacao}$, tem-se que $\complexificado{V}$ é um 2n-espaço vetorial complexo. Por construção $\Omega$ é uma aplicação $\complexo{}$-bilinear. Como $\omega^{ij}  = -\omega^{ji} $, tem-se que $\Omega$ é anti-simétrica. Além disso, $\Omega$ é não degenerada. De fato, tomando $v, u \in \complexificado{V}$ tal que $v= \complexificacaoelemento{x}{\alpha}$ e $u=\complexificacaoelemento{y}{\beta} $ tem-se, pelo Exemplo $\ref{exemplo_complexificacao_tensorial}$, que 
		$$
		\begin{aligned}
	...
	Suponha que $T=\complexificacaoelemento{1}{A}$ para algum operador linear $A:V \to V$. Como $T$ é uma transformação simplética, então $A$ é uma transformação simplética. De fato, tomando $u = \complexificacaoelemento{x}{\alpha}$ e $v=\complexificacaoelemento{y}{\beta}$ em $\complexificado{V}$ tem-se que $Tu = \complexificacaoelemento{x}{A\alpha}$ e $Tv= \complexificacaoelemento{y}{A\beta}$
	$$
	\begin{aligned}
	\Omega(Tv,Tu)
	&= \sum_{i,j = 1}^{2n}
	...
	Nesse apêncide serão apresentados alguns resultados preliminares necessários para o estudo da topologia de $\gruposimpletico{\real{2n}}$ e do conjunto das estruturas complexas $\estruturascomplexas{V}{\omega}$. As principais referências para o conteúdo aqui desenvolvidos são \cite{halmos} e \cite{hoffman_kunze}.
	...
	$$
	cujas soluções podem não existir no sentido usual, mas podem ser determinadas usando distribuições, como será mostrado a seguir.
	
	
	Nesse apêndice serão apresentadas  algumas definições com o objetivo principal de definir os espaços de Sobolev.
	...
	\begin{exemplo}
		...
		Note que $\partial\theta = \delta \in \distribuicoes$. De fato, é imediato que $\theta$ é localmente integrável, logo $\theta \in \distribuicoesgeral{\reta}$. Dado $\phi \in \funcoestestegeral{\reta}$ e definindo $K=supp(\phi)$, tem-se que 
	...
	\begin{thebibliography}{9}
		...
		\bibitem{artur_lopes}
		Lopes, Artur.
		\emph{Introdução à Mecânica Clássica},
		Edusp, Coleção Acadêmica, 2013.
