...
\begin{document}
	...
	\begin{prova}
		\begin{enumerate}
			...
			\right)
			$$ 
			é a matriz $2\times 2$ de rotação por um ângulo $-i\epsilon$ no plano $\real{2}$. Realizando as identificações $r(-t\epsilon) \mapsto \exp(-it\epsilon)$ e $R(-t\epsilon)\mapsto \exp(-it\epsilon)Id$, tem-se o determinante $det_{\complexo{}}(R(-t\epsilon)) = \exp(-int\epsilon)\in \circulo$.
			
			Note que, $\exp(\estruturacomplexa S)$ é uma matriz ortogonal e, como foi mostrado anteriormente, é uma transformação simplética, logo $\exp(t\estruturacomplexa S) \in \matrizSimpleticaOrtogonal$. Pela propriedade do determinante da aplicação $\rho:\gruposimpletico{2n}\to \circulo$ tem-se que 
			$$
			...
			\item \textbf{Caso $a_{(1)} = a_{(2)} = (-\epsilon,\dots,-\epsilon)$:} esse caso é análogo ao anterior, contudo, tem-se $R(t\epsilon)$ e $det_{\complexo{}}(R(t\epsilon)) = \exp(int\epsilon)$, o que implica em $\mu(\Psi_{s}) = 2deg(\exp(t\estruturacomplexa S)) = n$. Como $Ind(S) = 2n$, então $ Ind(S)-n = 2n-n = n = \mu(\Psi_{s})$, como desejávamos.
			
			\item \textbf{Caso $a_{(1)}(\epsilon,\dots,\epsilon), \; a_{(2)} = (-\epsilon,\dots,-\epsilon)$:}????????
		\end{enumerate}
	\end{prova}
