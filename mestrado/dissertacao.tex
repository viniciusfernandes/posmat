...
\begin{document}
	...
	\section{Índice de Maslov $\mu : \caminhosespeciais{\gruposimpletico{2n}} \to \inteiros$}
	...
	\begin{teorema}
		...
		\begin{enumerate}
			...
			\item $sign(det(Id-\gamma(1))) = (-1)^{\mu(\psi)-n}$.
	...
	\begin{prova}
		\begin{enumerate}
			...
			\item Sejam $\psi_{1}, \psi_{2}\in \caminhosespeciais{\gruposimpletico{2n}}$ e $h:\intervalo\times \intervalo \to \gruposimpletico{2n}$ uma homotopia entre eles tal que $h(t,0) = \psi_{1}(t)$ e $h(t,1) = \psi_{2}(t)$.
			...
			\item Suponha que $det(Id-\gamma(1) )>0$, ou seja, $sign(det(Id-\gamma(1) )) = (-1)^{m}$ para algum $m\in 2\inteiros$. Com isso, $\psi(1)=W^{+}$ e $\rho_{\psi}(1) = \rho(W^{+}) = (-1)^{n}$. Se $\rho(W^{+}) =1$, então $n\in 2\inteiros$, o que implica em $\alpha_{\psi}(1) = k\pi$ para $k\in 2\inteiros$. Logo $\mu(\psi) = (k\pi-0)/\pi \in 2\inteiros$, e podemos definir $m=\mu(\psi)-n \in 2\inteiros$. Se $\rho(W^{+}) =-1$, então então $n\in 2\inteiros+1$, o que implica em $\alpha_{\psi}(1) = k\pi$ para $k\in 2\inteiros+1$. Logo $\mu(\psi) = (k\pi-0)/\pi \in 2\inteiros+1$, e podemos definir $m=\mu(\psi)-n \in 2\inteiros+1$. Com argumentos análogos, pode-se mostrar que $sign(det(Id-\gamma(1) )) = (-1)^{\mu(\psi)-n }$ para $det(Id-\gamma(1) )<0$.
			...
			Sabendo que $((\estruturacomplexa S)^{k})^{t} = (S^{t})^{k}(\estruturacomplexa^{t})^{k}$ segue-se que
			$$
			\begin{aligned}
			...
			Pode-se afirmar que $S$ é diagonalizável pois é uma matriz simétrica. Além disso, existe uma matriz ortogonal $P \in \matrizortogonal{2n}$ tal que $P^{t}SP$ é diagonal (tal resultado pode ser encontrado em \cite{hoffman_kunze}). Seja $\mathcal{P}:[0,1]\to \matrizortogonal{2n}$ um caminho contínuo tal que $\mathcal{P}(0) = Id$ e $\mathcal{P}(1)=P$ e $\mathcal{S}:\intervalo\to \matrizquadcomplexa{2n}$ dada por $\mathcal{S}(s)=\mathcal{P}^{t}(s)S\mathcal{P}(s)$. Do fato de que $\mathcal{P}^{t}(s)=\mathcal{P}^{-1}(s)$ tem-se que $det(\mathcal{S}(s)) =det(\mathcal{P}^{t}(s)S \mathcal{P}(s)) = det(S)$, portanto $\mathcal{S}(s)$ é diagonalizável e $\espectrooperador{\mathcal{S}(s)} = \espectrooperador{S}$ para todo $s \in [0,1]$. Além disso, $\mathcal{S}(s)^{t}$ é simétrica. De fato, $\mathcal{S}(s)^{t} = (\mathcal{P}^{t}(s)S\mathcal{P}(s))^{t} = \mathcal{P}^{t}(s)S^{t}\mathcal{P}(s) = \mathcal{P}^{t}(s)S\mathcal{P}(s) = \mathcal{S}(s)$, o que implica em $\Psi_{s}(t)=\exp(t\estruturacomplexa \mathcal{S}(s)) \in \gruposimpletico{2n}$.

			Dado um inteiro $k\geq 0$ temos que $\estruturacomplexa^{2k} = (-1)^{k}$ e $\estruturacomplexa^{2k+1} = (-1)^{k}\estruturacomplexa$. Com isso tem-se que
			$$
			\begin{aligned}
			...
			\end{aligned}
			$$
			onde $a_{(1)}Id$ é a matriz $diag=\{a_{1}, \dots, a_{n}\}$. O análogo vale para $a_{(2)}Id$ . Com isso, tem-se a exponencial
			$$
			\begin{aligned}
			\exp(\estruturacomplexa S) &= \sum_{j=0}^{\infty}\bigparenteses{\frac{(\estruturacomplexa S)^{j}}{(2j)!} + \frac{(\estruturacomplexa S)^{2j+1}}{(2j+1)!}}
			\\
			&= \sum_{j=0}^{\infty}\frac{(-1)^{j}(a_{(1)}.a_{(2)})^{j}}{(2j)!}Id + \frac{(-1)^{j}(a_{(1)}.a_{(2)})^{j}}{(2j+1)!}	\left(
			\begin{array}{cc}
			0 & a_{(2)}Id
			\\
			-a_{(1)}Id & 0  
			...
			Como $|a_{j}|<2\pi$, a análise pode ser reduzida ao caso em que  $|a_{j}|=\epsilon \in (0,2\pi)$ para $1\leq j \leq 2n$. Com isso, tem-se
			\begin{enumerate}
				\item \textbf{Caso} $a_{(1)} = a_{(2)} = (\epsilon,\dots,\epsilon)$:
				$$
					\exp(\estruturacomplexa S) = \cos(\epsilon)Id+ \sin(\epsilon)\estruturacomplexa=	\left(
					\begin{array}{cc}
					(\cos(\epsilon))Id & (\sin(\epsilon))Id
					\\
					-(\sin(\epsilon))Id & (\cos(\epsilon))Id
					\end{array}
					\right).
					\
				$$
			...
			Note que, nesse caso, $\exp(\estruturacomplexa S)$ é uma matriz ortogonal, logo $\exp(t\estruturacomplexa S) \in \matrizSimpleticaOrtogonal$. Pela propriedade do determinante da aplicação $\rho:\gruposimpletico{2n}\to \circulo$ tem-se que $\rho(\exp(t\estruturacomplexa S)) = det(\cos(t\epsilon)Id +i\sin(t\epsilon)) = \exp(it\epsilon) \in \circulo$.
