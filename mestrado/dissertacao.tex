...
\begin{document}
	...
	De agora em diante $M$ será uma n-variedade Riemanniana diferenciável fechada.
	...
	Sejam $\gamma \in \lacocontrateis$ e $X \in \espacotangenteponto{\gamma}{\lacocontrateis}$. Considere o caminho $u : \reta \to \lacocontrateis$ como solução do sistema $u'(0)=X $ e $u(0) = \gamma$. Com isso, $u(s)\in \lacocontrateis$ para todo $s\in \reta$, isto é, $u(s):\circulo\to M$, onde $u(0,t) = \gamma(t)$ para todo $t\in \circulo$. Portanto
	$$
	\derivadaparcial{}{s}u(s,t)|_{s=0}=X(t) \in \espacotangenteponto{\gamma(t)}{M}.
	$$

	O que resulta na identificação $\espacotangenteponto{\gamma}{\lacocontrateis} \mapsto \pullbackfibradotangente{\gamma}{M}$. Além disso, como $u$ e de classe $C^{\infty}$, então $D_{(s,t)}u:\espacotangenteponto{(s,t)}{(\retacartesianocirculo)} \to \espacotangenteponto{u(s,t)}{M}$ está bem-definido e $2=dim(D_{(s,t)}u) = dim(\retacartesianocirculo)$. Logo $u$ é uma imersão de $\retacartesianocirculo$ em $M$. Como consequência, dado $X \in \espacotangenteponto{\gamma}{\lacocontrateis}$, temos $X(t) \in \espacotangenteponto{\gamma(t)}{M}$ para todo $t \in \reta$, 
	\begin{definicao}
