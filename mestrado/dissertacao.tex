...
\begin{document}
	...
	\begin{aligned}
	df_{H}(\gamma)(Y) 
	&=  \int_{[0,1]} \omega(\dot{\gamma}(t) - X(\gamma(t), t), Y(\gamma(t)))dt
	\\
	&= \int_{[0,1]} \omega(J(\gamma(t))(\dot{\gamma}(t) - X(\gamma(t), t)), J(\gamma(t))Y(\gamma(t)))dt
	\\
	&= \int_{[0,1]} \omega(\nabla f(\gamma(t)), J(\gamma(t))Y(\gamma(t)))dt,
	\end{aligned}
	$$
	logo 
	$$
	...
	Sejam $y, x\in \lacocontrateis$ e defina $\mathcal{M}(y, x)$ como sendo o espaço das órbitas conectantes com respeito ao fluxo gradiente de $f_{H}$. Essas são soluções da equação de Floer com a seguinte condição de contorno \vermelho{(estamos supondo que as mesmas existem)}
	$$
	...
	$$
	vamos definir $
	\mathcal{M} = \{u \in \mathcal{M}(y,x):y,x \in \lacocontrateis,\; |E(u)| < \infty, \; u \sim p \in M\}
	...
	\begin{definicao}\label{definicao_caminhos_decaimentos_exponenciais}
		(Caminhos de decaimentos exponenciais) Sejam $x^{-}, x^{+} \in \lacocontrateis$. Denotamos o conjunto dos caminhos suaves com decaimento exponencial conectando $x^{-}$ a $x^{+}, $ por
		$$
	...
	\begin{proposicao}\label{proposicao_variedade_banach}
		Sejam $x^{-}, x^{+} \in \lacocontrateis$ soluções contráteis do sistema Hamiltoniano, então $\caminhosexponenciaisconectantespadrao$ é uma variedade de Banach.
	\end{proposicao}
	\begin{prova}
	...
	Dado $x \in \lacocontrateis$ temos que $\dot{x}(t) = \campohamiltoniano{x(t)}$. Seja $y \in \lacocontrateis$ uma outra solução do sistema Hamiltoniano e suficientemente próxima a $x$. Realizando os mergulhos $\pullbackfibradotangenteM{x}, \pullbackfibradotangenteM{y} \subset \real{k}$, para um $k$ suficientemente grande, temos  $x(t), y(t) \in \real{k}$ para todo $t \in \circ$, logo existe $\xi:\real{}\to \real{k}$ talq ue $\xi(t)=x(t)-y(t)$, portanto
	$$
