...
\begin{footnotesize}
	...
	\begin{frame}
		...
		\begin{teorema}
			Sejam $f\in \funcoesmorse{M}$ e $p\in \pontoscriticos{f}$ com índice $\lambda_{p}$ tal que $f(p) = b$. Suponhamos que $f^{-1}([b-\epsilon,b+\epsilon])$ seja compacto e $f^{-1}([b-\epsilon,b+\epsilon])\cap \pontoscriticos{f} = \{p\}$ para algum $\epsilon>0$. Então o conjunto de nível $M^{b+\epsilon}$ tem o mesmo tipo de homotopia de $M^{b+\epsilon}$ com uma $\lambda_{p}$-célula colada, isto é, $M^{b+\epsilon} \simeq M^{b-\epsilon}\cup_{f_{\partial}} D^{\lambda_{p}}$.
		\end{teorema}
		
		\begin{observacao}
			O teorema anterior tem como hipótese a exitência de apenas um ponto crítico em $f^{-1}([b-\epsilon,b+\epsilon])$. No caso em que $f^{-1}([b-\epsilon,b+\epsilon]) \cap \pontoscriticos{f} = \{p_{j}\}_{j=1}^{r}$, teremos $M^{b+\epsilon} \simeq M^{b-\epsilon}\cup_{f_{\partial_{1}}} D^{\lambda_{1}}\dots  \cup_{f_{\partial_{r}}} D^{\lambda_{r}}$.
		\end{observacao}
	\end{frame}
	...
	\begin{frame}
			...
			\begin{figure}[!h]
				\centering
				\includegraphics[width=10cm]{../imagem/caminhos_especiais_em_Sp2n.pdf}
			\end{figure}
	\end{frame}
		...
		\begin{prova}
			\begin{enumerate}
				...
				\begin{itemize}
					...
					\begin{scriptsize}
						\item $W(a) = \{a,-1,\dots,-1, a^{-1},-1,\dots,-1\}$,$W^{+}=W(-1) $ e $W^{-}=W(2) $.
						
						\item $\rho_{\Psi}(0) = \rho(\Psi(0)) = 1$ e $\rho_{\Psi}(1) = \rho(\Psi(1)) = \rho(W^{\pm})$
						
						\item $\rho(W^{+})=(-1)^{n}$ e $\rho(W^{-})=(-1)^{n-1} \Rightarrow \rho(W^{\pm}) \in \{ -1,1\}$. 
						
						\item $\rho_{\Psi}(t) = \exp(i\alpha_{\Psi}(t))$. 
						\item $t=0\Rightarrow \alpha_{\Psi}(0)=0$ e $t=1\Rightarrow \rho(W^{\pm}) = \pm 1$. 
						
						\item $\alpha_{\Psi}(1) = k\pi$, para algum $k\in \inteiros$ 
						$$
						\mu(\psi) = 2\deg(\rho_{\Psi}) = (k\pi-0)/\pi \in \inteiros.$$
				...
				\begin{itemize}
					\begin{scriptsize}
						...
						\item $(\Leftarrow) \mu(\psi_{1}) = \mu(\psi_{2})$ implica imediatamente que $\Psi_{1}\sim \Psi_{2}$, logo $\psi_{1}\sim \psi_{2}$.
	...
	\begin{frame}
		\frametitle{Diferencial do Funcional de Ação}
			...
			$$
				\begin{figure}[!h]
					\centering
					\includegraphics[height=3cm]{../imagem/extensao_solucao.pdf}
